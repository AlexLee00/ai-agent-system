...
const CONTINUOUS = ['ai.jay.runtime', 'ai.n8n.server'];
const ALL_SERVICES = ['ai.jay.runtime', 'ai.n8n.server'];
...
function buildActionLines(report) {
  ...
  if (report.serviceHealth.warnCount > 0) {
    lines.push('  - launchd 경고 서비스 확인: launchctl list | rg "ai\\.(jay\\.runtime|n8n\\.server)"');
  }
