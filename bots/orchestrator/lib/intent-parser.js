...
const INTENT_PRIMARY_MODEL = 'gpt-5-mini';
const INTENT_PRIMARY_PROVIDER = 'openai';
const INTENT_FALLBACK_MODEL = 'gemini-2.5-flash';
const INTENT_FALLBACK_PROVIDER = 'google';
...
const KEYWORD_PATTERNS = [
  ...
  { re: /(스카|ska).*(헬스|건강|운영.*상태|운영상태|운영.*리포트|헬스.*리포트|헬스.*보고)|예약팀.*(헬스|건강|운영.*상태|운영.*리포트)/i, intent: 'ska_health', args: {} },
  { re: /(블로|blog|블로그).*(헬스|건강|운영.*상태|운영상태|운영.*리포트|헬스.*리포트|헬스.*보고)|블로그팀.*(헬스|건강|운영.*상태|운영.*리포트)/i, intent: 'blog_health', args: {} },
  { re: /(전체|통합|운영).*(헬스|건강|상태).*(브리핑|액션|조치)|헬스.*(브리핑|액션|조치).*(전체|통합)|ops.*health.*briefing/i, intent: 'ops_health', args: { query: 'briefing' } },
  ...
  { re: /(스카|ska).*(예측|forecast).*(헬스|상태|정확도|건강|리포트|보고)|예측.*(헬스|정확도|건강).*(스카|ska)|예약팀.*예측.*(정확도|상태)/i, intent: 'ska_forecast_health', args: {} },
  { re: /(스카|ska).*(예측|forecast).*(리뷰|진단|튜닝|검토|점검)|예측.*(튜닝|리뷰|진단).*(스카|ska)|예약팀.*예측.*(튜닝|리뷰|진단)/i, intent: 'ska_forecast_review', args: {} },
