...
async function runSkaForecastHealthDirect() {
  ...
  return await new Promise((resolve) => {
    ...
    const timer = setTimeout(() => {
      child.kill('SIGTERM');
      resolve('⏱ 스카 예측 헬스 조회가 60초 내 끝나지 않았습니다. 잠시 후 다시 시도해 주세요.');
    }, 60_000);
    ...
    child.on('error', (err) => {
      clearTimeout(timer);
      resolve(`⚠️ 스카 예측 헬스 실행 실패: ${err.message}`);
    });
    child.on('close', (code) => {
      clearTimeout(timer);
      if (code !== 0) {
        const msg = stripAnsi(stderr || stdout).trim().split('\n').filter(Boolean).slice(-6).join('\n');
        resolve(`⚠️ 스카 예측 헬스 실패${msg ? `\n${msg}` : ''}`);
        return;
      }
      resolve(stripAnsi(stdout).trim() || 'ℹ️ 스카 예측 헬스 결과가 비어 있습니다.');
    });
  });
}
...
async function runReportingHealthDirect(query = '') {
  ...
  return await runNodeScriptText(script, {
    ...
    emptyText: 'ℹ️ reporting 헬스 결과가 비어 있습니다.',
  });
}
...
async function runSkaForecastReviewDirect() {
  ...
  return await new Promise((resolve) => {
    const child = spawn(python, [script, '--mode=review'], {
      cwd: root,
      env: { ...process.env, FORCE_COLOR: '0' },
      stdio: ['ignore', 'pipe', 'pipe'],
    });
    ...
    const timer = setTimeout(() => {
      child.kill('SIGTERM');
      resolve('⏱ 스카 예측 리뷰가 180초 내 끝나지 않았습니다. 잠시 후 다시 시도해 주세요.');
    }, 180_000);
    ...
    child.on('error', (err) => {
      clearTimeout(timer);
      resolve(`⚠️ 스카 예측 리뷰 실행 실패: ${err.message}`);
    });
    child.on('close', (code) => {
      clearTimeout(timer);
      if (code !== 0) {
        const msg = stripAnsi(stderr || stdout).trim().split('\n').filter(Boolean).slice(-10).join('\n');
        resolve(`⚠️ 스카 예측 리뷰 실패${msg ? `\n${msg}` : ''}`);
        return;
      }
...
function isPickkoAlertResolveCommand(text = '') {
  // DEPRECATED:
  // Hub topic 자연어 답장 해제가 주 경로가 되면 제거 대상이다.
...
async function runPickkoAlertsResolveDirect() {
  // DEPRECATED:
  // Hub thread context 기반 자연어 해제가 주 경로가 되면 제거 대상이다.
  const root = path.join(__dirname, '..', '..', '..');
...
async function handleIntent(parsed, msg, notify = async () => {}) {
  ...
  switch (intent) {
    ...
    case 'claude_health': {
      await notify('⏳ 클로드 운영 헬스 확인 중...');
      return await runClaudeHealthDirect();
    }
    ...
    case 'ska_forecast_health': {
      await notify('⏳ 스카 예측 헬스 확인 중...');
      return await runSkaForecastHealthDirect();
    }

    case 'ska_forecast_review': {
      await notify('⏳ 스카 예측 리뷰 실행 중...');
      return await runSkaForecastReviewDirect();
    }
...
async function route(msg, sendReply) {
  ...
  try {
    // DEPRECATED:
    // Hub topic 답장 해제가 기본 경로지만, orchestrator 직접 수신 하위 호환을 남긴다.
