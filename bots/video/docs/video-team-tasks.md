...
```
목표: bots/video/ 디렉토리 생성 + DB 테이블 + config
의존: 없음 (첫 번째 작업)
산출: 디렉토리 구조 + DB 테이블 + config + 기획 MD 파일 배치

작업 위치:
  생성: bots/video/ (전체 디렉토리 구조)
  생성: bots/video/docs/ (개발 중 MD 파일 — 안정화 후 docs/로 이동)
...
2. docs/ 폴더의 MD 파일 4개는 bots/video/docs/ 에서 내용을 읽어서 그대로 복사
