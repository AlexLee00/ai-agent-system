...
> 최종 업데이트: 2026-03-21
> 상태: Phase 2 진행 중 — AI 싱크 매칭 파이프라인 (과제 A~E) / Phase 1 전체 완료
...
scene-indexer = 원본 전체 OCR 장면 인덱싱
narration-analyzer = 나레이션 STT + 구간별 의도/키워드 분석
sync-matcher = 나레이션 구간 ↔ 원본 장면 자동 매칭
intro-outro-handler = 파일/프롬프트 하이브리드 인트로·아웃트로 처리
...
원본 + 나레이션 업로드
FFmpeg 전처리 (나레이션 정규화)
Whisper STT → SRT 생성 → 자막 교정
원본 장면 인덱싱 (OCR + scene-indexer)
나레이션 구간 분석 (narration-analyzer)
AI 싱크 매칭 (sync-matcher)
인트로/아웃트로 생성 (선택적)
