...
3가지 접근법 비교 후 결정:
  ...
  A. 제이 스택 (Claude + OpenClaw + Agent + n8n + RAG + FFmpeg + Whisper)
...
┌──────────────────────────────────────────────────────────┐
│ 오케스트레이션 계층                                       │
│  Claude (전략/프롬프트) + OpenClaw (에이전트 관리)         │
│  n8n (워크플로우 자동화)                                  │
└──────────────────────┬───────────────────────────────────┘
                       │
┌──────────────────────▼───────────────────────────────────┐
...
│    ├ MCP Server: OpenClaw/Claude와 직접 연동              │
...
출력: CapCut 드래프트 폴더 (dfd_*)
  ...
  ★ 아키텍처 변경 사항:
    ...
    수정: OpenClaw가 CapCutAPI(MCP)를 통해 CapCut 드래프트를 자동 생성
  ...
  처리 흐름:
    1. OpenClaw → CapCutAPI MCP 서버에 연결
       → MCP config:
         {
           "mcpServers": {
             "capcut-api": {
