...
const { generateGemmaPilotText } = require('../../../packages/core/lib/gemma-pilot.js');

function sanitizeVideoInsightLine(text = '') {
  return String(text || '')
    .split('\n')
    .map((line) => line.trim())
    .find((line) =>
...
async function buildVideoCliInsight({
  bot,
  requestType,
  title,
  data,
  ...
  try {
    const prompt = `당신은 비디오 운영 결과 분석가입니다.
...
${JSON.stringify(data || {}, null, 2).slice(0, 1800)}`;
    ...
    return sanitizeVideoInsightLine(insight?.content || '') || fallback;
  } catch (error) {
