defmodule Darwin.V2.LLM.HubClient do
  ...
  LLM_HUB_ROUTING_ENABLED=true 일 때만 실제 호출.
  ...
  @doc "LLM_HUB_ROUTING_ENABLED=true 여부"
  def enabled? do
    System.get_env("LLM_HUB_ROUTING_ENABLED") == "true"
  end

  @doc "LLM_HUB_ROUTING_SHADOW=true 여부"
  def shadow? do
    System.get_env("LLM_HUB_ROUTING_SHADOW") == "true"
  end
  ...
  request 필드:
    :prompt          — 문자열 (필수, messages → to_prompt 변환 후)
  ...
  def call(request) when is_map(request) do
    ...
    body = %{
      ...
      "abstractModel" => to_string(request[:abstract_model] || :anthropic_sonnet),
