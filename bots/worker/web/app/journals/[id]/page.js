...
export default function JournalDetailPage() {
  ...
  const load = () => {
    ...
    api.get(`/journals/${id}`)
      .then(d => {
        ...
        setForm({ content: d.journal.content, category: d.journal.category });
      })
  ...
  if (!journal) return (
    <div className="text-center py-20">
      ...
      <button className="btn-secondary" onClick={() => router.push('/journals')}>목록으로</button>
    </div>
  );

  const catLabel = CATEGORIES.find(c => c.value === journal.category)?.label || journal.category;

  return (
    <div className="space-y-4 max-w-2xl">
      {/* 헤더 */}
      <div className="flex items-center gap-3">
        <button onClick={() => router.push('/journals')} className="text-gray-500 hover:text-gray-700 text-sm">
          ← 목록
        </button>
