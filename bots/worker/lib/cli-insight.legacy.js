...
function sanitizeWorkerInsightLine(text = '') {
  return String(text || '')
    .split('\n')
    .map((line) => line.trim())
    .find((line) =>
...
async function buildWorkerCliInsight({
  bot,
  requestType,
  title,
  data,
  ...
  try {
    const prompt = `당신은 워커 운영 결과 분석가입니다.
...
${JSON.stringify(data || {}, null, 2).slice(0, 1800)}`;
    ...
    return sanitizeWorkerInsightLine(insight?.content || '') || fallback;
  } catch (error) {
