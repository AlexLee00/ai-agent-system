...
class FakeChild extends EventEmitter {
  constructor(output = '') {
    ...
    this.pid = 999999;
    this.killSignals = [];
    if (output) process.nextTick(() => this.stdout.emit('data', Buffer.from(output)));
  }
...
async function main() {
  ...
  const paymentChild = new FakeChild('PICKKO_PAYMENT_STAGE_ENTERED\nPICKKO_PAYMENT_SUBMIT_STARTED attempt=1\n');
  const paymentService = createNaverPickkoRunnerService({
    isCancelledKey: async () => false,
    getReservation: async () => ({ id: 'booking-payment-timeout', status: 'pending', pickkoStatus: null, retries: 0 }),
    markSeen: async () => {},
    resolveAlertsByBooking: async () => {},
    updateBookingState: async (bookingId, booking, state) => paymentStates.push({ bookingId, state }),
    updateReservation: async (bookingId, patch) => paymentPatches.push({ bookingId, patch }),
    addCancelledKey: async () => {},
    sendAlert: async () => {},
    ragSaveReservation: async () => {},
    publishReservationAlert: async () => {},
  ...
  const paymentCode = await paymentService.runPickko({
    booking: {
      phone: '01012345678',
      phoneRaw: '01012345678',
      date: '2099-01-01',
  ...
  assert.equal(paymentCode, 1);
  assert.ok(!paymentStates.some((item) => item.state === 'failed'), 'post-submit timeout must not retry registration');
  assert.ok(
    paymentPatches.some((item) => item.patch.pickkoStatus === 'manual_pending'),
    'post-submit timeout must enter the payment follow-up queue',
  );
