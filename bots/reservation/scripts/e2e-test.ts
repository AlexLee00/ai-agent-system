...
async function runTests() {
  ...
  await step('DB 연결', async () => {
    const ok = await pgPool.ping('reservation');
    if (!ok) throw new Error('DB 연결 실패');
  });
  ...
  await step('markSeen + isSeenId', async () => {
    if (await isSeenId(TEST_ID)) throw new Error('테스트 ID가 이미 존재');
    await markSeen(TEST_ID);
    if (!(await isSeenId(TEST_ID))) throw new Error('markSeen 후 isSeenId 실패');
  });

  await step('addReservation + getReservation', async () => {
    await addReservation(TEST_ID, {
      name: '테스트고객', phone: '01012345678', room: '스터디룸A',
      date: '2099-01-01', start: '10:00', end: '12:00',
      status: 'pending', source: 'e2e-test',
    });
    const r = await getReservation(TEST_ID);
    if (!r) throw new Error('getReservation 반환값 없음');
    if (r.name !== '테스트고객') throw new Error(`이름 불일치: ${r.name}`);
  });

  await step('addCancelledKey + isCancelledKey', async () => {
    const key = `cancel|e2e|test|${Date.now()}`;
    await addCancelledKey(key);
    if (!(await isCancelledKey(key))) throw new Error('cancelledKey 저장 실패');
  });
  ...
  await step('publishReservationAlert 전달 확인', async () => {
    const result = await publishReservationAlert({ from_bot: 'ska', event_type: 'health_check', alert_level: 1, message: 'E2E 테스트 메시지' });
  ...
  await step('적용된 마이그레이션 1개 이상 (12)', async () => {
    const rows = await getAppliedMigrations();
    if (!rows || rows.size === 0) throw new Error('마이그레이션 적용 기록 없음');
  });
