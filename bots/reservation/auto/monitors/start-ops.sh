...
log "  ⏱️  NAVER_MONITOR_DURATION_MS=${NAVER_MONITOR_DURATION_MS} (브라우저 세션 주기)"
...
while true; do
  ...
  NAVER_HEADLESS_VALUE=1
  PLAYWRIGHT_HEADLESS_VALUE=true
  log "  🫥 OPS 백그라운드 모드 강제 — headed 플래그 무시"

  MODE=ops PICKKO_ENABLE=1 STRICT_TIME=1 PICKKO_STRICT_REQUEST_WINDOW=1 \
  PLAYWRIGHT_HEADLESS="$PLAYWRIGHT_HEADLESS_VALUE" NAVER_HEADLESS="$NAVER_HEADLESS_VALUE" \
  NAVER_MONITOR_DURATION_MS="$NAVER_MONITOR_DURATION_MS" \
  ...
  PICKKO_CANCEL_ENABLE=${PICKKO_CANCEL_ENABLE:-0} \
  PICKKO_CANCEL_MUTATION_ENABLE=${PICKKO_CANCEL_MUTATION_ENABLE:-0} \
  SKA_ENABLE_PICKKO_CANCEL_MUTATION=${SKA_ENABLE_PICKKO_CANCEL_MUTATION:-0} \
  PICKKO_HEADLESS=1 \
