...
**모든 스크립트에서 따라야 하는 정규식 기반 검증 규칙**

---

## 🎯 원칙

1. **모든 데이터는 저장 전에 검증한다**
2. **검증에 실패한 데이터는 버린다** (NULL 처리)
3. **피크코 호출 전에 한 번 더 검증한다** (안전장치)
...
| **정규식** | `^\d{11}$` |
| **설명** | 숫자만 11자리 (하이픈 제거) |
| **예시** | `01035000586` ✅ / `010-3500-0586` ❌ |
| **오류 시** | 데이터 버림 (필수 필드) |
| **함수** | `validatePhoneNumber(phone)` |

```javascript
// ❌ 잘못된 예
"010-3500-0586"   // 하이픈 포함
"01035000586"     // 10자리
"010-35-00-0586"  // 형식 오류
