...
import { searchReservationCases } from '../../../packages/core/lib/reservation-rag';

interface PgPoolLike {
  query(schema: string, sql: string, params?: unknown[]): Promise<Record<string, unknown>[]>;
  get(schema: string, sql: string, params?: unknown[]): Promise<Record<string, unknown> | null>;
}

interface SkaReadServiceDeps {
  pgPool: PgPoolLike;
  rag?: any;
}

export function createSkaReadService({ pgPool, rag = null }: SkaReadServiceDeps) {
  async function queryReservations(args: { date?: string } = {}) {
  ...
  async function queryTodayStats(args: { date?: string } = {}) {
    const date = args.date || kst.today();
    try {
  ...
  async function queryAlerts(args: { limit?: number } = {}) {
    try {
      ...
      if (rag && rows.length > 0) {
        ...
        pastCases = await searchReservationCases(rag, String(first.type || '알람'), String(first.title || ''));
      }
