...
type CancelRetryFailureHook = (payload: {
  booking: Record<string, any>;
  cancelKey: string;
  output?: string;
...
export type CreateNaverPickkoRunnerServiceDeps = {
  ...
  log: Logger;
  onCancelRetrySuccess?: CancelRetrySuccessHook;
  onCancelRetryFailure?: CancelRetryFailureHook;
...
export function createNaverPickkoRunnerService(deps: CreateNaverPickkoRunnerServiceDeps) {
  const {
    ...
    toKst,
    log,
    onCancelRetrySuccess,
    onCancelRetryFailure,
    spawnImpl = nodeSpawn,
  ...
  function isPickkoCancelMutationEnabled() {
    return process.env.PICKKO_CANCEL_MUTATION_ENABLE === '1'
  ...
  function runPickkoCancel({
    ...
    return new Promise(async (resolve) => {
      ...
      const mutationEnabled = isPickkoCancelMutationEnabled();
      ...
      if (!mutationEnabled) {
        log(`🛡️ [픽코 취소 차단] PICKKO_CANCEL_MUTATION_ENABLE=1 및 SKA_ENABLE_PICKKO_CANCEL_MUTATION=1 필요 — 실제 취소 실행 안 함: ${maskPhone(phoneRawForKey)} ${booking.date} ${booking.start}~${booking.end} ${booking.room || ''}`);
        ...
        await Promise.resolve(sendAlert({
          ...
          room: booking.room,
          reason: 'PICKKO_CANCEL_MUTATION_ENABLE=1 및 SKA_ENABLE_PICKKO_CANCEL_MUTATION=1 필요',
      ...
      const onCancelSuccess = async (isRetry: boolean) => {
        ...
        if (onCancelRetrySuccess) {
          await Promise.resolve(onCancelRetrySuccess({
            booking,
            cancelKey: doneKey,
            resultCode: 0,
          })).catch((error: any) => {
            log(`⚠️ [취소 재시도 큐] 성공 hook 실패: ${error?.message || String(error)}`);
          });
        }
      };
