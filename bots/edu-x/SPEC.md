...
- BTC/USDT 커뮤니티/뉴스 이슈와 공개 시장 데이터 → 10 섹션 표준 게시글
- 암호화폐 심볼은 게시글/제목/표에서 `BASE/QUOTE` 형식으로 표시한다. 예: `BTC/USDT`, `ETH/USDT`
- 암호화폐 본문 우선순위는 1순위 BTC/USDT 정보, 2순위 암호화폐 커뮤니티 이슈, 3순위 루나팀 자동매매 정보로 고정한다.
- ①~⑩ 섹션 헤더에는 가시성용 이모지 아이콘을 붙이되, 번호는 줄 맨 앞에 유지해 HTML 변환과 품질 검증을 보존한다.
