...
| 실 발행 | 7일 검증 실행 35건+와 마감 슬롯 evidence(kis:1600/overseas:0630 각 5건+, fail 0건) + 마스터 승인 후만 허용. 검증 실행은 이미지 없는 non-fixture dry-run과 실제 API 성공(`[TEST]` one-off 포함)을 인정한다. |
...
4. 7일 검증 실행 35건+와 마감 슬롯 각 5건+ 검토 → 실 발행 승인. 이미지는 운영 정책상 첨부하지 않는다.
