...
- PROTECTED Hub 재시작 전에도 Edu-X 요청 단의 `policyOverride`로 `openai-oauth/gpt-5.4 → qwen/qwen3-32b → gpt-5.4-mini` 체인을 전달한다. `HUB_LLM_GEMINI_DISABLED` 운영과 맞춰 Gemini route는 기본 제외하고, 필요 시에만 `EDUX_FORMATTER_GEMINI_ENABLED=true`로 명시 활성화한다. 기본 호출 timeout은 OpenAI OAuth 장문 작성 관측치에 맞춰 180초이며, route별 timeout은 primary/fallback이 전체 timeout 안에서 종료되도록 분리한다. 필요 시 `EDUX_FORMATTER_POLICY_OVERRIDE=false`로 중앙 selector-only 운영으로 되돌릴 수 있다.
