...
function formatText(digest = {}) {
  const lines = [
    ...
    digest?.runtimeNarrative ? `운영 해설: ${digest.runtimeNarrative}` : null,
    '',
    '[Sense]',
    ...
    `- guarded publish: ${digest?.autonomySummary?.guardedPublishCount ?? 0}`,
    `- quality hold: ${digest?.autonomySummary?.holdCount ?? 0}`,
