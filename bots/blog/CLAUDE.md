...
- 현재 우선순위는 네이버 블로그 본문 품질, 댓글·답댓글·공감 루프, Edu-X 연계다. 마케팅과 SNS 게시 기능은 은퇴했다.
- 이웃댓글은 하루 30건이며 각 댓글에 공감을 함께 시도한다. 단독 공감은 하루 30건으로 분리해 총 공감 목표는 60건이다.
...
## 소셜·마케팅 상태
- 인스타그램/페이스북 자동 발행과 마케팅 실행기는 2026-07-23 은퇴했다.
- `bots/social-media` 코드는 삭제하지 않는다. MCP·이미지·숏폼·소셜 모듈은 차후 확장용으로 보존한다.
- 실제 `~/Library/LaunchAgents` bootout/delete는 마스터가 수행한다.
- 관련 env가 true여도 블로 런타임에서 SNS/마케팅 실행을 재활성화하지 않는다.
- `BLOG_IMAGE_GEN_ENABLED` 기본값은 false다.
- secrets-store의 기존 SNS 키는 삭제하거나 변경하지 않고 감사용으로 보존한다.
- 2026년 4월 Phase 1~7 자율 마케팅 로드맵은 공식 은퇴 상태다.
