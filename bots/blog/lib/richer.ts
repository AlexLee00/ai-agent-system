...
function extractMetricByLabel(text, label) {
  ...
  return _extractMetric(normalized, patterns);
}

async function fetchNaverBlogStatsFromStatPage(url) {
  const statUrl = buildPostStatUrl(url);
  ...
  try {
    ...
    await page.goto(statUrl, { waitUntil: 'networkidle2', timeout: 45000 });
    await new Promise((resolve) => setTimeout(resolve, 1200));
    const text = await page.evaluate(() => document.body ? document.body.innerText : '');
    if (!text || /로그인|아이디 또는 전화번호|비밀번호/.test(text)) return null;

    const stats = {
      views: extractMetricByLabel(text, '누적 조회수') || extractMetricByLabel(text, '조회수'),
      comments: extractMetricByLabel(text, '누적 댓글수') || extractMetricByLabel(text, '댓글수'),
      likes: extractMetricByLabel(text, '누적 공감수') || extractMetricByLabel(text, '공감수'),
      url,
      statUrl,
      source: 'naver_stat_page',
    };
    if (stats.views <= 0 && stats.comments <= 0 && stats.likes <= 0) {
      return null;
    }
    return stats;
  } catch (e) {
    console.warn('[리처] 네이버 통계 페이지 수집 실패:', e.message);
    return null;
  } finally {
...
async function fetchNaverBlogStats(post = {}) {
  ...
  const statPageStats = await fetchNaverBlogStatsFromStatPage(resolvedUrl);
  if (statPageStats) {
    return statPageStats;
  }
