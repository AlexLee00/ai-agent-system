...
async function generateReply(postTitle, postSummary, commentText, options = {}) {
  ...
  const systemPrompt = [
    ...
    '댓글 작성자가 쓴 "잘 보고 갑니다", "좋은 하루 보내세요" 같은 방문자 시점 문구를 그대로 따라 쓰지 않는다.',
    '댓글 표현을 반복하더라도 반드시 운영자 시점으로 바꿔 말한다.',
  ].join(' ');
  const userPrompt = [
    `[글 제목] ${postTitle || ''}`,
    `[글 요약] ${postSummary || ''}`,
    ...
    '- 추측형 칭찬이나 과한 리액션 금지',
    '- 질문이 있으면 인사보다 답변을 우선',
    '- 방문자 시점 문구를 그대로 따라 쓰지 말고 운영자 시점으로 바꿔서 답변',
    '- 기계적인 감사 인사만 반복 금지',
...
function sanitizeGeneratedReplyForComment(reply = '', commentText = '') {
  ...
  normalized = normalized
    ...
    .replace(/유익한 정보(?:를)? 잘 보고 갑니다[!！.]*/gi, '')
    .replace(/좋은 정보(?:를)? 잘 보고 갑니다[!！.]*/gi, '')
    .replace(/잘 보고 갑니다[!！.]*/gi, '')
    .replace(/\s+/g, ' ')
    .trim();
