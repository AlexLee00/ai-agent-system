...
function selectBonusInsights(botType: 'pos' | 'gems' | 'star', recentHistory: string[] = []) {
  ...
  const available = pool.filter((bonus) => !usedIds.has(bonus.id));
  ...
  return selected.map((bonus, index) => ({
    ...bonus,
    insertAfter: positions[index] || 'after_insight_3',
  }));
}
