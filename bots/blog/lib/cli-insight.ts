...
function loadGenerateGemmaPilotText() {
  if (cachedGenerateGemmaPilotText !== undefined) {
    return cachedGenerateGemmaPilotText;
  }

  try {
    const compiledPath = path.join(
      env.PROJECT_ROOT,
      'dist/ts-runtime/packages/core/lib/gemma-pilot.js',
    );
    const loaded = require(compiledPath);
    cachedGenerateGemmaPilotText =
      typeof loaded?.generateGemmaPilotText === 'function'
        ? loaded.generateGemmaPilotText
        : null;
  } catch (_error) {
    cachedGenerateGemmaPilotText = null;
  }
...
async function buildBlogCliInsight({
  bot,
  requestType,
  title,
  data,
