// @ts-nocheck
/**
 * Blog A2A Server — Google A2A Protocol (JSON-RPC 2.0 + SSE)
 * Port: 8770
 */
...
import { registerContentGenerationSkill } from './skills/content-generation.ts';
import { registerSeoAnalysisSkill } from './skills/seo-analysis.ts';
...
const agentCard = JSON.parse(
  await import('fs').then(m => m.readFileSync(new URL('./blog-card.json', import.meta.url), 'utf-8'))
);

const PORT = parseInt(process.env.BLOG_A2A_PORT || '8770', 10);
const app = express();
app.use(express.json());

registerContentGenerationSkill();
registerSeoAnalysisSkill();
...
app.listen(PORT, () => { console.log(`[Blog][A2A] 서버 가동 :${PORT}`); });
