defmodule Jay.V2.DailyBriefing do
  ...
  def generate(team_data, date) do
    sections =
      Enum.map(team_data, fn {team, data} ->
  ...
  defp format_team_section(team, data) do
    case data[:metric_type] do
      ...
      :agent_health ->
        "#{team_emoji(team)} *#{team}팀*\n" <>
          "  에이전트 #{data[:active_agents]}명 | 평균점수 #{format_score(data[:avg_score])} | 저성과 #{data[:low_score_agents]}명\n"
  ...
  defp format_score(score) when is_binary(score) do
    case Float.parse(score) do
      {parsed, _} -> format_score(parsed)
      :error -> "0.0"
    end
  end

  defp format_score(_score), do: "0.0"
