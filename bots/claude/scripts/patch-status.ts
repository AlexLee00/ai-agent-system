    ...
    for (const file of files) {
      ...
      try {
        ...
        if (ticket.patches?.length > 0) {
          ...
          for (const patch of ticket.patches) {
            const brk = patch.breaking ? ' ⚠️Breaking' : '';
            console.log(`    ${priorityEmoji(patch.priority)} ${patch.package}: ${patch.current} → ${patch.latest}${brk}`);
        ...
        if (ticket.security?.length > 0) {
          ...
          for (const sec of ticket.security) {
            console.log(`    ${severityEmoji(sec.severity)} [${sec.severity}] ${sec.package}: ${sec.summary}`);
          }
        }

        if (ticket.llm_api?.length > 0) {
          console.log(`  LLM API 변경 (${ticket.llm_api.length}건):`);
          for (const item of ticket.llm_api) {
            console.log(`    🤖 [${item.provider}] ${item.title}`);
          }
        }

        if (ticket.ai_techniques?.length > 0) {
          console.log(`  AI 기술 트렌드 (${ticket.ai_techniques.length}건):`);
          for (const item of ticket.ai_techniques) {
