...
const PROCESSED_DIR = path.join(AUTO_DEV_DIR, 'processed');
const ALARM_INCIDENT_PATTERN = /^ALARM_INCIDENT_.*\.md$/;
...
async function scanAndEnqueue(): Promise<{
  ...
  let files: string[];
  try {
    files = await fs.promises.readdir(AUTO_DEV_DIR);
  } catch {
    return result;
  }

  const targets = files.filter((f: string) => ALARM_INCIDENT_PATTERN.test(f));
  result.scanned = targets;

  for (const fileName of targets) {
    const srcPath = path.join(AUTO_DEV_DIR, fileName);
    const dstPath = path.join(PROCESSED_DIR, fileName);
