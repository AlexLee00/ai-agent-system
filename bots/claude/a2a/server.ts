// @ts-nocheck
/**
 * Claude Ops A2A Server — Google A2A Protocol (JSON-RPC 2.0 + SSE)
 * Port: 8771
 */
...
import { registerHealthCheckSkill } from './skills/health-check.ts';
import { registerErrorDiagnosisSkill } from './skills/error-diagnosis.ts';
...
const agentCard = JSON.parse(
  await import('fs').then(m => m.readFileSync(new URL('./claude-card.json', import.meta.url), 'utf-8'))
);

const PORT = parseInt(process.env.CLAUDE_A2A_PORT || '8771', 10);
const app = express();
app.use(express.json());

registerHealthCheckSkill();
registerErrorDiagnosisSkill();
...
app.listen(PORT, () => { console.log(`[Claude][A2A] 서버 가동 :${PORT}`); });
