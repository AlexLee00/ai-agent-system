 ...
 *      - 미바인딩 → error
 *   3. 프로세스 메모리 사용량
 *      - > MEM_WARN_MB(600MB) → warn (조기 감시)
 *      - > MEM_CRIT_MB(900MB) → _autoRestart (--fix 자동 재시작)
 *   4. 메모리 누수 추세 (6시간 윈도우, 150MB 이상 증가 → 자동 재시작)
 *   5. 프로세스 장기 운영 (3일+ → 주기적 재시작 트리거)
 *
 * 자동 재시작 조건 (--fix 모드에서 autofix.js가 처리):
 *   - MEM_CRIT_MB 초과
 *   - 6시간 내 LEAK_THRESHOLD_MB(150MB) 이상 증가 추세
 *   - UPTIME_RESTART_DAYS(3일) 이상 장기 운영
 *
...
// 메모리 임계값
const MEM_WARN_MB = 600;    // 경고 (기본 점유가 큰 편이라 600MB대부터 조기 감시)
const MEM_CRIT_MB = 900;    // 자동 재시작 (900MB 초과 → 1GB 고착 전 선제 정리)

// 누수 추세 감지
const LEAK_WINDOW_MS    = 6 * 60 * 60 * 1000; // 6시간 윈도우
const LEAK_THRESHOLD_MB = 150;                  // 6시간 내 150MB 이상 증가 → 누수
const MAX_HISTORY       = 72;                   // 최대 72개 (full 1h 기준 3일분)

// 장기 운영 재시작
const UPTIME_RESTART_DAYS = 3;
