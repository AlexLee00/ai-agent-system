...
export async function sendTask(agentId: string, task: Omit<A2ATask, 'id'>): Promise<A2ATaskResult> {
  ...
  const payload = {
    jsonrpc: '2.0',
    method: 'tasks/send',
    id: `ska-${Date.now()}-${Math.random().toString(36).slice(2, 7)}`,
...
export async function broadcast(
  ...
  const notif: A2ANotification = { ...notification, timestamp: new Date().toISOString(), source: 'ska' };
  await Promise.allSettled(
    targets.map(async (agentId) => {
      ...
      try {
        ...
        console.log(`[uska][A2A] broadcast to ${agentId} 실패: ${err}`);
      }
    })
  );
}
