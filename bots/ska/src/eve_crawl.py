...
타겟: bots/ska/db/ska.duckdb → exam_events 테이블
        ...
        for stype, type_name in QNET_TYPES.items():
            ...
            try:
                ...
                for row in rows:
                    ...
                    written_text  = tds[2].inner_text().strip()   # 필기시험
                    practical_text= tds[6].inner_text().strip()   # 실기시험
...
#
# 가천대학교 (성남 수정구): ?year=YYYY&month=N  URL 순회, 탭 구분 MM.DD 형식
# 단국대학교 죽전 (용인 수지구): 단일 URL, li 요소 YYYY.MM.DD ~ YYYY.MM.DD 형식
# 크롤링 실패 시 오류 출력 후 0건 반환 (다른 이벤트 수집에 영향 없음)

# 키워드 → (exam_type, score_weight_당일)
# '성적' 포함 줄은 성적 입력/확인 기간이므로 제외
UNIV_KEYWORD_MAP = [
    ('중간고사', 'univ_midterm', 4),
    ('기말고사', 'univ_final', 4),
...
UNIV_PREP_DAYS = 14   # 시험 D-14부터 D-1 prep
...
#
# 국가직·지방직 9급, 국가직 7급, 경찰·소방 공채 필기시험일 정적 등록.
# 공식 시험일 확정 시 아래 CIVIL_EXAM_SCHEDULE 날짜를 업데이트할 것.
# (매년 1~2월 사이에 각 기관이 시행계획 공고를 통해 확정 날짜 발표)
#
# 점수 단계:
#   D-30 ~ D-22: civil_exam_prep_far  score=2
#   D-21 ~ D-8:  civil_exam_prep_mid  score=3
#   D-7  ~ D-1:  civil_exam_prep_near score=5
#   당일:         civil_exam           score=score_weight

CIVIL_EXAM_SCHEDULE = {
    2025: [
        # (날짜_str, exam_name, score_weight_당일)
    ...
    2026: [
        ...
        ('2026-04-11', '2026년 국가직 9급 공채 필기', 5),   # 4월 둘째 토
        ('2026-06-20', '2026년 지방직 9급 공채 필기', 5),   # 6월 셋째 토 (예상)
        ('2026-08-29', '2026년 국가직 7급 공채 필기', 4),   # 8월 말 토 (예상)
        ('2026-05-16', '2026년 경찰공채 1차 필기', 3),      # 5월 셋째 토 (예상)
        ('2026-04-25', '2026년 소방공채 1차 필기', 3),      # 4월 말 토 (예상)
    ],
}

CIVIL_PREP_BANDS = [
    # (days_from, days_to, score, exam_type)
...
def upsert_events(con, events, source='calc'):
    """exam_events 테이블에 UPSERT (PRIMARY KEY: date + exam_type + exam_name)"""
    inserted = 0
...
def show_upcoming(con, days=14):
    ...
    rows = con.execute(f"""
        ...
        FROM exam_events
        WHERE date >= current_date AND date <= current_date + INTERVAL {int(days)} DAY
...
def run_crawl(year=None, skip=None):
    ...
    # ── 집계 ────────────────────────────────────────────────────────────────────
    row_count  = con.execute("SELECT COUNT(*) FROM exam_events").fetchone()[0]
    date_count = con.execute("SELECT COUNT(DISTINCT date) FROM exam_events").fetchone()[0]
    src_counts = con.execute(
        "SELECT source, COUNT(*) FROM exam_events GROUP BY source ORDER BY source"
    ).fetchall()
