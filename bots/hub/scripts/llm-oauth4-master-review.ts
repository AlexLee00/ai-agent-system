...
function buildReport(stats, options = {}) {
  ...
  if (byProvider['claude-code-oauth'].share_pct < 45 && totalCalls > 0) {
    warnings.push('runtime_claude_code_share_depends_on_traffic_mix_selector_share_used_for_pass_fail');
  }
  ...
  const review = {
    ...
    targets: {
      ...
      selector_claude_code_primary_min_pct: 45,
      anthropic_provider_calls_target: 0,
      max_failed_rate_pct: maxFailedRatePct,
    },
    verdict: {
      selector_claude_code_share_ok: selectorSnapshot.claude_code_primary_share_pct >= 45,
