...
function main() {
  ...
  if (target === 'unit') {
    ...
    const activeRuntimeIsolationStatus = run(tsxBin, [path.join(scriptDir, 'active-runtime-openclaw-isolation-smoke.ts')]);
    if (activeRuntimeIsolationStatus !== 0) process.exit(activeRuntimeIsolationStatus);
    const legacyOpenClawAdminGuardStatus = run(tsxBin, [path.join(scriptDir, 'legacy-openclaw-admin-guard-smoke.ts')]);
    if (legacyOpenClawAdminGuardStatus !== 0) process.exit(legacyOpenClawAdminGuardStatus);
    const claudeRuntimeIsolationStatus = run(tsxBin, [path.join(scriptDir, 'claude-runtime-openclaw-isolation-smoke.ts')]);
    if (claudeRuntimeIsolationStatus !== 0) process.exit(claudeRuntimeIsolationStatus);
    const investmentRuntimeIsolationStatus = run(tsxBin, [path.join(scriptDir, 'investment-runtime-openclaw-isolation-smoke.ts')]);
    if (investmentRuntimeIsolationStatus !== 0) process.exit(investmentRuntimeIsolationStatus);
    const reservationRuntimeIsolationStatus = run(tsxBin, [path.join(scriptDir, 'reservation-runtime-openclaw-isolation-smoke.ts')]);
