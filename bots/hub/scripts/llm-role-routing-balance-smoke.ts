...
for (const target of targets) {
  ...
  if (chain.some((entry: any) => providerOf(entry) === 'claude-code') && !claudeFirstWritingSelectors.has(target.selectorKey)) {
...
assert.equal(unexpectedClaudeCodeRoutes.length, 0, `Claude Code must stay limited to approved writing selectors: ${unexpectedClaudeCodeRoutes.join(', ')}`);
