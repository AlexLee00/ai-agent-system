...
async function main() {
  const result = await pgPool.query(`DELETE FROM llm_cache WHERE expires_at < NOW() RETURNING id`);
  const deleted = result.rowCount || 0;
  console.log(`[llm-cache] 만료 캐시 ${deleted}건 삭제`);

  try {
    await pgPool.query('REFRESH MATERIALIZED VIEW CONCURRENTLY llm_cache_stats');
