...
async function main() {
  ...
  pgPool.query = async (_schema: string, sql: string) => {
    queryLog.push(sql);
  ...
  try {
    ...
    const noise = await buildAlarmNoiseReport({ minutes: 60, limit: 5 });
    ...
    const stale = await scanStaleAutoRepair({ staleMinutes: 60, limit: 5 });
