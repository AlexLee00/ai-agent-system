...
function getFirstHeader(req: Request, names: string[]): unknown {
  for (const name of names) {
    const value = getHeader(req, name);
    if (normalizeString(value, 512)) return value;
  }
  return null;
}

function resolveTraceId(req: HubRequest): string {
  const raw = normalizeString(getFirstHeader(req, ['x-trace-id', 'x-hub-trace-id']), 128);
  if (raw) return raw;
...
function resolvePriority(req: HubRequest): string {
  const fromHeader = normalizeString(getFirstHeader(req, ['x-priority', 'x-hub-priority']), 16);
  const fromBody = normalizeString(req.body?.priority, 16) || normalizeString(req.body?.urgency, 16);
...
function resolveCallerTeam(req: HubRequest): string | null {
  return normalizeString(req.body?.callerTeam, 120) || normalizeString(getFirstHeader(req, ['x-caller-team', 'x-hub-team']), 120);
}

function resolveAgent(req: HubRequest): string | null {
  return normalizeString(req.body?.agent, 120) || normalizeString(getFirstHeader(req, ['x-agent', 'x-hub-agent']), 120);
}
