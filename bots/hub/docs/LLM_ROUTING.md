...
Gemini provider는 영구 폐기 상태다. `HUB_LLM_GEMINI_DISABLED=true`는 배포 상태를 표시하는 선언값(`declaration_only`)이며, false 또는 allow 계열 환경변수로 재활성화할 수 없다. 재도입은 공통 폐기 정책과 회귀 계약을 함께 변경하는 검토된 코드 변경만 허용한다(`code_change_only`).
...
- timeout 또는 abort 시 실행 중인 Claude/로컬 자식 프로세스 그룹을 종료한다. Gemini 자식 프로세스는 폐기 경계에서 시작 전에 차단된다.
