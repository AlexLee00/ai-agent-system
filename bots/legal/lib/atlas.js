...
const atlasClient = require(path.join(env.PROJECT_ROOT, 'bots/legal/lib/atlas-client'));
...
async function searchForeignCases(caseId, caseData) {
  ...
  const externalContext = await collectForeignCaseContext(caseData, resolvedType);
...
async function collectForeignCaseContext(caseData, resolvedType) {
  const keywords = buildSearchKeywords(caseData, resolvedType);
  const sections = [];

  for (const keyword of keywords) {
    try {
      const result = await atlasClient.searchUsOpinions(keyword, { limit: 3, timeoutMs: 12000 });
      if (result.items.length) {
        sections.push([
          `[CourtListener 검색어] ${keyword}`,
          ...result.items.map((item) =>
            `- ${item.caseName} | ${item.citation || item.docketNumber || 'citation 없음'} | ${item.court} | ${item.dateFiled} | ${item.absoluteUrl}`),
        ].join('\n'));
      }
    } catch (error) {
      sections.push(`[CourtListener 검색어] ${keyword}\n- 검색 실패: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
...
function buildSearchKeywords(caseData, resolvedType) {
  ...
  const fallbackByType = {
    copyright: ['software copyright infringement', 'source code copying software'],
    defect: ['software defect breach contract', 'software implementation defect'],
    contract: ['software development contract dispute', 'software services breach'],
    trade_secret: ['source code trade secret misappropriation', 'software confidential information'],
    other: ['software litigation'],
  };
...
function parseForeignPrecedents(text) {
  ...
  for (const section of sections.slice(1)) {
    const nameMatch = section.match(/\*\*사건명\*\*[:\s]+([^\n]+)/) || section.match(/사건명[:\s]+([^\n]+)/);
    const numberMatch = section.match(/\*\*사건번호\*\*[:\s]+([^\n]+)/) || section.match(/사건번호[:\s]+([^\n]+)/);
    ...
    if (idMatch) {
      const caseName = cleanField(nameMatch?.[1] || '');
      const caseNumber = cleanField(numberMatch?.[1] || idMatch[1] || '');
      precedents.push({
        case_number: caseName && caseNumber ? `${caseName} (${caseNumber})` : caseNumber,
        court: [cleanField(jurisdictionMatch?.[1] || ''), cleanField(courtMatch?.[1] || '')].filter(Boolean).join(' / ') || '미상',
        decision_date: cleanField(dateMatch?.[1] || '') || null,
