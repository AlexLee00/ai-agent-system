// @ts-nocheck
/**
 * Sigma A2A Server — Google A2A Protocol (JSON-RPC 2.0 + SSE)
 * Port: 8767
 */
...
import { registerMetaAnalysisSkill } from './skills/meta-analysis.ts';
import { registerConsistencyCheckSkill } from './skills/consistency-check.ts';
...
const agentCard = JSON.parse(
  await import('fs').then(m => m.readFileSync(new URL('./sigma-card.json', import.meta.url), 'utf-8'))
);

const PORT = parseInt(process.env.SIGMA_A2A_PORT || '8767', 10);
const app = express();
app.use(express.json());

registerMetaAnalysisSkill();
registerConsistencyCheckSkill();
...
app.listen(PORT, () => { console.log(`[Sigma][A2A] 서버 가동 :${PORT}`); });
