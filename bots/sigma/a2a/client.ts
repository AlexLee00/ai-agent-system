...
export async function sendTask(agentId: string, task: Omit<A2ATask, 'id'>): Promise<A2ATaskResult> {
  ...
  const payload = {
    jsonrpc: '2.0',
    method: 'tasks/send',
    id: `sigma-${Date.now()}-${Math.random().toString(36).slice(2, 7)}`,
...
export async function broadcast(
  ...
  const notif: A2ANotification = { ...notification, timestamp: new Date().toISOString(), source: 'sigma' };
  await Promise.allSettled(
    targets.map(async (agentId) => {
      ...
      try {
        ...
        console.log(`[usigma][A2A] broadcast to ${agentId} 실패: ${err}`);
      }
    })
  );
}
