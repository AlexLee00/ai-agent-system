...
function titleForRecord(record: any): string {
  const firstText = String(record.piiRedactedText || record.text || '')
    ...
    .trim()
    .slice(0, 96);
  return `[${record.sourceKind}] ${firstText || record.sourceId}`;
}

function tagsForRecord(record: any): string[] {
  const tags = new Set(['sigma-library', 'luna', String(record.sourceKind || 'unknown')]);
  const payload = record.payload || {};
  if (payload.market) tags.add(String(payload.market));
  if (payload.status) tags.add(`status:${payload.status}`);
  if (payload.strategyFamily) tags.add(`strategy:${payload.strategyFamily}`);
...
export async function runSigmaLunaVaultFeed(options: {
  sinceHours?: number;
...
async function main() {
  ...
  const result = await runSigmaLunaVaultFeed({
  ...
  if (json) console.log(JSON.stringify(result, null, 2));
    ...
    console.log(`[sigma-luna-vault-feed] dryRun=${result.dryRun} candidates=${result.candidates} persisted=${result.persisted.ok}/${result.persisted.attempted} embedded=${result.persisted.embedded}`);
  }
