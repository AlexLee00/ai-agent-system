...
async function collectWeeklyMetrics() {
  const [
    ...
    countRows('public', 'sigma_v2_directive_audit', `WHERE tier = 2 AND outcome = 'tier2_applied' AND executed_at >= NOW() - INTERVAL '7 days'`),
    countRows('public', 'sigma_mcp_usage_audit', `WHERE request_at >= NOW() - INTERVAL '7 days'`),
    countRows('public', 'sigma_mcp_usage_audit', `WHERE endpoint = 'tools/call' AND request_at >= NOW() - INTERVAL '7 days'`),
...
function buildWeeklyMessage(input: {
  ...
  return [
    ...
    `- directives: total=${input.metrics.directives7d.value}, tier2_applied=${input.metrics.tier2Directives7d.value}`,
    `- MCP calls: total=${input.metrics.mcpCalls7d.value}, tool_calls=${input.metrics.mcpToolCalls7d.value}`,
