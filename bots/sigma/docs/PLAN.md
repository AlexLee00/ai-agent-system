...
│   ├ scheduler.ex                              138          │
│   └ feedback.ex                               163          │
│ bots/sigma/legacy-skills/   │    5   │   308        │  ← 죽은 코드
...
**문제**: `bots/sigma/legacy-skills/*.ts`가 존재하지만 `grep -r 'evaluateDataset\|evaluateCausalRisk\|evaluateExperimentDesign\|planFeatures\|buildObservabilityPlan' bots/orchestrator/` 결과 0건.
...
**Skills**: `bots/sigma/legacy-skills/*.ts` 5개, **Node.js 내부 함수**로만 사용 가능. 다른 에이전트(claude.ai 대화, Codex, 외부 LLM)에서 호출 불가.
...
│   bots/sigma/legacy-skills/*.ts → archive             │
...
**현재**: 에디 비디오 n8n 워크플로우는 retired archive로 이동했다. 시그마팀은 n8n 미사용.
...
**결정**: 시그마팀은 **n8n 도입 안 함**. retired 에디 n8n 워크플로우는 현역 런타임으로 재도입하지 않는다.
...
│   `bots/sigma/legacy-skills/` 각 스킬은                   │
...
2. `bots/sigma/elixir/lib/sigma/v2/` 새 디렉토리 생성 (v2 네임스페이스)
...
**5.1 TS 시그마 폐기**
- `bots/sigma/ts/lib/*.ts` → `docs/archive/sigma-legacy/` 이관
- `bots/sigma/ts/src/sigma-daily.ts` → thin adapter로 축소 (Jido v2 호출)
