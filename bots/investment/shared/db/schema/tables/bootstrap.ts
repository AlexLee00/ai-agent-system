...
export async function runInvestmentSchemaBootstrap(run, { log = true } = {}) {
  ...
  await run(`
    CREATE TABLE IF NOT EXISTS luna_vault_shadow_on_candidates (
      id              BIGSERIAL PRIMARY KEY,
  ...
  `);
  try { await run(`CREATE INDEX IF NOT EXISTS idx_luna_vault_shadow_on_candidates_scope ON luna_vault_shadow_on_candidates(scope_market, scope_family, scope_direction)`); } catch { /* 무시 */ }
  try { await run(`CREATE INDEX IF NOT EXISTS idx_luna_vault_shadow_on_candidates_status ON luna_vault_shadow_on_candidates(gate_status)`); } catch { /* 무시 */ }
