...
export const LUNA_L5_TRANSITION_HISTORY_FILE = 'luna-l5-transition-history.jsonl';
...
export function getLunaL5TransitionHistoryPath() {
  return investmentOpsRuntimeFile(LUNA_L5_TRANSITION_HISTORY_FILE);
}

export function appendLunaL5TransitionHistory(event = {}) {
  const file = getLunaL5TransitionHistoryPath();
...
export function readRecentLunaL5TransitionHistory({ limit = 20 } = {}) {
  const file = getLunaL5TransitionHistoryPath();
  let lines = [];
  try {
