...
function buildSectionStates({
  ...
  const collect = {
    ...
    strategyFeedbackOutcomes: strategyFeedbackOutcomes ? {
      ...
      trend: strategyFeedbackOutcomeTrend ? {
        historyCount: Number(strategyFeedbackOutcomeTrend.historyCount || 0),
        previousStatus: strategyFeedbackOutcomeTrend.previous?.status || null,
        delta: strategyFeedbackOutcomeTrend.delta || {},
        previousWeakest: strategyFeedbackOutcomeTrend.previous?.weakest || null,
      } : null,
    } : null,
    ...
    riskApprovalReadiness: riskApprovalReadiness ? {
      ...
      trend: riskApprovalReadinessTrend ? {
        historyCount: Number(riskApprovalReadinessTrend.historyCount || 0),
    ...
    riskApprovalModeAudit: riskApprovalModeAudit ? {
      ...
      trend: riskApprovalModeAuditTrend ? {
        historyCount: Number(riskApprovalModeAuditTrend.historyCount || 0),
        previousStatus: riskApprovalModeAuditTrend.previous?.status || null,
        delta: riskApprovalModeAuditTrend.delta || {},
      } : null,
    } : null,
    ...
    openJournalReconcile: openJournalReconcile ? {
      ...
      writeCommand: 'npm --prefix /Users/alexlee/projects/ai-agent-system/bots/investment run runtime:reconcile-open-journals -- --write --confirm-live --symbols=SYMBOL/USDT --max-affected-trades=10',
    } : null,
  };
...
export async function buildRuntimeLearningLoopReport({ days = 14, json = false } = {}) {
  const [freshness, runtimeDecision, executionGate, autotune, runtimeSuggestions, backtest, validation, regimeCoverage, regimePerformance, strategyFamilyPerformance, strategyFeedbackOutcomes, strategyFeedbackOutcomeTrend, riskApproval, riskApprovalTrend, riskApprovalReadiness, riskApprovalReadinessTrend, riskApprovalModeAudit, riskApprovalModeAuditTrend, executionRiskGuard, executionRiskGuardTrend, collectionAudit, openJournalReconcile] = await Promise.all([
    ...
    buildRuntimeRiskApprovalModeAuditHistory({ days: 30, json: true, write: false }).catch(() => null),
