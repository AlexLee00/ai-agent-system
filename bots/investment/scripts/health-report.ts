      ...
      lines: report.positionStrategyAudit
        ? [
            ...
            ...(report.positionStrategyRemediationHistory
              ? [
                `  remediation history: count ${report.positionStrategyRemediationHistory.historyCount || 0} / changed ${report.positionStrategyRemediationHistory.statusChanged ? 'yes' : 'no'} / next changed ${report.positionStrategyRemediationHistory.nextCommandChanged ? 'yes' : 'no'}${report.positionStrategyRemediationHistory.nextCommandChanged ? ` (${report.positionStrategyRemediationHistory.nextCommandTransition?.previous || 'none'} -> ${report.positionStrategyRemediationHistory.nextCommandTransition?.current || 'none'})` : ''} / age ${report.positionStrategyRemediationHistory.ageMinutes ?? 'n/a'}m / stale ${report.positionStrategyRemediationHistory.stale ? 'yes' : 'no'}`,
