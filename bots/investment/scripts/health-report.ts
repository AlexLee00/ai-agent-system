  ...
  return buildHealthDecision({
    warnings: [
      ...
      },
      {
        ...
        reason: `position strategy remediation — ${positionStrategyRemediation?.remediationSummary?.headline || positionStrategyRemediation?.decision?.headline || positionStrategyHygiene?.decision?.headline || '포지션 전략 위생 점검 필요'} / duplicate managed ${(positionStrategyRemediation?.remediationSummary?.counts?.duplicateManaged ?? positionStrategyAudit?.duplicateManagedProfileScopes ?? 0)} / orphan ${(positionStrategyRemediation?.remediationSummary?.counts?.orphanProfiles ?? positionStrategyOrphans ?? 0)} / unmatched managed ${(positionStrategyRemediation?.remediationSummary?.counts?.unmatchedManaged ?? positionStrategyAudit?.unmatchedManagedPositions ?? 0)}${positionStrategyRemediationHistory ? ` / history changed ${positionStrategyRemediationHistory.statusChanged ? 'yes' : 'no'} / next changed ${positionStrategyRemediationHistory.nextCommandChanged ? 'yes' : 'no'}${positionStrategyRemediationHistory.nextCommandChanged ? ` (${positionStrategyRemediationHistory.nextCommandTransition?.previous || 'none'} -> ${positionStrategyRemediationHistory.nextCommandTransition?.current || 'none'})` : ''} / history age ${positionStrategyRemediationHistory.ageMinutes ?? 'n/a'}m / history stale ${positionStrategyRemediationHistory.stale ? 'yes' : 'no'} / duplicate delta ${positionStrategyRemediationHistory.delta?.duplicateManaged >= 0 ? '+' : ''}${positionStrategyRemediationHistory.delta?.duplicateManaged || 0} / orphan delta ${positionStrategyRemediationHistory.delta?.orphanProfiles >= 0 ? '+' : ''}${positionStrategyRemediationHistory.delta?.orphanProfiles || 0}` : ''}${positionStrategyRemediation?.remediationSummary?.refreshState?.reason ? ` / ${positionStrategyRemediation.remediationSummary.refreshState.reason}` : (positionStrategyRemediation?.remediationRefreshState?.reason ? ` / ${positionStrategyRemediation.remediationRefreshState.reason}` : '')} / duplicate apply ${duplicateStrategyNormalization?.decision?.safeToApply === true ? 'yes' : 'no'} / orphan apply ${orphanStrategyRetirement?.decision?.safeToApply === true ? 'yes' : 'no'} / next command ${positionStrategyRemediation?.remediationSummary?.nextCommand || positionStrategyRemediation?.remediationActions?.nextCommand || 'npm --prefix /Users/alexlee/projects/ai-agent-system/bots/investment run runtime:position-strategy-remediation -- --json'}`,
      },
    ],
      ...
      lines: report.positionStrategyAudit
        ? [
            ...
            ...(report.positionStrategyRemediationHistory
              ? [
                `  remediation history: count ${(report.positionStrategyRemediationSummary?.trend?.historyCount ?? report.positionStrategyRemediationHistory.historyCount ?? 0)} / changed ${report.positionStrategyRemediationSummary?.trend?.statusChanged ? 'yes' : 'no'} / next changed ${report.positionStrategyRemediationSummary?.trend?.nextCommandChanged ? 'yes' : 'no'}${report.positionStrategyRemediationSummary?.trend?.nextCommandChanged ? ` (${report.positionStrategyRemediationSummary?.trend?.nextCommandTransition?.previous || 'none'} -> ${report.positionStrategyRemediationSummary?.trend?.nextCommandTransition?.current || 'none'})` : ''} / age ${(report.positionStrategyRemediationSummary?.trend?.ageMinutes ?? report.positionStrategyRemediationHistory.ageMinutes ?? 'n/a')}m / stale ${report.positionStrategyRemediationSummary?.trend?.stale ? 'yes' : 'no'}`,
                `  remediation delta: duplicate ${((report.positionStrategyRemediationSummary?.trend?.duplicateDelta ?? report.positionStrategyRemediationHistory.delta?.duplicateManaged ?? 0) >= 0) ? '+' : ''}${(report.positionStrategyRemediationSummary?.trend?.duplicateDelta ?? report.positionStrategyRemediationHistory.delta?.duplicateManaged ?? 0)} / orphan ${((report.positionStrategyRemediationSummary?.trend?.orphanDelta ?? report.positionStrategyRemediationHistory.delta?.orphanProfiles ?? 0) >= 0) ? '+' : ''}${(report.positionStrategyRemediationSummary?.trend?.orphanDelta ?? report.positionStrategyRemediationHistory.delta?.orphanProfiles ?? 0)} / unmatched ${((report.positionStrategyRemediationSummary?.trend?.unmatchedDelta ?? report.positionStrategyRemediationHistory.delta?.unmatchedManaged ?? 0) >= 0) ? '+' : ''}${(report.positionStrategyRemediationSummary?.trend?.unmatchedDelta ?? report.positionStrategyRemediationHistory.delta?.unmatchedManaged ?? 0)}`,
              ]
              : []),
            ...
            `  remediation report: ${report.positionStrategyRemediationSummary?.commands?.report || report.positionStrategyRemediationActions?.reportCommand || report.positionStrategyHygiene?.remediationPlan?.remediationReportCommand || 'npm --prefix /Users/alexlee/projects/ai-agent-system/bots/investment run runtime:position-strategy-remediation -- --json'}`,
            `  remediation history: ${report.positionStrategyRemediationSummary?.commands?.history || report.positionStrategyRemediationActions?.historyCommand || report.positionStrategyHygiene?.remediationPlan?.remediationHistoryCommand || 'npm --prefix /Users/alexlee/projects/ai-agent-system/bots/investment run runtime:position-strategy-remediation-history -- --json'}`,
            `  remediation refresh: ${report.positionStrategyRemediationSummary?.commands?.refresh || report.positionStrategyRemediationActions?.refreshCommand || report.positionStrategyHygiene?.remediationPlan?.remediationRefreshCommand || 'npm --prefix /Users/alexlee/projects/ai-agent-system/bots/investment run runtime:position-strategy-remediation-refresh -- --if-stale --json'}`,
            `  hygiene report: ${report.positionStrategyRemediationSummary?.commands?.hygiene || report.positionStrategyRemediationActions?.hygieneCommand || report.positionStrategyHygiene?.remediationPlan?.hygieneReportCommand || 'npm --prefix /Users/alexlee/projects/ai-agent-system/bots/investment run runtime:position-strategy-hygiene -- --json'}`,
            `  normalize dry-run: ${report.positionStrategyRemediationSummary?.commands?.normalizeDryRun || report.positionStrategyRemediationActions?.normalizeDryRunCommand || report.positionStrategyHygiene?.remediationPlan?.normalizeDryRunCommand || 'npm --prefix /Users/alexlee/projects/ai-agent-system/bots/investment run runtime:normalize-duplicate-strategy-profiles -- --json'}`,
            `  normalize apply: ${report.positionStrategyRemediationSummary?.commands?.normalizeApply || report.positionStrategyRemediationActions?.normalizeApplyCommand || report.positionStrategyHygiene?.remediationPlan?.normalizeApplyCommand || 'npm --prefix /Users/alexlee/projects/ai-agent-system/bots/investment run runtime:normalize-duplicate-strategy-profiles -- --apply --json'}`,
            `  retire orphan dry-run: ${report.positionStrategyRemediationSummary?.commands?.retireDryRun || report.positionStrategyRemediationActions?.retireDryRunCommand || report.positionStrategyHygiene?.remediationPlan?.retireDryRunCommand || 'npm --prefix /Users/alexlee/projects/ai-agent-system/bots/investment run runtime:retire-orphan-strategy-profiles -- --json'}`,
            `  retire orphan apply: ${report.positionStrategyRemediationSummary?.commands?.retireApply || report.positionStrategyRemediationActions?.retireApplyCommand || report.positionStrategyHygiene?.remediationPlan?.retireApplyCommand || 'npm --prefix /Users/alexlee/projects/ai-agent-system/bots/investment run runtime:retire-orphan-strategy-profiles -- --apply --json'}`,
          ]
...
async function buildReport() {
  ...
  const report = {
    ...
    positionStrategyRemediationStatus: positionStrategyRemediation?.remediationStatus || positionStrategyRemediation?.remediationSummary?.status || null,
    positionStrategyRemediationHeadline: positionStrategyRemediation?.remediationHeadline || positionStrategyRemediation?.remediationSummary?.headline || null,
    positionStrategyRemediationCounts: positionStrategyRemediation?.remediationCounts || positionStrategyRemediation?.remediationSummary?.counts || null,
    positionStrategyRemediationRecommendedExchange: positionStrategyRemediation?.remediationRecommendedExchange || positionStrategyRemediation?.remediationSummary?.recommendedExchange || null,
    positionStrategyRemediationDuplicateManaged: positionStrategyRemediation?.remediationDuplicateManaged ?? positionStrategyRemediation?.remediationSummary?.counts?.duplicateManaged ?? null,
    positionStrategyRemediationOrphanProfiles: positionStrategyRemediation?.remediationOrphanProfiles ?? positionStrategyRemediation?.remediationSummary?.counts?.orphanProfiles ?? null,
    positionStrategyRemediationUnmatchedManaged: positionStrategyRemediation?.remediationUnmatchedManaged ?? positionStrategyRemediation?.remediationSummary?.counts?.unmatchedManaged ?? null,
    positionStrategyRemediationHistory,
    ...
    positionStrategyRemediationActionReportCommand: positionStrategyRemediation?.remediationActionReportCommand || null,
    positionStrategyRemediationActionHistoryCommand: positionStrategyRemediation?.remediationActionHistoryCommand || null,
    positionStrategyRemediationActionRefreshCommand: positionStrategyRemediation?.remediationActionRefreshCommand || null,
    positionStrategyRemediationActionHygieneCommand: positionStrategyRemediation?.remediationActionHygieneCommand || null,
    positionStrategyRemediationActionNormalizeDryRunCommand: positionStrategyRemediation?.remediationActionNormalizeDryRunCommand || null,
    positionStrategyRemediationActionNormalizeApplyCommand: positionStrategyRemediation?.remediationActionNormalizeApplyCommand || null,
    positionStrategyRemediationActionRetireDryRunCommand: positionStrategyRemediation?.remediationActionRetireDryRunCommand || null,
    positionStrategyRemediationActionRetireApplyCommand: positionStrategyRemediation?.remediationActionRetireApplyCommand || null,
    ...
    positionStrategyRemediationNextCommand: positionStrategyRemediation?.remediationNextCommand || positionStrategyRemediation?.remediationSummary?.nextCommand || null,
