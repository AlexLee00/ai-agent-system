  ...
  return buildHealthDecision({
    warnings: [
      ...
      },
      {
        ...
        })()} / duplicate apply ${duplicateStrategyNormalization?.decision?.safeToApply === true ? 'yes' : 'no'} / orphan apply ${orphanStrategyRetirement?.decision?.safeToApply === true ? 'yes' : 'no'} / next command npm --prefix /Users/alexlee/projects/ai-agent-system/bots/investment run runtime:position-strategy-remediation -- --json`,
      },
    ],
        ...
        ? [
            ...
            ...(report.positionStrategyRemediation
              ? [
                ...
                ...(report.positionStrategyRemediation.decision?.actionItems || [])
                  .filter((item) => typeof item === 'string' && item.startsWith('history refresh'))
                  .map((item) => `  remediation refresh: ${item}`),
              ]
              : []),
