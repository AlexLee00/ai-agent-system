  ...
  return buildHealthDecision({
    warnings: [
      ...
      },
      {
        ...
        level: 'medium',
        reason: `strategy feedback outcomes — ${strategyFeedbackOutcomes?.headline || '전략 피드백 적용 결과 점검 필요'} / tagged ${strategyFeedbackOutcomes?.total || strategyFeedbackOutcomes?.totalTagged || 0} / closed ${strategyFeedbackOutcomes?.closed || strategyFeedbackOutcomes?.closedTagged || 0} / pnl ${strategyFeedbackOutcomes?.pnlSummary || (strategyFeedbackOutcomes?.pnlNet ?? 0)} / trend tagged Δ${strategyFeedbackOutcomes?.trend?.delta?.total ?? 0} closed Δ${strategyFeedbackOutcomes?.trend?.delta?.closed ?? 0} / pnl Δ${strategyFeedbackOutcomes?.trend?.delta?.pnlSummary ?? strategyFeedbackOutcomes?.trend?.delta?.pnlNet ?? 0} / weakest ${(strategyFeedbackOutcomes?.weak || strategyFeedbackOutcomes?.weakest)?.familyBias || 'n/a'} ${(strategyFeedbackOutcomes?.weak || strategyFeedbackOutcomes?.weakest)?.family || 'n/a'} avg ${(strategyFeedbackOutcomes?.weak || strategyFeedbackOutcomes?.weakest)?.avgPnlPercent ?? 'n/a'}%`,
      },
      {
      ...
      lines: report.runtimeLearningLoop
        ? (() => {
          ...
          return [
            ...
            `  strategy feedback outcomes: ${strategyFeedbackOutcomes?.status || 'unknown'} / tagged ${strategyFeedbackOutcomes?.total || strategyFeedbackOutcomes?.totalTagged || 0} / closed ${strategyFeedbackOutcomes?.closed || strategyFeedbackOutcomes?.closedTagged || 0} / pnl ${strategyFeedbackOutcomes?.pnlSummary || (strategyFeedbackOutcomes?.pnlNet ?? 0)}`,
