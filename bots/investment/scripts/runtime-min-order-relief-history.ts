...
const DEFAULT_FILE = '/tmp/investment-runtime-min-order-relief-history.jsonl';
...
function renderText(payload) {
  return [
    '🗂️ Runtime Min Order Relief History',
    `저장 파일: ${payload.file}`,
    ...
    `gap 변화: ${payload.delta.gap >= 0 ? '+' : ''}${Math.round(payload.delta.gap).toLocaleString()} KRW`,
    '',
...
export async function buildRuntimeMinOrderReliefHistory({ days = 14, file = DEFAULT_FILE, json = false } = {}) {
  const report = await buildRuntimeMinOrderReliefReport({ days, json: true });
  const current = {
    recordedAt: new Date().toISOString(),
    status: report.decision.status,
    headline: report.decision.headline,
    gap: Number(report.decision.metrics?.gap || 0),
    blockedByOrderCap: Boolean(report.decision.metrics?.blockedByOrderCap),
    candidateStarterApprove: Number(report.decision.metrics?.candidateStarterApprove || 0),
  ...
  const payload = {
    ...
    delta: { gap: previous ? current.gap - Number(previous.gap || 0) : 0 },
  };
...
async function main() {
  ...
  const result = await buildRuntimeMinOrderReliefHistory(args);
...
if (isDirectExecution(import.meta.url)) {
  await runCliMain({
    run: main,
    errorPrefix: '❌ runtime-min-order-relief-history 오류:',
  });
}
