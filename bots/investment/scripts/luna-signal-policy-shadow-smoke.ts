...
export async function runLunaSignalPolicyShadowSmoke() {
  ...
  assert.equal(written.status, 'luna_signal_policy_shadow_written');
  assert.equal(written.summary.written, 2);
  assert.ok(writes.some((item) => String(item.sql).includes('CREATE TABLE IF NOT EXISTS luna_signal_policy_shadow')));
  assert.ok(writes.some((item) => String(item.sql).includes('INSERT INTO luna_signal_policy_shadow')));
  ...
  return {
    ...
    writtenRows: written.summary.written,
