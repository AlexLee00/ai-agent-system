...
export async function runSmoke() {
  const previousAuto = process.env.LUNA_FAILED_SIGNAL_REFLEXION_AUTO;
  process.env.LUNA_FAILED_SIGNAL_REFLEXION_AUTO = 'false';
  try {
    const signal = {
    ...
    if (previousAuto == null) delete process.env.LUNA_FAILED_SIGNAL_REFLEXION_AUTO;
    else process.env.LUNA_FAILED_SIGNAL_REFLEXION_AUTO = previousAuto;
  }
}
