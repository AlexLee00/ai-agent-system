...
function renderText(result = {}) {
  return [
    `Runtime planner attach dryrun: ${result.ok ? 'ok' : 'failed'}`,
...
export async function runRuntimePlannerAttachDryrun({ market = 'binance', json = false } = {}) {
  ...
  const result = {
    ...
    sessionId: collect.sessionId,
    market: resolvedMarket,
    triggerType: row?.triggerType || 'research',
...
async function main() {
  ...
  const result = await runRuntimePlannerAttachDryrun(args);
...
if (isDirectExecution(import.meta.url)) {
  await runCliMain({
    run: main,
    errorPrefix: '❌ runtime-planner-attach-dryrun 오류:',
  });
}
