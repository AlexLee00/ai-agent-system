...
export const LUNA_COMPONENT_REGISTRY_SEED = Object.freeze([
  ...
  { component: 'ml-price-predictor', currentMode: 'shadow', targetMode: 'shadow_scored', criteria: { minSamples: 200, minIc: 0, lookaheadIntegrity: true, evidence: 'luna_ml_prediction_shadow maturity_status=matured' }, notes: 'C12 shadow forecast persistence and maturity scoring; no live influence' },
  ...
  { component: 'learned-regime-bias', currentMode: 'shadow', targetMode: 'active_router_bias', criteria: { durationWeeks: 4, minSamplesPerFamilyRegime: 30, virtualExpectancyDeltaPositive: true, evidence: 'closed nonpaper trade_journal.strategy_route.learnedBias treatment observations across the fixed 4x4 learner grid' }, notes: 'Self-evolution loop; normalize supported router families into four learner archetypes; historical snapshot count 92 is context only, not promotion evidence' },
  ...
  { component: 'market-deployment-gate', currentMode: 'shadow', targetMode: 'supervised_l4', criteria: { durationWeeks: 4, compareAgainst: 'gate_off_virtual', metrics: ['halt_reduced_avoidance_delta'], evidence: 'luna_market_gate_history' }, notes: 'P1-2 C1 market deployment gate shadow history' },
  { component: 'regime-engine-hmm', currentMode: 'shadow', targetMode: 'core_c2', criteria: { durationWeeks: 4, metrics: ['brier_hmm_lt_fallback', 'transition_alert_precision'], evidence: 'luna_regime_calibration' }, notes: 'P1-3 C2 HMM regime engine shadow facade and calibration' },
  ...
  { component: 'strategy-exit-shadow', currentMode: 'shadow', targetMode: 'supervised_l4', criteria: { durationWeeks: 4, minSamples: 30, exitPerformanceDeltaPositive: true, evidence: 'luna_strategy_exit_shadow' }, notes: 'ET-D C3 strategy-family exit shadow comparison against current position reevaluator; no live closeout mutation' },
  ...
  { component: 'pattern-relaxation-shadow-sim', currentMode: 'shadow', targetMode: 'supervised_l4', criteria: { durationWeeks: 4, minSamplesPerFamilyRegime: 30, positiveExpectancyRequired: true, evidence: 'luna_strategy_signal_outcomes' }, notes: 'C3/C8 pattern-entry threshold relaxation would-have simulation; counts relaxed entries without writing strategy signals' },
  ...
  { component: 'backtest-nextbar-execution', currentMode: 'shadow', targetMode: 'advisory', criteria: { durationWeeks: 4, compareAgainst: 'same_bar_close', metrics: ['nextbar_return_delta', 'nextbar_trade_count_delta'], evidence: 'luna_nextbar_execution_shadow' }, notes: 'P1-6 C7 next-bar execution shadow comparison' },
  { component: 'meeting-room-orchestrator', currentMode: 'shadow', targetMode: 'advisory', criteria: { durationWeeks: 4, completedMeetings: 10, grillCoverage: true, decisionTracking: true, evidence: 'luna_meeting_sessions,luna_meeting_minutes,luna_meeting_decisions' }, notes: 'MR-A CLI meeting-room orchestrator advisory/shadow backend' },
