...
const DEFAULT_FILE = '/tmp/investment-runtime-allow-candidates-validation-history.jsonl';
...
function renderText(payload) {
  const lines = [
    '🗂️ Runtime Allow Candidate Validation History',
    `저장 파일: ${payload.file}`,
    ...
    `ready 변화: ${payload.delta.ready >= 0 ? '+' : ''}${payload.delta.ready}`,
    `blocked 변화: ${payload.delta.blocked >= 0 ? '+' : ''}${payload.delta.blocked}`,
    '',
...
export async function buildRuntimeAllowCandidatesValidationHistory({ days = 14, limit = 20, file = DEFAULT_FILE, json = false } = {}) {
  const report = await buildRuntimeAllowCandidatesValidation({ days, limit, json: true });
  const current = {
    recordedAt: new Date().toISOString(),
    status: report.decision.status,
    headline: report.decision.headline,
    ready: Number(report.decision.metrics.ready || 0),
    observe: Number(report.decision.metrics.observe || 0),
    blocked: Number(report.decision.metrics.blocked || 0),
  ...
  const payload = {
    ...
    delta: {
      ready: previous ? current.ready - Number(previous.ready || 0) : 0,
      blocked: previous ? current.blocked - Number(previous.blocked || 0) : 0,
    },
  };
...
async function main() {
  ...
  const result = await buildRuntimeAllowCandidatesValidationHistory(args);
...
if (isDirectExecution(import.meta.url)) {
  await runCliMain({
    run: main,
    errorPrefix: '❌ runtime-allow-candidates-validation-history 오류:',
  });
}
