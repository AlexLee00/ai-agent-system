...
export function buildRiskApprovalSuggestions(riskApproval = null, riskApprovalTrend = null, config = null) {
  ...
  if (decision.status === 'risk_approval_preview_divergence') {
    suggestions.push({
      key: `runtime_config.nemesis.riskApprovalChain.model.${topModel?.model || 'unknown'}.divergenceReview`,
      current: 'shadow_preview',
      suggested: divergenceDelta > 0 ? 'tighten_threshold_review' : 'inspect_divergence_samples',
      action: total >= 10 && divergence >= 2 ? 'promote_candidate' : 'observe',
      confidence: total >= 20 ? 'medium' : 'low',
      reason: `리스크 승인 preview ${total}건 중 기존 승인/preview 거절 충돌이 ${divergence}건입니다. 직전 대비 divergence Δ${divergenceDelta}이며 top model은 ${topModel?.model || 'n/a'}(adjust ${topModel?.adjust || 0}, reject ${topModel?.reject || 0})입니다. enforce 전환 전 divergence sample과 모델 임계값을 우선 검토합니다.`,
    });
  } else if (total > 0) {
    suggestions.push({
      key: 'runtime_config.nemesis.riskApprovalChain.shadowMonitor',
      current: 'shadow_preview',
    ...
    suggestions.push({
      key: 'runtime_config.nemesis.riskApprovalChain.shadowMonitor',
      current: 'shadow_preview',
      suggested: 'collect_preview_samples',
      action: 'observe',
      confidence: 'low',
      reason: '리스크 승인 preview 표본이 아직 없어 runtime_config 조정보다 네메시스 rationale 표본 누적이 먼저입니다.',
    });
  }
...
function buildRiskApprovalReadinessSuggestions(riskApprovalReadiness = null, riskApprovalReadinessTrend = null) {
  const suggestions = [];
  const decision = riskApprovalReadiness?.decision || {};
  const modeConfig = riskApprovalReadiness?.modeConfig || {};
  const metrics = decision.metrics || {};
  const trendDelta = riskApprovalReadinessTrend?.delta || {};
  const blockerDelta = Number(trendDelta.blockerCount || 0);

  if (decision.status === 'risk_approval_readiness_assist_ready') {
    suggestions.push({
      key: 'runtime_config.nemesis.riskApprovalChain.mode',
      current: modeConfig.mode || 'shadow',
      suggested: 'assist',
      action: 'promote_candidate',
      confidence: Number(metrics.total || 0) >= 50 ? 'medium' : 'low',
      reason: `리스크 승인 체인 preview ${metrics.total || 0}건에서 divergence ${metrics.divergence || 0}, execution stale ${metrics.executionStale || 0}, execution bypass ${metrics.executionBypass || 0}입니다. readiness blocker Δ${blockerDelta}이며, shadow에서 assist로 올리면 preview 거절은 차단하지 않고 금액 감산만 제한적으로 반영할 수 있습니다.`,
    });
  } else if (decision.status === 'risk_approval_readiness_enforce_candidate') {
    suggestions.push({
      key: 'runtime_config.nemesis.riskApprovalChain.mode',
      current: modeConfig.mode || 'assist',
      suggested: 'enforce',
      action: 'promote_candidate',
      confidence: Number(metrics.total || 0) >= 100 ? 'medium' : 'low',
      reason: `assist 운용 표본 ${metrics.total || 0}건이 blocker 없이 누적되어 enforce 후보입니다. readiness blocker Δ${blockerDelta}이며, preview 거절이 실제 차단되어도 되는지 샘플 리뷰 후 마스터 승인으로만 전환해야 합니다.`,
    });
  } else if (decision.status === 'risk_approval_readiness_blocked') {
    suggestions.push({
      key: 'runtime_config.nemesis.riskApprovalChain.mode',
      current: modeConfig.mode || 'shadow',
      suggested: 'hold_current_mode',
      action: 'hold',
      confidence: 'medium',
      reason: `리스크 승인 체인 전환 blocker가 있습니다: ${(decision.blockers || []).join(' / ') || 'unknown'}(Δ${blockerDelta}). mode 전환보다 divergence와 실행 직전 승인 가드 원인을 먼저 해결해야 합니다.`,
    });
  } else {
    suggestions.push({
      key: 'runtime_config.nemesis.riskApprovalChain.mode',
      current: modeConfig.mode || 'shadow',
      suggested: 'continue_readiness_sampling',
      action: 'observe',
      confidence: 'low',
      reason: `${decision.headline || '리스크 승인 체인 전환 준비도를 계속 관찰합니다.'} readiness blocker Δ${blockerDelta}, preview Δ${trendDelta.previewTotal ?? 0}입니다.`,
    });
  }
...
function buildRiskApprovalModeAuditSuggestions(riskApprovalModeAudit = null, riskApprovalModeAuditTrend = null) {
  const suggestions = [];
  const decision = riskApprovalModeAudit?.decision || {};
  const metrics = decision.metrics || {};
  const delta = riskApprovalModeAuditTrend?.delta || {};
  if (decision.status === 'risk_approval_mode_audit_attention') {
    suggestions.push({
      key: 'runtime_config.nemesis.riskApprovalChain.mode',
      current: metrics.currentMode || 'unknown',
      suggested: 'shadow_until_audit_clears',
      action: 'hold',
      confidence: 'medium',
      reason: `${decision.headline || '리스크 승인 mode audit attention'} readiness ${metrics.readinessStatus || 'unknown'} / blockers ${metrics.blockerCount || 0} / non-shadow applications ${metrics.nonShadowApplications || 0}(Δ${delta.nonShadowApplications ?? 0}). mode 전환보다 설정 drift나 조기 적용 여부를 먼저 점검합니다.`,
    });
  } else if (decision.status === 'risk_approval_mode_audit_mode_watch') {
    suggestions.push({
      key: 'runtime_config.nemesis.riskApprovalChain.mode',
      current: metrics.currentMode || 'unknown',
      suggested: 'watch_before_new_buy',
      action: 'observe',
      confidence: 'low',
      reason: `${decision.headline || 'readiness blocker가 남은 non-shadow mode입니다.'} blocker Δ${delta.blockerCount ?? 0}, non-shadow Δ${delta.nonShadowApplications ?? 0}. 신규 BUY 승인 전에 readiness blocker를 재확인합니다.`,
    });
  } else if (decision.status === 'risk_approval_mode_audit_preview_watch') {
    suggestions.push({
      key: 'runtime_config.nemesis.riskApprovalChain.previewTelemetry',
      current: 'preview_unavailable_observed',
      suggested: 'inspect_preview_failed_inputs',
      action: 'observe',
      confidence: 'low',
      reason: `preview unavailable 표본 ${metrics.unavailablePreviewCount || 0}건(Δ${delta.unavailablePreviewCount ?? 0})이 관찰됩니다. risk approval chain 입력/모델/DB 컨텍스트 누락을 확인합니다.`,
    });
  }
  return suggestions;
}
...
function buildSuggestions(
  ...
  riskApprovalReadiness = null,
  riskApprovalReadinessTrend = null,
  riskApprovalModeAudit = null,
...
function printHuman(report) {
  ...
  if (report.riskApproval?.decision) {
    const decision = report.riskApproval.decision;
    const summary = report.riskApproval.summary || {};
    const trend = report.riskApprovalTrend || null;
    lines.push('');
  ...
  if (report.riskApprovalReadiness?.decision) {
    const decision = report.riskApprovalReadiness.decision;
    const dryRun = report.riskApprovalReadiness.modeDryRun || {};
    const trend = report.riskApprovalReadinessTrend || null;
    lines.push('');
    lines.push('risk approval readiness 요약:');
    lines.push(`- ${decision.status}: ${decision.headline}`);
    lines.push(`  current ${decision.currentMode || 'n/a'} -> target ${decision.targetMode || 'n/a'}`);
    lines.push(`  dry-run assist: applied ${dryRun.assist?.applied ?? 0} / rejected ${dryRun.assist?.rejected ?? 0} / amount delta ${dryRun.assist?.amountDelta ?? 0}`);
    lines.push(`  dry-run enforce: applied ${dryRun.enforce?.applied ?? 0} / rejected ${dryRun.enforce?.rejected ?? 0} / amount delta ${dryRun.enforce?.amountDelta ?? 0}`);
    if (trend?.delta) {
      lines.push(`  trend: history ${trend.historyCount || 0} / blocker Δ${trend.delta.blockerCount ?? 0} / preview Δ${trend.delta.previewTotal ?? 0} / reject Δ${trend.delta.previewRejects ?? 0} / divergence Δ${trend.delta.divergence ?? 0}`);
    }
  }
  if (report.riskApprovalModeAudit?.decision) {
    const decision = report.riskApprovalModeAudit.decision;
    const metrics = decision.metrics || {};
    const trend = report.riskApprovalModeAuditTrend || null;
    lines.push('');
    lines.push('risk approval mode audit 요약:');
    lines.push(`- ${decision.status}: ${decision.headline}`);
    lines.push(`  mode ${metrics.currentMode || 'n/a'} / readiness ${metrics.readinessStatus || 'n/a'} / blockers ${metrics.blockerCount || 0}`);
    lines.push(`  application applied ${metrics.applied || 0} / rejected ${metrics.rejected || 0} / non-shadow ${metrics.nonShadowApplications || 0} / unavailable ${metrics.unavailablePreviewCount || 0}`);
    if (trend?.delta) {
      lines.push(`  trend: history ${trend.historyCount || 0} / blocker Δ${trend.delta.blockerCount ?? 0} / non-shadow Δ${trend.delta.nonShadowApplications ?? 0} / unavailable Δ${trend.delta.unavailablePreviewCount ?? 0}`);
    }
  }
...
export async function buildRuntimeConfigSuggestionsReport({ days = 14, write = false } = {}) {
  ...
  const riskApprovalReadiness = await buildRuntimeRiskApprovalReadiness({ days: Math.max(days, 30), json: true }).catch(() => null);
  const riskApprovalReadinessTrend = await buildRuntimeRiskApprovalReadinessHistory({ days: Math.max(days, 30), json: true, write: false }).catch(() => null);
  const riskApprovalModeAudit = await buildRuntimeRiskApprovalModeAudit({ days: Math.max(days, 30), json: true }).catch(() => null);
  ...
  report.aiSummary = await buildInvestmentCliInsight({
    ...
    data: {
      ...
      riskApproval: report.riskApproval?.decision || null,
      ...
      riskApprovalReadiness: report.riskApprovalReadiness?.decision || null,
      riskApprovalModeDryRun: report.riskApprovalReadiness?.modeDryRun || null,
      riskApprovalReadinessTrend: report.riskApprovalReadinessTrend ? {
        historyCount: report.riskApprovalReadinessTrend.historyCount,
        delta: report.riskApprovalReadinessTrend.delta,
      } : null,
      riskApprovalModeAudit: report.riskApprovalModeAudit?.decision || null,
      riskApprovalModeAuditTrend: report.riskApprovalModeAuditTrend ? {
        historyCount: report.riskApprovalModeAuditTrend.historyCount,
        delta: report.riskApprovalModeAuditTrend.delta,
      } : null,
