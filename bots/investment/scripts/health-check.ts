  ...
  try {
    ...
    if (hygiene?.decision?.status === 'position_strategy_hygiene_attention') {
      if (hsm.canAlert(state, key)) {
        ...
        const remediationActions = remediation?.remediationActions || remediationSummary?.actions || null;
        const remediationCommands = remediationSummary?.commands || null;
        const remediationRefreshHint = remediation?.remediationRefreshReason || remediationSummary?.refreshState?.reason || remediation?.remediationRefreshState?.reason || null;
        const remediationTrend = remediation?.remediationTrend || remediationSummary?.trend || null;
        const remediationNextCommandTransition = remediation?.remediationNextCommandTransition || remediationSummary?.nextCommandTransition || null;
        const recommendedExchange = remediation?.remediationRecommendedExchange || remediationSummary?.recommendedExchange || remediationPlan?.recommendedExchange || null;
        const remediationAlertLevel = remediation?.remediationRefreshStale
          || remediationSummary?.refreshState?.stale
        ...
        issues.push({
          ...
          meta: {
            remediationFlat: remediation?.remediationFlat || null,
            remediationSummary,
            remediationStatus: remediation?.remediationStatus || remediationSummary?.status || null,
            remediationHeadline: remediation?.remediationHeadline || remediationSummary?.headline || null,
            remediationCounts: remediation?.remediationCounts || remediationSummary?.counts || null,
            remediationDuplicateManaged: remediation?.remediationDuplicateManaged ?? remediationSummary?.counts?.duplicateManaged ?? null,
            remediationOrphanProfiles: remediation?.remediationOrphanProfiles ?? remediationSummary?.counts?.orphanProfiles ?? null,
            remediationUnmatchedManaged: remediation?.remediationUnmatchedManaged ?? remediationSummary?.counts?.unmatchedManaged ?? null,
            remediationRefreshState: remediationSummary?.refreshState || remediation?.remediationRefreshState || null,
            remediationRefreshNeeded: remediation?.remediationRefreshNeeded ?? remediationSummary?.refreshState?.needed ?? null,
            remediationRefreshStale: remediation?.remediationRefreshStale ?? remediationSummary?.refreshState?.stale ?? null,
            remediationRefreshReason: remediation?.remediationRefreshReason || remediationSummary?.refreshState?.reason || null,
            ...
            remediationTrendChanged: remediation?.remediationTrendChanged ?? remediationTrend?.statusChanged ?? null,
            remediationTrendNextChanged: remediation?.remediationTrendNextChanged ?? remediationTrend?.nextCommandChanged ?? null,
            remediationTrendAgeMinutes: remediation?.remediationTrendAgeMinutes ?? remediationTrend?.ageMinutes ?? null,
            remediationTrendStale: remediation?.remediationTrendStale ?? remediationTrend?.stale ?? null,
            remediationTrendDuplicateDelta: remediation?.remediationTrendDuplicateDelta ?? remediationTrend?.duplicateDelta ?? null,
            remediationTrendOrphanDelta: remediation?.remediationTrendOrphanDelta ?? remediationTrend?.orphanDelta ?? null,
            remediationTrendUnmatchedDelta: remediation?.remediationTrendUnmatchedDelta ?? remediationTrend?.unmatchedDelta ?? null,
            remediationNextCommand: remediation?.remediationNextCommand || remediationSummary?.nextCommand || null,
            remediationNextCommandTransition,
            remediationNextCommandChanged: remediation?.remediationNextCommandChanged ?? remediationTrend?.nextCommandChanged ?? null,
          ...
          msg: `⚠️ [루나 헬스] position strategy remediation\n${remediation?.remediationHeadline || remediationSummary?.headline || remediation?.decision?.headline || hygiene.decision?.headline || '포지션 전략 remediation 후보 감지'}\nduplicate managed scopes ${(remediation?.remediationDuplicateManaged ?? remediationSummary?.counts?.duplicateManaged ?? hygiene.audit?.duplicateManagedProfileScopes ?? 0)} / orphan profiles ${(remediation?.remediationOrphanProfiles ?? remediationSummary?.counts?.orphanProfiles ?? hygiene.audit?.orphanProfiles ?? 0)} / unmatched managed ${(remediation?.remediationUnmatchedManaged ?? remediationSummary?.counts?.unmatchedManaged ?? hygiene.audit?.unmatchedManagedPositions ?? 0)}${remediationTrend ? `\nremediation history: count ${remediation?.remediationTrendHistoryCount ?? remediationTrend.historyCount || 0} / changed ${(remediation?.remediationTrendChanged ?? remediationTrend.statusChanged) ? 'yes' : 'no'} / next changed ${(remediation?.remediationTrendNextChanged ?? remediationTrend.nextCommandChanged) ? 'yes' : 'no'}${(remediation?.remediationTrendNextChanged ?? remediationTrend.nextCommandChanged) ? ` (${remediation?.remediationNextCommandPrevious || remediationTrend.nextCommandTransition?.previous || 'none'} -> ${remediation?.remediationNextCommandCurrent || remediationTrend.nextCommandTransition?.current || 'none'})` : ''} / age ${remediation?.remediationTrendAgeMinutes ?? remediationTrend.ageMinutes ?? 'n/a'}m / stale ${(remediation?.remediationTrendStale ?? remediationTrend.stale) ? 'yes' : 'no'} / duplicate delta ${(remediation?.remediationTrendDuplicateDelta ?? remediationTrend.duplicateDelta ?? 0) >= 0 ? '+' : ''}${remediation?.remediationTrendDuplicateDelta ?? remediationTrend.duplicateDelta || 0} / orphan delta ${(remediation?.remediationTrendOrphanDelta ?? remediationTrend.orphanDelta ?? 0) >= 0 ? '+' : ''}${remediation?.remediationTrendOrphanDelta ?? remediationTrend.orphanDelta || 0}` : (remediationHistory ? `\nremediation history: count ${remediationHistory.historyCount || 0} / changed ${remediationHistory.statusChanged ? 'yes' : 'no'} / next changed ${remediationHistory.nextCommandChanged ? 'yes' : 'no'}${remediationHistory.nextCommandChanged ? ` (${remediationHistory.nextCommandTransition?.previous || 'none'} -> ${remediationHistory.nextCommandTransition?.current || 'none'})` : ''} / age ${remediationHistory.ageMinutes ?? 'n/a'}m / stale ${remediationHistory.stale ? 'yes' : 'no'} / duplicate delta ${remediationHistory.delta?.duplicateManaged >= 0 ? '+' : ''}${remediationHistory.delta?.duplicateManaged || 0} / orphan delta ${remediationHistory.delta?.orphanProfiles >= 0 ? '+' : ''}${remediationHistory.delta?.orphanProfiles || 0}` : '')}${remediationRefreshHint ? `\n${remediationRefreshHint}` : ''}${recommendedExchange ? `\nrecommended exchange: ${recommendedExchange}` : ''}${duplicateSample ? `\nduplicate sample: ${duplicateSample.exchange}/${duplicateSample.symbol} keeper=${duplicateSample.keeperProfileId} retirements=${duplicateSample.retirements?.length || 0}` : ''}${orphanSample ? `\norphan sample: ${orphanSample.exchange}/${orphanSample.symbol} ${orphanSample.tradeMode} ${orphanSample.lifecycleStatus}` : ''}\nnext command: ${remediation?.remediationNextCommand || remediationSummary?.nextCommand || remediationActions?.nextCommand || remediationPlan?.remediationReportCommand || 'npm --prefix /Users/alexlee/projects/ai-agent-system/bots/investment run runtime:position-strategy-remediation -- --json'}\ncommands:\n- ${remediation?.remediationActionReportCommand || remediationCommands?.report || remediationActions?.reportCommand || remediationPlan?.remediationReportCommand || 'npm --prefix /Users/alexlee/projects/ai-agent-system/bots/investment run runtime:position-strategy-remediation -- --json'}\n- ${remediation?.remediationActionHistoryCommand || remediationCommands?.history || remediationActions?.historyCommand || remediationPlan?.remediationHistoryCommand || 'npm --prefix /Users/alexlee/projects/ai-agent-system/bots/investment run runtime:position-strategy-remediation-history -- --json'}\n- ${remediation?.remediationActionRefreshCommand || remediationCommands?.refresh || remediationActions?.refreshCommand || remediationPlan?.remediationRefreshCommand || 'npm --prefix /Users/alexlee/projects/ai-agent-system/bots/investment run runtime:position-strategy-remediation-refresh -- --if-stale --json'}\n- ${remediation?.remediationActionNormalizeDryRunCommand || remediationCommands?.normalizeDryRun || remediationActions?.normalizeDryRunCommand || remediationPlan?.normalizeDryRunCommand || 'npm --prefix /Users/alexlee/projects/ai-agent-system/bots/investment run runtime:normalize-duplicate-strategy-profiles -- --json'}\n- ${remediation?.remediationActionRetireDryRunCommand || remediationCommands?.retireDryRun || remediationActions?.retireDryRunCommand || remediationPlan?.retireDryRunCommand || 'npm --prefix /Users/alexlee/projects/ai-agent-system/bots/investment run runtime:retire-orphan-strategy-profiles -- --json'}`,
        });
      }
