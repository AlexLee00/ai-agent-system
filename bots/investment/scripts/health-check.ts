    ...
    if (hygiene?.decision?.status === 'position_strategy_hygiene_attention') {
      if (hsm.canAlert(state, key)) {
        ...
        issues.push({
          ...
          msg: `⚠️ [루나 헬스] position strategy remediation\n${remediation?.remediationHeadline || remediationFlat?.headline || remediationSummary?.headline || remediation?.decision?.headline || hygiene.decision?.headline || '포지션 전략 remediation 후보 감지'}\nduplicate managed scopes ${(remediation?.remediationDuplicateManaged ?? remediationFlat?.duplicateManaged ?? remediationSummary?.counts?.duplicateManaged ?? hygiene.audit?.duplicateManagedProfileScopes ?? 0)} / orphan profiles ${(remediation?.remediationOrphanProfiles ?? remediationFlat?.orphanProfiles ?? remediationSummary?.counts?.orphanProfiles ?? hygiene.audit?.orphanProfiles ?? 0)} / unmatched managed ${(remediation?.remediationUnmatchedManaged ?? remediationFlat?.unmatchedManaged ?? remediationSummary?.counts?.unmatchedManaged ?? hygiene.audit?.unmatchedManagedPositions ?? 0)}${remediationTrend ? `\nremediation history: count ${remediation?.remediationTrendHistoryCount ?? remediationFlat?.trendHistoryCount ?? remediationTrend.historyCount || 0} / changed ${(remediation?.remediationTrendChanged ?? remediationFlat?.trendChanged ?? remediationTrend.statusChanged) ? 'yes' : 'no'} / next changed ${(remediation?.remediationTrendNextChanged ?? remediationFlat?.trendNextChanged ?? remediationTrend.nextCommandChanged) ? 'yes' : 'no'}${(remediation?.remediationTrendNextChanged ?? remediationFlat?.trendNextChanged ?? remediationTrend.nextCommandChanged) ? ` (${remediation?.remediationNextCommandPrevious || remediationFlat?.nextCommandPrevious || remediationTrend.nextCommandTransition?.previous || 'none'} -> ${remediation?.remediationNextCommandCurrent || remediationFlat?.nextCommandCurrent || remediationTrend.nextCommandTransition?.current || 'none'})` : ''} / age ${remediation?.remediationTrendAgeMinutes ?? remediationFlat?.trendAgeMinutes ?? remediationTrend.ageMinutes ?? 'n/a'}m / stale ${(remediation?.remediationTrendStale ?? remediationFlat?.trendStale ?? remediationTrend.stale) ? 'yes' : 'no'} / duplicate delta ${(remediation?.remediationTrendDuplicateDelta ?? remediationFlat?.trendDuplicateDelta ?? remediationTrend.duplicateDelta ?? 0) >= 0 ? '+' : ''}${remediation?.remediationTrendDuplicateDelta ?? remediationFlat?.trendDuplicateDelta ?? remediationTrend.duplicateDelta || 0} / orphan delta ${(remediation?.remediationTrendOrphanDelta ?? remediationFlat?.trendOrphanDelta ?? remediationTrend.orphanDelta ?? 0) >= 0 ? '+' : ''}${remediation?.remediationTrendOrphanDelta ?? remediationFlat?.trendOrphanDelta ?? remediationTrend.orphanDelta || 0}` : (remediationHistory ? `\nremediation history: count ${remediationHistory.historyCount || 0} / changed ${remediationHistory.statusChanged ? 'yes' : 'no'} / next changed ${remediationHistory.nextCommandChanged ? 'yes' : 'no'}${remediationHistory.nextCommandChanged ? ` (${remediationHistory.nextCommandTransition?.previous || 'none'} -> ${remediationHistory.nextCommandTransition?.current || 'none'})` : ''} / age ${remediationHistory.ageMinutes ?? 'n/a'}m / stale ${remediationHistory.stale ? 'yes' : 'no'} / duplicate delta ${remediationHistory.delta?.duplicateManaged >= 0 ? '+' : ''}${remediationHistory.delta?.duplicateManaged || 0} / orphan delta ${remediationHistory.delta?.orphanProfiles >= 0 ? '+' : ''}${remediationHistory.delta?.orphanProfiles || 0}` : '')}${remediationRefreshHint ? `\n${remediationRefreshHint}` : ''}${recommendedExchange ? `\nrecommended exchange: ${recommendedExchange}` : ''}${duplicateSample ? `\nduplicate sample: ${duplicateSample.exchange}/${duplicateSample.symbol} keeper=${duplicateSample.keeperProfileId} retirements=${duplicateSample.retirements?.length || 0}` : ''}${orphanSample ? `\norphan sample: ${orphanSample.exchange}/${orphanSample.symbol} ${orphanSample.tradeMode} ${orphanSample.lifecycleStatus}` : ''}\nnext command: ${remediation?.remediationNextCommand || remediationFlat?.nextCommand || remediationSummary?.nextCommand || remediationActions?.nextCommand || remediationPlan?.remediationReportCommand || 'npm --prefix /Users/alexlee/projects/ai-agent-system/bots/investment run runtime:position-strategy-remediation -- --json'}\ncommands:\n- ${remediation?.remediationActionReportCommand || remediationFlat?.actionReportCommand || remediationActions?.reportCommand || remediationCommands?.report || remediationPlan?.remediationReportCommand || 'npm --prefix /Users/alexlee/projects/ai-agent-system/bots/investment run runtime:position-strategy-remediation -- --json'}\n- ${remediation?.remediationActionHistoryCommand || remediationFlat?.actionHistoryCommand || remediationActions?.historyCommand || remediationCommands?.history || remediationPlan?.remediationHistoryCommand || 'npm --prefix /Users/alexlee/projects/ai-agent-system/bots/investment run runtime:position-strategy-remediation-history -- --json'}\n- ${remediation?.remediationActionRefreshCommand || remediationFlat?.actionRefreshCommand || remediationActions?.refreshCommand || remediationCommands?.refresh || remediationPlan?.remediationRefreshCommand || 'npm --prefix /Users/alexlee/projects/ai-agent-system/bots/investment run runtime:position-strategy-remediation-refresh -- --if-stale --json'}\n- ${remediation?.remediationActionNormalizeDryRunCommand || remediationFlat?.actionNormalizeDryRunCommand || remediationActions?.normalizeDryRunCommand || remediationCommands?.normalizeDryRun || remediationPlan?.normalizeDryRunCommand || 'npm --prefix /Users/alexlee/projects/ai-agent-system/bots/investment run runtime:normalize-duplicate-strategy-profiles -- --json'}\n- ${remediation?.remediationActionRetireDryRunCommand || remediationFlat?.actionRetireDryRunCommand || remediationActions?.retireDryRunCommand || remediationCommands?.retireDryRun || remediationPlan?.retireDryRunCommand || 'npm --prefix /Users/alexlee/projects/ai-agent-system/bots/investment run runtime:retire-orphan-strategy-profiles -- --json'}`,
        });
      }
