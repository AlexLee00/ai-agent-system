  ...
  for (const row of rows) {
    ...
    for (const meta of (row.meta_rows || [])) {
      ...
      const bucket = validationSummary.get(market) || {
        decision: 0, buy: 0, hold: 0, approved: 0, executed: 0, weak: 0, risk: 0, weakReasons: {}, strategyRouteCounts: {}, strategyRouteQualityCounts: {}, strategyRouteReadinessSum: 0, strategyRouteReadinessCount: 0,
      };
  ...
  for (const market of markets) {
    ...
    if ((summary?.executed || 0) > 0 || trade.total > 0) {
      ...
      const routeReadiness = Number(summary?.strategyRouteReadinessCount || 0) > 0
        ...
        : null;
      lines.push(`  - ${getMarketLabel(market)}: 승격 후보 — validation에서 executed ${summary?.executed || 0}, trades ${trade.total} (LIVE ${trade.live} / PAPER ${trade.paper})${topWeakReason ? ` | weakTop ${topWeakReason[0]}` : ''}${topRoute ? ` | routeTop ${topRoute[0]}` : ''}${topRouteQuality ? ` | routeQuality ${topRouteQuality[0]}` : ''}${routeReadiness == null ? '' : ` | readiness ${routeReadiness}`}`);
      continue;
    }
