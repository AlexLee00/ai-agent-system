defmodule Luna.V2.LLM.CostTracker do
  @moduledoc """
  루나 V2 LLM 비용 추적 (일일 예산 대비).

  DB: luna_llm_cost_tracking 테이블
  환경변수: LUNA_LLM_DAILY_BUDGET_USD (기본 $30.0)
  ...
  def handle_call({:track_tokens, entry}, _from, state) do
    ...
    result =
      Jay.Core.Repo.query(
        """
        INSERT INTO luna_llm_cost_tracking
    ...
    case result do
      {:ok, _}    -> :ok
      {:error, e} -> Logger.error("[루나V2 비용] DB INSERT 실패: #{inspect(e)}")
    end
  ...
  def handle_call(:check_budget, _from, state) do
    daily_limit =
      System.get_env("LUNA_LLM_DAILY_BUDGET_USD", "30.0")
      |> parse_float(30.0)
    ...
    daily_spent =
      case Jay.Core.Repo.query(
             "SELECT COALESCE(SUM(cost_usd), 0.0) FROM luna_llm_cost_tracking WHERE timestamp::date = CURRENT_DATE",
             []
           ) do
    ...
    result =
      if daily_spent < daily_limit do
        ...
        Logger.error("[루나V2 비용] 일일 예산 초과: $#{daily_spent} / $#{daily_limit}")
