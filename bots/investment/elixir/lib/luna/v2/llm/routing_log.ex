defmodule Luna.V2.LLM.RoutingLog do
  @moduledoc """
  루나 V2 LLM 라우팅 기록 — 모델 선택 품질 측정 + 실패율 피드백용.

  DB: luna_v2_llm_routing_log 테이블
  ...
  entry 필드:
    ...
    response_ok, error_reason, urgency, task_type, budget_ratio, recommended_reason, provider
  """
  def record(entry) do
  ...
  defp do_record(entry) do
    sql = """
    INSERT INTO luna_v2_llm_routing_log
      (agent_name, model_primary, model_used, fallback_used,
    ...
    e ->
      Logger.warning("[루나V2 LLM] routing_log 기록 실패 (#{inspect(e)}) — 무시")
      :ok
  end
