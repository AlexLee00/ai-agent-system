...
function normalizeLegacyKoreanLlmNoise(content) {
  return dedupeDeterministicAnalysisTitle(content)
    ...
    .replace(/\*{0,2}장전\s*중단\*{0,2}\s*상태/g, 'halt 상태')
    ...
    .replace(/C2\s*레짐에\s*따르면,\s*/g, '레짐 기준으로, ')
    .replace(/C2\s*레짐\s*[:：]\s*/g, '레짐: ')
    ...
    .replace(/\*{0,2}\bbull\b\*{0,2}\s+상태/g, '상승 레짐')
    .replace(/\*{0,2}\bbear\b\*{0,2}\s+상태/g, '하락 레짐')
    .replace(/\*{0,2}\bsideways\b\*{0,2}\s+상태/g, '수평 레짐')
    .replace(/\*{0,2}\bvolatile\b\*{0,2}\s+상태/g, '변동 레짐')
    ...
    .replace(/따라서,\s*추가적인 분석과 조치가 필요합니다\./g, '따라서 추가 분석이 필요하며, 조치는 별도 마스터 확인 후 다룹니다.')
    .replace(/따라서,\s*([^.\n]*?)조치를 취해야 합니다\./g, '따라서 $1마스터 확인 대상으로 유지합니다.')
    ...
    .replace(/활성 서킷:\s*최신 데이터 영역 기준으로 봅니다\.\s*으로,?\s*/g, '활성 서킷은 최신 데이터 영역 기준입니다. ')
    ...
    .replace(/활성 서킷:\s*최신 데이터 영역 기준으로 확인하세요/g, '활성 서킷은 최신 데이터 영역 기준입니다')
...
function inferAskIntent(question) {
  ...
  if (hasLaunchdCue && hasMeetingJobCue) return 'schedule_ops';
...
function buildRuleBasedAgentAnswer(agent, question, planNote = {}, globalPendingDecisions = [], options = {}) {
  ...
  if (intent === 'schedule_ops') {
    return [
      ...
      '2. launchd 상태는 launchctl print gui/$(id -u)/ai.luna.meeting-morning-0500 명령에서 runs, last exit code, calendar trigger를 봅니다.',
      '3. PID - 또는 not running은 calendar job이 실행 시간 사이에 대기 중이라는 뜻일 수 있으므로 단독으로 실패로 보지 않습니다.',
      '4. 실행 로그는 /Users/alexlee/.ai-agent-system/logs/luna-meeting-morning.log 와 /Users/alexlee/.ai-agent-system/logs/luna-meeting-morning-error.log 를 확인합니다. 첫 실행 전에는 로그 파일이 없거나 0바이트일 수 있습니다.',
      '5. launchctl print 전체 출력은 공유하지 않습니다. inherited/default/environment 블록은 토큰·secret이 섞일 수 있으므로 삭제하고, label/state/runs/last exit code/calendar trigger만 남깁니다.',
      '6. 로그에 secret이나 토큰 값을 붙여 공유하지 말고, 오류 요약과 회의 ID 생성 여부만 남깁니다.',
      options.scheduleStatus || buildScheduleExecutionStatus([], options.now || new Date()),
      `권장 다음 행동: ${ruleBasedActionForIntent(intent, false)}`,
      `질문 요지: ${String(question || '').slice(0, 160)}`,
    ].join('\n');
  ...
  if (intent === 'telegram') {
    ...
    return [
      ...
      '상위 9건 기준: 웹 전체 결정 대기함 표시 순서와 동일하게 기한이 빠른 항목을 먼저 보고, 기한이 없으면 최근 생성 항목을 먼저 봅니다.',
