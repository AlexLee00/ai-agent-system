defmodule TeamJay.Blog.Writer.Pos do
  @moduledoc """
  블로그팀 강의 포스팅 writer 스캐폴드.

  `research_done` 이벤트를 받아 lecture 포스트만 골라
  `draft_ready("pos")` 토픽으로 스캐폴드 draft를 발행한다.
  """
  ...
  def handle_info({:blog_event, _topic, {:research_done, result}}, state) do
    drafts =
      result.items
      |> Enum.filter(&(&1.post_type == :lecture))
      |> Enum.map(&build_draft/1)

    Enum.each(drafts, fn draft ->
      :ok = PubSub.broadcast_draft_ready("pos", draft)
    end)
  ...
  defp build_draft(item) do
    Map.merge(item, %{
      ...
      note: "Phase 1 pos writer scaffold"
    })
  end
end
