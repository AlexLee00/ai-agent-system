defmodule TeamJay.Blog.InstagramExecutor do
  @moduledoc """
  블로그팀 인스타그램 채널 실행 준비기.

  `handoff:instagram` 이벤트를 받아 실제 업로드 직전의 실행 payload를 만든다.
  현재는 공개 URL/캡션/채널 메타를 담은 scaffold execution만 고정한다.
  """
  ...
  @impl true
  def init(_opts) do
    {:ok, _ref} = PubSub.subscribe(Topics.handoff("instagram"))
  ...
  @impl true
  def handle_info({:blog_event, _topic, {:handoff_ready, "instagram", queued}}, state) do
    execution = build_execution(queued)
    :ok = PubSub.broadcast_execution("instagram", execution)
  ...
  defp build_execution(queued) do
    ...
    %{
      ...
      execution_note: "Phase 1 instagram executor prepared upload scaffold",
      channel: "instagram",
