defmodule TeamJay.Blog.NaverBlogAgent do
  @moduledoc """
  블로그팀 네이버 블로그 채널 스캐폴드.

  `social:naver_blog` 이벤트를 받아 실제 배포 직전의
  채널 payload를 정리하고 큐잉 상태를 만든다.

  현재는 업로드 호출 없이, 네이버 블로그 전용 준비 상태만 고정한다.
  """
  ...
  @impl true
  def init(_opts) do
    {:ok, _ref} = PubSub.subscribe(Topics.social("naver_blog"))
  ...
  defp build_naver_blog_queue(relay) do
    %{
      channel: "naver_blog",
      queue_status: :queued,
      queue_note: "Phase 1 naver blog agent scaffold queued",
