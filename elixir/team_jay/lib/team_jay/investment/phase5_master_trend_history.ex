defmodule TeamJay.Investment.Phase5MasterTrendHistory do
  @moduledoc """
  Phase 5 master trend 스냅샷을 DB에 누적 저장하고 직전 상태와 비교한다.
  """
  ...
  defp ensure_table! do
    ...
    SQL.query!(
      Repo,
      """
      CREATE TABLE IF NOT EXISTS investment.phase5_master_trend_snapshots (
    ...
    SQL.query!(
      Repo,
      """
      CREATE INDEX IF NOT EXISTS phase5_master_trend_snapshots_recorded_at_idx
      ON investment.phase5_master_trend_snapshots (recorded_at DESC)
      """,
      []
    )
  end
  ...
  defp persist_snapshot(recorded_at, result) do
    SQL.query!(
      Repo,
      """
      INSERT INTO investment.phase5_master_trend_snapshots (
        status,
        ready,
        blocker_count,
        total_delta_rows,
      ...
      [
        ...
        Atom.to_string(result.trend_status),
        recorded_at
      ]
    )
  end
