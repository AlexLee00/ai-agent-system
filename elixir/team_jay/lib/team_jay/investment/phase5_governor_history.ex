defmodule TeamJay.Investment.Phase5GovernorHistory do
  @moduledoc """
  Phase 5 governor 스냅샷을 DB에 누적 저장하고 직전 상태와 비교한다.
  """
  ...
  defp ensure_table! do
    ...
    SQL.query!(
      Repo,
      """
      CREATE TABLE IF NOT EXISTS investment.phase5_governor_snapshots (
        ...
        operations_history_status TEXT NOT NULL,
        mission_control_status TEXT NOT NULL,
        closeout_status TEXT NOT NULL,
        diagnostics_severity TEXT NOT NULL,
    ...
    SQL.query!(
      Repo,
      """
      CREATE INDEX IF NOT EXISTS phase5_governor_snapshots_recorded_at_idx
      ON investment.phase5_governor_snapshots (recorded_at DESC)
      """,
      []
    )
  end
  ...
  defp previous_snapshot do
    case SQL.query(
           ...
           FROM investment.phase5_governor_snapshots
  ...
  defp persist_snapshot(recorded_at, result, blockers) do
    SQL.query!(
      Repo,
      """
      INSERT INTO investment.phase5_governor_snapshots (
        status,
        ready,
        blocker_count,
        blockers,
        operations_status,
        operations_history_status,
      ...
      [
        ...
        Atom.to_string(result.operations.status),
        Atom.to_string(result.operations_history.status),
        Atom.to_string(result.mission_control.status),
        Atom.to_string(result.closeout.status),
        Atom.to_string(result.operations.diagnostics_severity),
        recorded_at
      ]
    )
  end
