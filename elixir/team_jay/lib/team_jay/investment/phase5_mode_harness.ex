defmodule TeamJay.Investment.Phase5ModeHarness do
  @moduledoc """
  Phase 5-E 장기/단기 동적 전환 scaffold가 strategy_profile까지 도달하는지 확인하는 harness.
  """
  ...
  def run_once(opts \\ []) do
    ...
    topics = [Topics.market_modes(symbol), Topics.strategy_profiles(symbol)]
  ...
  defp loop(symbol, deadline, events) do
    ...
    receive do
      {:investment_event, topic, payload} ->
        ...
        if topic == Topics.strategy_profiles(symbol) do
          %{
            status: :ok,
            completed: true,
