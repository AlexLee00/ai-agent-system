defmodule TeamJay.Investment.Phase5MasterStatusHistoryReport do
  @moduledoc """
  Phase 5 master status 변화를 사람이 읽기 쉬운 텍스트로 정리한다.
  """

  alias TeamJay.Investment.Phase5MasterStatusHistory

  def run_defaults(opts \\ []) do
    result = Phase5MasterStatusHistory.run_defaults(opts)
  ...
  def render(result) do
    header =
      case result.status do
        :phase5_master_advanced -> "Phase 5 master history ADVANCED"
        :phase5_master_ready -> "Phase 5 master history READY"
        _ -> "Phase 5 master history CHECK"
      end
