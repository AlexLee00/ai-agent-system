defmodule TeamJay.Investment.Phase5MissionControlHistory do
  @moduledoc """
  Phase 5 mission control 스냅샷을 DB에 누적 저장하고 직전 상태와 비교한다.
  """
  ...
  defp ensure_table! do
    ...
    SQL.query!(
      Repo,
      """
      CREATE TABLE IF NOT EXISTS investment.phase5_mission_control_snapshots (
        ...
        command_center_status TEXT NOT NULL,
        resource_health_status TEXT NOT NULL,
    ...
    SQL.query!(
      Repo,
      """
      CREATE INDEX IF NOT EXISTS phase5_mission_control_snapshots_recorded_at_idx
      ON investment.phase5_mission_control_snapshots (recorded_at DESC)
      """,
      []
    )
  end
  ...
  defp previous_snapshot do
    case SQL.query(
           ...
           FROM investment.phase5_mission_control_snapshots
  ...
  defp persist_snapshot(recorded_at, result, blockers) do
    SQL.query!(
      Repo,
      """
      INSERT INTO investment.phase5_mission_control_snapshots (
        status,
        ready,
        blocker_count,
        blockers,
      ...
      [
        ...
        Atom.to_string(result.executive.status),
        Atom.to_string(result.command_center.status),
        Atom.to_string(result.resource_health.status),
