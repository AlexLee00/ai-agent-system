defmodule TeamJay.Investment.Phase5ResourceHealthPersistenceReport do
  @moduledoc """
  Phase 5 resource health DB persistence 검증 결과를 텍스트로 정리한다.
  """
  ...
  def render(result) do
    header =
      if result.all_ok do
        "Phase 5 resource health persistence OK (#{result.passed}/#{result.total})"
      else
        "Phase 5 resource health persistence CHECK (#{result.passed}/#{result.total})"
      end
