defmodule TeamJay.Investment.Phase5ExecutiveHistory do
  @moduledoc """
  Phase 5 executive 스냅샷을 DB에 누적 저장하고 직전 상태와 비교한다.
  """
  ...
  defp ensure_table! do
    ...
    SQL.query!(
      Repo,
      """
      CREATE TABLE IF NOT EXISTS investment.phase5_executive_snapshots (
        ...
        command_center_history_status TEXT NOT NULL,
    ...
    SQL.query!(
      Repo,
      """
      CREATE INDEX IF NOT EXISTS phase5_executive_snapshots_recorded_at_idx
      ON investment.phase5_executive_snapshots (recorded_at DESC)
      """,
      []
    )
  end
  ...
  defp previous_snapshot do
    case SQL.query(
           ...
           FROM investment.phase5_executive_snapshots
  ...
  defp persist_snapshot(recorded_at, result, blockers) do
    SQL.query!(
      Repo,
      """
      INSERT INTO investment.phase5_executive_snapshots (
        status,
        ready,
        blocker_count,
        blockers,
        command_center_status,
        command_center_history_status,
      ...
      [
        ...
        Atom.to_string(result.command_center_history.status),
