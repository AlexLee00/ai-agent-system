defmodule TeamJay.Investment.Phase5LoopHarness do
  @moduledoc """
  Phase 5-B trading loop scaffold가 cycle_complete까지 도달하는지 확인하는 harness.
  """
  ...
  def run_once(opts \\ []) do
    ...
    topics = [Topics.loop_cycles(symbol), Topics.feedback(symbol), Topics.condition_checks(symbol)]
  ...
  defp loop(symbol, deadline, events) do
    ...
    receive do
      {:investment_event, topic, payload} ->
        ...
        if topic == Topics.loop_cycles(symbol) do
          %{
            status: :ok,
            completed: true,
