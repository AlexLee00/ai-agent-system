defmodule TeamJay.Investment.Phase5CircuitPersistenceReport do
  @moduledoc """
  Phase 5.5-5 서킷 브레이커 DB persistence 검증 결과를 텍스트로 정리한다.
  """
  ...
  def render(result) do
    header =
      if result.all_ok do
        "Phase 5.5-5 circuit breaker persistence OK (#{result.passed}/#{result.total})"
      else
        "Phase 5.5-5 circuit breaker persistence CHECK (#{result.passed}/#{result.total})"
      end
