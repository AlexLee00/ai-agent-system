defmodule TeamJay.Investment.Phase5OperationsHistory do
  @moduledoc """
  Phase 5 operations 스냅샷을 DB에 누적 저장하고 직전 상태와 비교한다.
  """
  ...
  defp ensure_table! do
    ...
    SQL.query!(
      Repo,
      """
      CREATE TABLE IF NOT EXISTS investment.phase5_operations_snapshots (
    ...
    SQL.query!(
      Repo,
      """
      CREATE INDEX IF NOT EXISTS phase5_operations_snapshots_recorded_at_idx
      ON investment.phase5_operations_snapshots (recorded_at DESC)
      """,
      []
    )
  end
  ...
  defp persist_snapshot(recorded_at, result, blockers) do
    SQL.query!(
      Repo,
      """
      INSERT INTO investment.phase5_operations_snapshots (
        status,
        ready,
        blocker_count,
        blockers,
      ...
      [
        ...
        Atom.to_string(result.diagnostics.severity),
        recorded_at
      ]
    )
  end
