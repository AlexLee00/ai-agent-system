defmodule TeamJay.Claude.FeedbackLoop do
  ...
  - codex_approval/codex_rejection → 폐기된 docs/codex 실행 경로이므로 무시
  ...
  defp process_event(%{"event_type" => "codex_approval"} = event, state) do
    ...
    Logger.info("[FeedbackLoop] 코덱스 승인 무시: #{codex_name} — docs/auto_dev 인박스를 사용해야 함")
    state
  end

  defp process_event(%{"event_type" => "codex_rejection"} = event, state) do
    codex_name = event["title"] || event["bot_name"]
    Logger.info("[FeedbackLoop] 코덱스 거부 무시: #{codex_name} — docs/codex 실행 경로 폐기됨")
    state
  end
  ...
  defp process_event(%{"event_type" => type} = event, state) do
    case Map.get(@team_error_events, type) do
      ...
      {layer, reason} ->
        ...
        %{state | dispatch_count: state.dispatch_count + 1, last_dispatch: DateTime.utc_now()}
    end
  end
