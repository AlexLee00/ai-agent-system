...
function aggregateIncidents(incidents) {
  ...
  for (const item of incidents) {
    ...
    if (!byTeam.has(item.label)) {
      byTeam.set(item.label, { label: item.label, team: item.team, count: 0, high: 0, medium: 0, low: 0 });
    }
    ...
    if (!byCategory.has(item.category)) {
      byCategory.set(item.category, { category: item.category, label: item.categoryLabel, count: 0, severity: item.severity });
    }
    byCategory.get(item.category).count += 1;
  }

  return {
    repeated: Array.from(byGroup.values()).sort((a, b) => {
      const sev = severityRank(b.severity) - severityRank(a.severity);
      if (sev !== 0) return sev;
      return b.count - a.count;
    }),
    teams: Array.from(byTeam.values()).sort((a, b) => b.count - a.count),
...
function main() {
  ...
  const report = {
    ...
    recommendations: buildRecommendations(aggregated.categories, aggregated.repeated),
  };
  ...
  if (report.teams.length) {
    ...
    for (const item of report.teams.slice(0, 10)) {
      out.push(`- ${item.label}: 총 ${fmt(item.count)}건 (high ${fmt(item.high)} / medium ${fmt(item.medium)} / low ${fmt(item.low)})`);
    }
  }
  ...
  if (report.categories.length) {
    out.push('');
    out.push('오류 유형 상위:');
    for (const item of report.categories.slice(0, 10)) {
      out.push(`- ${item.label}: ${fmt(item.count)}건`);
    }
  }
  ...
  if (report.repeated.length) {
    out.push('');
    out.push('반복 오류:');
    for (const item of report.repeated.slice(0, 8)) {
