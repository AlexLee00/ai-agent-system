...
코덱스:   docs/codex/ (분석/설계/검토 문서, 자동실행 금지) + docs/auto_dev/ (자동 구현 단일 인박스)
...
2. `docs/codex/`는 자동 구현 트리거가 아니다. 분석/설계/검토 문서 보관 전용이며, 감시/실행/커밋/푸시 경로를 두지 않는다.
3. 클로드팀 auto-dev가 `docs/auto_dev/*.md`를 접수하면 문서/코드 분석 → 구현계획 알림 → 구현 → 리뷰/보안 → 수정 → 테스트 → 수정 → 완료 알림 순서로 처리
4. 코덱스 완료 시 → `docs/archive/codex-completed/`로 이동
5. 세션 종료 시 → `docs/OPUS_FINAL_HANDOFF.md` 업데이트 필수
6. 전략 변경 시 → `docs/STRATEGY.md` 또는 `docs/strategy/{팀}.md` 반영
7. 작업 기록 → `docs/history/WORK_HISTORY.md`에 날짜별 추가
8. 연구/리서치 → `docs/research/`에 축적
...
- **PROTECTED launchd 무중단**: `ai.{ska,luna,investment,claude,elixir,hub}.*` (11개) — 직접 중지 금지 (스카 매출 + 루나 LIVE 영향!)
...
## 🌟 Karpathy 행동 가이드라인 (적용 — 2026-05-12)

> 출처: [forrestchang/andrej-karpathy-skills](https://github.com/forrestchang/andrej-karpathy-skills) (71k+ stars)
> 풀버전: `.claude/rules/karpathy-full.md` (65줄, 원본 그대로)

| # | 원칙 | 핵심 |
|---|------|------|
| 1 | **Think Before Coding** | 가정 X → 명시! 모호성 → 옵션 제시! 막힘 → 질문! |
| 2 | **Simplicity First** | 최소 코드! 1000줄 가능하면 100줄! 추측 X! |
| 3 | **Surgical Changes** | 요청만 변경! 인접 코드 X! 무관 dead code 언급만! |
| 4 | **Goal-Driven Execution** | "Fix bug" → "테스트 작성 + 통과"! 검증 가능 성공 기준! |

**Caution > Speed**: 단순 작업은 판단 사용. 풀버전은 `.claude/rules/karpathy-full.md` 참조.
