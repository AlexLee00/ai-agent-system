...
1. 워커 `attendance`에 확인 결과 창 추가
2. 출근/퇴근을 `proposal_generated -> field_edited -> confirmed -> committed`로 연결
3. 역할별 `llm_mode` 정책 테이블 추가
4. committed feedback를 `feedback_cases` RAG artifact로 저장
