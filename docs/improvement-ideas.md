...
|---|---|---|---|---|
| 진행 중 | 2026-03-16 | 워커 확인 결과 창 UX | attendance, leave, schedules, employees, payroll, sales, projects, journals에 자연어 제안/수정/확정 흐름 연결 완료. 주요 메뉴는 `메뉴 내부 프롬프트 1개 + 하단 승인 리스트` 구조로 통일됨 | 대시보드 운영 캔버스 고도화, 승인 리스트 시각 정교화 |
...
| 상태 | 목표 | 비고 |
|---|---|---|
| 진행 중 | 자연어 대화형 업무 등록 메인 UX | 근태, 일정, 직원, 급여, 매출, 프로젝트, 업무일지에 확인 결과 창 기반 자연어 등록이 연결됨. 현재는 핵심 메뉴가 `메뉴 내부 프롬프트 1개 + 하단 승인 리스트` 구조까지 정리됨 |
...
| 진행 중 | 채팅 + 캔버스 레이아웃 정착 | dashboard는 `프롬프트 + 결과 캔버스`, 정형 메뉴는 `로컬 프롬프트 + 승인 리스트` 구조로 정리됨. 남은 것은 시각적 일관화와 마스터 캔버스 심화 |
