...
| 진행 중 | 2026-03-16 | 워커 권한별 화면 분기 | dashboard 역할 분기, 공용 채팅의 메뉴별 prompt/canvas 정책, 페이지 액션 정책까지 반영됨. 마스터/관리자 대시보드 프롬프트는 대상 봇 선택까지 연결됨 | 관리자 현황 위젯 심화 |
...
| 진행 중 | 봇 대화 API의 역할별 확장 | 마스터/관리자 대시보드 프롬프트에서 대상 봇 선택과 권장 봇 자동 선택이 연결됨. 남은 것은 메뉴별 전문 봇 시나리오 심화 |
