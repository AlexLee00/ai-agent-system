...
## 2026-07-19 — 스카 예약 복구 및 결제 완료 검증 강화
- 네이버 확정 상태로 재확인된 미래 예약 4건을 픽코에 등록하고, 픽코 상세와 예약 DB를 `completed / verified`로 정합화했다.
- 결제 제출 전 모든 가격 입력값과 직렬화 payload가 0원인지 2회 안정 판정하고, 제출 후에는 전화번호·날짜·룸·전체 시간·최종 결제금액이 정확히 일치할 때만 완료로 인정하도록 보강했다.
- 취소 후 같은 예약이 다시 활성화될 때 과거 픽코 주문·오류·완료·seen 상태를 초기화하고, 자정 통과 예약은 날짜별 2건으로 분리하도록 fail-closed 검증을 추가했다.
- 전문가 독립 리뷰에서 제기된 전화번호 suffix 충돌, 다른 금액 행의 `0원` 오인, 숨은 결제 payload, 자정 종료시간 문제를 수정하고 재검증했다.
- 보호된 `ai.ska.naver-monitor`는 수동 중단하지 않았으며, 12:30 자연 재기동 후 로그인·확정/취소 파싱·후보 없음 상태가 정상임을 확인했다.
