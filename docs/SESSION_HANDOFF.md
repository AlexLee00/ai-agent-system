...
- 비디오
  ...
  - 2026-03-22 검증 기준선은 `scene_count=42`, `segment_count=3`, `sync_confidence=0.3445`, `keyword=2`, `unmatched=1`이다. 현재 1순위 보강 포인트는 `scene-indexer`보다 오프라인 `narration-analyzer` fallback 세그먼트 granularity와 `sync-matcher` 첫 구간 unmatched 처리다.
