...
- Hub/team selector 기본 모델
  - 외부 설정 파일: Hub/selector runtime config
...
- 정책 집약 파일
  ...
  - 의미: Hub 기본 모델과 별개로, 제이 앱 레벨 커스텀 모델 정책을 한 곳에서 관리
...
- gateway 정합성 점검/동기화
  ...
  - 의미: `runtime_config.jayModels.gatewayPrimary`와 Hub/team selector primary 일치 여부를 확인
  ...
  - 필요 시 `--apply`로 Hub/team selector primary를 runtime_config 기준으로 동기화 가능
- gateway 실험 로그 스냅샷
  ...
  - 기본 저장 위치: Jay runtime experiment log store
- gateway 실험 리뷰
