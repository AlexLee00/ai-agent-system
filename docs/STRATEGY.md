...
팀별 적용 계획:
  제이:   OpenClaw 라우팅 최적화 → 일일 사용 패턴 학습 → 자동 조정
  루나팀: Chronos 전략 → 백테스트 → 승격/강등 자동 루프
         RAG에 과거 매매 사례 축적 → 유사 상황 시 참조
  스카팀: 예약 실패 사례 RAG → 복구 전략 자동 선택
         forecast shadow → primary 자동 승격
  블로팀: 글 품질 피드백 → 프롬프트 자동 최적화 (OpenAI Self-Evolving)
         독자 반응 데이터 → 주제/스타일 자동 조정

RAG 일일 성장 사이클:
  매일 자정: 운영 데이터 → pgvector 임베딩 저장
  매일 아침: 전일 성과 분석 → 패턴 추출 → runtime_config 조정 제안
  주간: 누적 학습 → 프롬프트 개선안 생성 → Shadow 비교 → 승격
```
...
운영 중:
  ...
  4. 클로드팀 — 시스템 모니터링/복구 (덱스터/아처/닥터)
  5. 블로팀 — 네이버 블로그 자동화
  6. 워커팀 — 비즈니스 관리 SaaS
  7. 에디팀 — YouTube 영상 편집 자동화

예정:
  8. 연구팀 — 기술 리서치/논문/벤치마크
  9. 감정팀 — (미정)
```
