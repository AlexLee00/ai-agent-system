...
- 2026-03-28: `node --check bots/investment/scripts/health-report.js` / `node bots/investment/scripts/health-report.js --json` / 텍스트 리포트에서 `cryptoValidationBudgetPolicyHealth.decision = consider_policy_split`, `decisionLabel = 정책 분리 검토`, 근거 `soft cap 차단 0건 / validation capital guard 90.8% / LIVE gate blocked / closed review 1 / weak 50`이 함께 출력되는 것 확인.
