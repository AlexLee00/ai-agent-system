...
- 기존 `공용 프롬프트` 개념은 워커 웹에서 `프롬프트 어드바이저` 공용 컴포넌트로 대체 중이다.
- 이 컴포넌트는 다음을 기본 제공한다.
  - 챗형 입력 바
  - 파일 추가
  - 초기화 / 전송
  - 마크다운 결과 렌더링
  - 동적 캔버스 렌더링
- 현재는 [dashboard/page.js](/Users/alexlee/projects/ai-agent-system/bots/worker/web/app/dashboard/page.js)에서 먼저 사용하고, 이후 필요 메뉴로 확장하는 방식이 적합하다.
...
- AI 분석
  - 관리자/마스터 상단 `운영 바로가기`
  - `Prompt Flows` 카드에서 프롬프트 재질의 또는 관련 메뉴 이동 가능
- 정형 메뉴
  - 메뉴 내부 프롬프트 1개
  - 하단 `확인 및 승인 대기` 리스트
  - 유사 확정 사례
  - 승인 대기 카드와 유사 사례 카드 모두 프롬프트 흐름과 연결
  - 관리자/마스터일 때 상단 `운영 바로가기`
  - 주요 메뉴는 공통 `상단 요약 헤더 + 빠른 액션 카드` 축으로 정리됨
- 마스터 관리 메뉴
  - 인텐트 학습, 업체 관리, 사용자 관리, 직원 관리, 급여 관리
  - 관리자/마스터 상단 `운영 바로가기`
...
현재 워커 웹은 다음 흐름을 공통 패턴으로 사용한다.

`운영 요약/리스트/사례 → 프롬프트에 채우기 → 확인/승인 → feedback 저장 → RAG 사례 축적`
...
- 정형 메뉴의 상단 빠른 액션 카드
- AI 분석의 Prompt Flows
- 관리자/마스터 화면의 빠른 액션 카드

즉 워커 웹은 이제 단순 페이지 모음이 아니라, 여러 화면이 같은 프롬프트 재사용 흐름과 feedback 학습 흐름 위에서 연결되는 구조다.
...
2. 관리자 현황 위젯 강화
3. 마스터용 봇 대화 경험 심화
4. `운영 바로가기`를 기준으로 관리자 화면 정보 구조와 톤 통일
5. 정형 메뉴의 상단 요약 헤더와 빠른 액션 카드 시각 완성도 마감
6. feedback analytics와 품질 리포트 자동화
