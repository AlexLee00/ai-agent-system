...
- GitHub Agentic Workflows는 Markdown으로 저장소 자동화를 정의하고, GitHub Actions의 guardrail 안에서 Copilot/Claude/Codex 같은 에이전트를 구동하는 방향이다. 클로드팀도 문서 기반 자동 구현은 맞지만, OPS 로컬 환경이므로 launchd/Guardian/Builder를 guardrail로 삼는다.
...
- 실제 구현 시작은 이제 `ai.claude.auto-dev` launchd 러너가 담당한다. hook/FileChanged 전환은 추후 최적화 후보로 남기되, 현재 운영 기본값은 `docs/auto_dev` 5분 감시 + 상태머신 실행이다.

## 3. 이번 구현 결정

- `docs/auto_dev/`를 클로드팀 자동 구현 인박스로 승격한다.
- 아처는 `docs/auto_dev/PATCH_REQUEST.md`를 생성한다.
- 코덱스 알림기는 `docs/auto_dev`만 감지한다. 기존 `docs/codex`는 보안 보호 대상일 뿐 자동 구현 감지 경로로 쓰지 않는다.
- `docs/auto_dev/*`는 `.gitignore`와 pre-commit으로 보호한다.
- `CLAUDE.md` 세션 시작 루틴에 `docs/auto_dev` 확인을 추가한다.
- `bots/claude/lib/auto-dev-pipeline.ts`를 추가해 문서별 상태, 시작/종료 알림, 리뷰/보안/테스트 수정 루프를 관리한다.
- `bots/claude/launchd/ai.claude.auto-dev.plist`를 추가해 `docs/auto_dev/*.md` 인박스를 5분 주기로 처리한다.

## 4. 자동 구현 라이프사이클

| 단계 | 담당 에이전트 | 실행 내용 |
|------|--------------|-----------|
| 문서 접수 | auto-dev-orchestrator | `docs/auto_dev/*.md` 스캔, 중복 job 차단 |
| 문서/코드 분석 | auto-dev-orchestrator | 제목, 참조 파일, 검색어, 관련 코드 후보 추출 |
| 구현계획 수립 | auto-dev-orchestrator | 단계별 계획 생성 후 시작 알림 발송 |
| 구현 | codex-implementation-agent | Claude Code `--print` 기반 구현 실행 |
| 리뷰 | Reviewer + Guardian | 코드 리뷰와 6계층 보안 검증 |
| 리뷰 수정 | codex-implementation-agent | 리뷰/보안 실패 메시지를 포함해 수정 재실행 |
| 테스트 | Builder + test-runner | 빌드 체크와 하드 테스트 실행 |
...
2. 완료 후 아카이브 이동: 처리 성공 문서를 `docs/archive/codex-completed/`로 이동하되, 현재는 재처리/감사 편의를 위해 원본을 보존한다.
...
- AI coding agent PR failure study: https://arxiv.org/abs/2601.15195
