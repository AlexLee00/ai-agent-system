...
```
팀장: 다윈 (Darwin) — 연구 총괄, 사이클 오케스트레이션
  역할: 연구 주제 선정, 서칭 에이전트 배정, 실험 계획 수립, 결과 평가, 전체 팀 배포 결정
  모델: claude-code/sonnet 또는 anthropic (고품질 판단 필요)

에이전트 1~8: 서칭 에이전트 — 분야별 전문 서칭 에이전트 그룹

  원칙: 서칭 에이전트는 분야별 서칭만 담당한다.
        서칭 결과는 다윈이 판단하여 적절한 팀에 배분한다.
        하나의 서칭 결과가 여러 팀에 동시 적용될 수 있다.
        서칭 에이전트는 특정 팀에 종속되지 않는다.

  뉴런 (Neuron) — AI/멀티에이전트 기술 서칭
  ...
  서칭 에이전트 공통 패턴:
    ...
    빈도: 각 서칭 에이전트 매일 1회 실행 (분야별 시차 배치)
    기존 자산: 아처(archer)의 GitHub/npm 수집 패턴을 뉴런/인프라가 재활용

### 서칭 방법론 상세

```
뉴런: arXiv API 일일 신규 논문 스캔 → 초록+결론 → 핵심 기법 추출
      GitHub trending/weekly (stars>100) → README → 아키텍처 패턴
      적용 가능성 = 구현 난이도(1~5) × 예상 효과(1~5) = 종합(1~10)

골드: arXiv q-fin 일일 + SSRN 주간 → 수익률/샤프비율/최대낙폭 추출
      TradingView 전략 인기순 → VectorBT 백테스트 가능성 평가
      바이낸스/업비트 공지 모니터링 (규제, 상장, 수수료)

잉크: Google Search Central + Moz/Ahrefs 주간 → 알고리즘 변화 영향
      네이버 검색 알고리즘 변화 + AI 탐지 도구 업데이트 추적
      경쟁 블로그 상위 노출 패턴 (구조, 키워드 밀도, 글자 수)

가벨: 대법원 신규 판례 주간 (SW, 저작권, 프로그램)
      한국소프트웨어감정평가학회 학술지/세미나 + 해외 소송 추적
      판례 쟁점 추출 → 판결 근거 → 감정팀 적용 시사점

매트릭스: arXiv stat.ML/cs.DB 일일 + Databricks/Langfuse/Grafana 릴리즈
          새 분석 기법의 팀 제이 규모(수천~수만 건) 적용 가능성

프레임: GitHub AI 영상 도구 + ffmpeg/yt-dlp + CapCut/Twick SDK 변경
        에디팀 파이프라인 통합 가능성 + 품질 개선 효과

기어: MLX 신규 모델 + Homebrew 보안 + Apple macOS + CVE 스캔
      새 모델 vs qwen2.5-7b 성능 비교 + 보안 긴급도 판단

펄스: 네이버 일일 인기 키워드 + Google Trends 주간 급상승
      경쟁 블로그 분석 + 애드센스/제휴 수익 최적화 커뮤니티
      다음 주 주제 추천 + 바이럴 패턴(상위 10% 공통점) 추출
```

에이전트 2: 에디슨 (Edison) — 코드 구현/실험 전문
  역할: 서칭 에이전트가 발굴한 기법을 코드로 구현, 벤치마크 실행
  모델: claude-code/sonnet (코드 구현에 최적)
  입력: 연구 리포트 + 구현 스펙
  출력: 실험 코드 + 벤치마크 결과 + 성공/실패 판정
  작업 디렉토리: bots/research/experiments/

에이전트 3: 프루프 (Proof) — 코드 테스트 전문
  역할: 에디슨이 구현한 코드가 "올바르게 동작하는가" 확인
...
에이전트 4: 검증자 (Verifier) — 품질/안전/성능 검증 전문
  ...
  프루프 vs 검증자 차이:
    프루프: 코드가 작동하는가? (기술적 정확성)
    검증자: 적용할 가치가 있는가? (비즈니스 판단)

에이전트 5: 그래프트 (Graft) — 에이전트 반영 전문
...
에이전트 6: 메딕 (Medic) — 저성과 에이전트 심층 분석
...
에이전트 7: 스칼라 (Scholar) — 심층 연구 전문
  역할: 서칭 에이전트가 발굴한 주제를 깊이 연구 + 논문 리뷰 + 적용 방안 설계
  모델: anthropic (심층 분석, 긴 컨텍스트)
  입력: 서칭 에이전트의 리포트 (상위 후보)
  출력: 심층 연구 리포트 (기법 상세, 장단점, 적용 시나리오, 리스크)
  서칭 에이전트와의 차이:
    서칭 에이전트 = 넓게 빠르게 서칭 (발굴, 매일)
    스칼라 = 깊게 느리게 분석 (심층 연구, 주제당 1~3일)
  예시:
    뉴런: "Self-Evolving Agents 서베이 발견, 적용 가능성 8/10"
    스칼라: 서베이 30페이지 분석 → "팀 제이에 적용 가능한 5가지 기법 추출
            + 기법별 구현 난이도/예상 효과/의존성 분석"
  저장: research.topics (심층 분석) + rag_research (대도서관)

에이전트 8: 멘토 (Mentor) — 에이전트 교육 전문
  역할: 저성과 에이전트 재교육 + 신규 에이전트 온보딩 + 스킬 전수
  모델: anthropic (교육적 프롬프트 생성)
  기능:
    A. 재교육 프로그램:
      → 메딕 진단 기반 맞춤 교육 과정 설계
...
│   ├── edison.js                ← 코드 구현/실험
│   ├── proof.js                    ← 코드 테스트 (문법/기능/엣지/회귀)
│   ├── verifier.js                  ← 품질/안전/성능 검증 (배포 판단)
│   ├── graft.js                   ← 에이전트 반영
│   ├── medic.js                 ← 저성과 분석
│   ├── scholar.js                ← 심층 연구
│   ├── mentor.js                     ← 에이전트 교육/재교육/스킬 전수
...
[Phase 3.5: 테스트] 프루프
  ...
  → 테스트 실패 시 에디슨에게 반려 + 버그 목록 (최대 2회 재시도)
  → 테스트 통과 시 검증자에게 전달
  ↓
[Phase 3.7: 검증] 검증자
  ...
  → 검증 통과 시 그래프트에게 전달
  → 검증 실패 시 다윈에게 "성과 부족/안전 위험" 보고
  ↓
[Phase 4: 적용] 그래프트 (테스트+검증 모두 통과 시에만)
  → 대상 에이전트 프롬프트/설정 수정안 작성
  → research.agent_improvements INSERT (status=pending)
  → 다윈 승인 → 적용 (status=applied)
  → 필드 투입 → 성과 모니터링 시작
  ↓
[Phase 5: 피드백] 다윈
...
```
연구팀 → 블로팀:
  서칭 에이전트가 블로그 SEO 트렌드 서칭 → 블로팀 작가 프롬프트에 반영
  메딕가 포스 품질 하락 감지 → 진단 → 그래프트가 프롬프트 수정

연구팀 → 루나팀:
  스카우트가 트레이딩 전략 논문 서칭 → Chronos 전략에 반영
  에디슨이 백테스트 실험 → 성공 시 루나팀에 새 전략 추가
...
⑤ 토큰 예산 관리
  → 일일 연구 사이클 토큰 상한 설정
  → 서칭 에이전트: 로컬 LLM 우선 (qwen2.5-7b)
  → 다윈 판단: anthropic (고품질 필요 시만)
```
