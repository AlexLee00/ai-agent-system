...
에이전트 3: 테스터 (Tester) — 테스트 + 검증 전문
  역할: 랩러너가 구현한 코드의 정확성/안전성/성능을 독립적으로 검증
  모델: claude-code/sonnet (코드 검증에 최적)
  입력: 실험 코드 + 벤치마크 결과 + 실험 가설
  출력: 검증 리포트 (테스트 통과율, 엣지케이스, 안전성, 재현성)
  검증 항목:
    → 문법 검증: node --check, lint
    → 기능 검증: 실험 가설 대비 실제 결과 일치 여부
    → 안전 검증: OPS 영향 없는지, 데이터 손상 없는지
    → 성능 검증: before/after 벤치마크 재현 + 통계적 유의성
    → 회귀 검증: 기존 기능 훼손 없는지
  원칙: 랩러너와 독립적 판단 (같은 에이전트가 만들고 검증하지 않음)

에이전트 4: 어댑터 (Adapter) — 에이전트 반영 전문
...
에이전트 5: 앰뷸런스 (Ambulance) — 저성과 에이전트 심층 분석
...
[Phase 3: 실험] 랩러너
  ...
  ↓
[Phase 3.5: 검증] 테스터
  → 랩러너 코드 독립 검증 (문법/기능/안전/성능/회귀)
  → 검증 통과 시 어댑터에게 전달
  → 검증 실패 시 랩러너에게 반려 + 실패 원인 전달 (최대 2회 재시도)
  → 2회 실패 시 프로페서에게 "실험 실패" 보고
  ↓
[Phase 4: 적용] 어댑터 (검증 통과 시에만)
