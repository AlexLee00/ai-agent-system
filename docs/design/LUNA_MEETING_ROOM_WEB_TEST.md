...
| W-04 | 회의 시작(정상) | 타입 선택→시작 | 진행 상태 폴링→완료 후 실제 세션 타임라인으로 전환 | [자동] start API+completed run 전환 |
...
| W-12 | evidence 펼침 | `근거 JSON 보기` 클릭 | JSON `<pre>` 표시(이건 의도 — 머신리더블 보존) | [자동] 라벨+[수동] 펼침 |
...
| W-24 | LLM 발언 품질 | 번역투("하트")·동일 문단 반복 없음·status 값 halt/reduced/full 원문 유지 | [자동] 반복 표시 축약+status 복원+[수동] 🔁 다음 실회의 |

## C. 화면② 에이전트 질의
| ID | 시나리오 | 기대 결과 | 커버 |
|---|---|---|---|
| W-30 | @멘션 질의 | 에이전트 선택+질문→응답 스레드(advisory 라벨·provider 표기·호출 비용/한도 안내) | [자동] 안전 안내+[수동] ✅UI 기본 상태·실호출 보류 |
