...
작성일: 2026-04-25
상태: 설계 검토안
범위: Hub/OpenClaw 완전 분리, Telegram topic 기반 운영 채널, LLM chat 기반 자율 제어면

## 1. 결론

Team Jay는 OpenClaw를 "런타임 의존성"이 아니라 "참고 구현"으로만 취급해야 한다. 우리가 가져올 것은 OAuth/profile/CLI adapter 패턴이지, OpenClaw process, workspace, webhook, auth profile store 그 자체가 아니다.
...
1. **OpenClaw 분리**: `openclaw-client`, `.openclaw/workspace`, port `18789`, OpenClaw auth profile store는 모두 compatibility layer로 낮춘다.
...
OpenClaw 공식 OAuth 문서는 token/profile 저장을 per-agent `~/.openclaw/agents/<agentId>/agent/auth-profiles.json`에 둔다고 설명한다. 또한 legacy OAuth 파일을 import-only로 유지한다. 참고: [OpenClaw OAuth docs](https://github.com/openclaw/openclaw/blob/main/docs/concepts/oauth.md).

이 구조에서 배울 점은 **provider별 token profile과 runtime profile을 분리하는 패턴**이다. 반대로 Team Jay가 피해야 할 점은 OpenClaw state dir, gateway, hook, auth profile 파일에 직접 의존하는 것이다.
...
- Claude Code CLI/설정 재사용은 Hub-owned adapter로 격리하고 OpenClaw auth profile은 읽지 않는다.
...
이번 설계는 OpenClaw만 참고하지 않고, 현재 커뮤니티에서 실제로 쓰이는 agent workflow 레포의 공통 패턴도 반영한다.
...
요약하면, Team Jay의 목표는 "OpenClaw를 다시 만드는 것"이 아니라 **gstack의 delivery discipline + OpenClaude의 provider/tool abstraction + Hermes의 gateway/subagent/memory 구조 + Hub-owned policy/executor**를 합친 운영 제어면이다.
...
- `hub-alarm-client`가 OpenClaw webhook보다 우선되는 구조로 이동했다.
...
- `packages/core/lib/openclaw-client.ts`: 현재는 shim이지만 여전히 많은 팀 import가 남아 있다.
- `packages/core/lib/openclaw-client.legacy.js`: legacy loader.
- `packages/core/lib/telegram-sender.ts`: pending queue가 `~/.openclaw/workspace`에 남는다.
- `bots/orchestrator/src/mainbot.ts`: retired compatibility shim이다. 실제 runtime은 `jay-runtime.ts`로 이동했다.
- `bots/orchestrator/src/dashboard.ts`: `ai.openclaw.gateway`를 핵심 서비스로 표시한다.
- `bots/orchestrator/lib/steward/openclaw-session-manager.ts`: OpenClaw session repair가 steward 루프 일부다.
- `packages/core/lib/hub-client.ts`: secret category에 `openclaw`가 남아 있다.
...
| 경로 | 현재 역할 | 문제 |
| --- | --- | --- |
| `packages/core/lib/telegram-sender.ts` | 모든 팀의 topic 발송, 2초 batch, rate-limit retry | batch window가 짧고 사람/agent/audit 트래픽을 구분하지 않는다. pending queue도 아직 OpenClaw workspace 계열이다. |
...
| team-specific `publishAlert`/`publishToMainBot` alias | 팀별 알림 호환 계층 | 이름과 정책이 legacy mainbot/OpenClaw 관성을 유지한다. |
...
- OpenClaw webhook fallback을 사람 알림의 필수 경로로 두지 않는다.
...
- OpenClaw gateway down 상태에서도 alarm intake, agent 협의, digest, approval, emergency가 동작한다.
...
## 5. OpenClaw 완전 분리 로드맵

### Phase A: compatibility surface 축소

- `openclaw-client` import를 `hub-alarm-client`로 팀별 이관한다.
- `OPENCLAW_*` env는 `HUB_*` 이름을 우선하고 legacy fallback으로만 유지한다.
- `.openclaw/workspace` pending queue/lock을 `bots/hub/runtime` 또는 `/tmp/team-jay`로 이동한다.
- dashboard에서 `ai.openclaw.gateway`를 required core service에서 제거한다.

완료 조건:

- `legacy_openclaw_compat` 185 -> 50 이하.
- OpenClaw launchd down 상태에서 `/hub/health`, Telegram alarm, LLM stable path 정상.
...
- `/hub/oauth/openai/status`, `/hub/oauth/claude/status`가 OpenClaw 없이 동작.
- OpenClaw auth profile 파일 삭제/이동 후에도 LLM canary 통과.
...
### Phase E: OpenClaw 제거 완료

- `openclaw-client` shim은 compile-time compatibility만 남기고 network fallback은 기본 비활성화한다.
- `OPENCLAW_*` env fallback은 migration window 이후 제거한다.
- `.openclaw/workspace` runtime path, lock, queue, logs 참조를 Hub/Jay runtime path로 모두 이동한다.
- `ai.openclaw.gateway` launchd와 health dependency를 운영 필수 목록에서 제거한다.
- OpenClaw auth profile import는 one-shot migration tool로만 남기고 runtime read를 금지한다.

완료 조건:

- `rg "openclaw|OpenClaw|\\.openclaw"` 결과가 문서/마이그레이션/compat shim 외에는 0건이다.
- OpenClaw process, port `18789`, auth profile store가 없어도 Hub/Jay/Telegram/LLM smoke가 통과한다.
...
1. **OpenClaw runtime path 제거 1차**
   - `telegram-sender.ts`의 pending queue path를 `.openclaw/workspace`에서 Hub runtime path로 이동.
   - `jay-runtime.ts` lock path는 Hub/Jay runtime path를 사용한다.
   - 호환 fallback migration smoke 추가.

2. **Orchestrator dashboard 정리**
   - `ai.openclaw.gateway`를 core required에서 제거하고 `legacy compatibility` 섹션으로 이동.
...
8. **L5 acceptance smoke**
   - OpenClaw gateway stopped 또는 port 18789 unavailable 상태에서 Hub health/alarm/control plan smoke 통과.
...
| OpenClaw 제거 중 기존 팀 알림 깨짐 | compatibility shim 유지, 팀별 이관, inventory 숫자 gate |
...
- OpenClaw binary/path/port 없이 core path가 동작하는가?
...
- L5 smoke가 "OpenClaw off", "Hub restart", "Telegram callback retry", "provider 429"을 포함하는가?
