...
에이전트 1: 드래프터 (Drafter) — 사건분석 + 감정요청 분석 + 문서 작성 전문
  역할: 감정 촉탁서 분석, 사건/감정소요 분석, 감정계획서/질의서/감정보고서 작성
...
에이전트 2: 코드아이 (CodeEye) — 소스코드 분석 전문
...
에이전트 3: 케이스헌터-국내 (CaseHunter-KR) — 국내 판례 서칭 전문
...
에이전트 4: 케이스헌터-해외 (CaseHunter-Intl) — 해외 판례 서칭 전문
...
에이전트 5: 드래프터 (Drafter) — 감정서 초안 작성 전문
  역할: 분석 결과를 법원 제출용 감정서 형식으로 작성
  모델: anthropic (법률 문서 작성, 정확한 용어)
  입력: 코드아이 분석 리포트 + 케이스헌터 국내/해외 판례 리포트
  출력: 감정서 초안 (법원 양식 준수)
  감정서 구조:
    1) 감정 개요 (사건번호, 당사자, 감정사항)
    2) 분석 방법론 (사용 도구, 분석 기준)
...
│   ├── drafter.js                   ← 사건분석 + 감정소요 + 문서 작성
│   ├── code-eye.js                  ← 소스코드 분석
│   ├── case-hunter-kr.js            ← 국내 판례 서칭
│   ├── case-hunter-intl.js          ← 해외 판례 서칭
│   ├── reviewer.js                  ← 품질 검증
...
[11단계: 현장실사] 코드아이 + 드래프터
  ...
  → 각 기능별 판정:
    ...
    ⚠️ 부분가동: 일부 기능만 작동
    ❌ 불가동: 작동하지 않음
  → 코드아이: 소스코드 유사도/구조 분석 (필요 시)
  → 기능 분류표 + 가동 판정표 생성
  ↓
[12단계: 감정보고서 작성] 드래프터 + 리뷰어 + 저지
  → 드래프터: 전체 프로세스 결과 종합 → 감정보고서 초안
  → 리뷰어: 논리/법률/증거/중립성 검증
  → 저지: 최종 검토
  → 마스터: 최종 수정 + 서명 → 법원 제출
  ↓
