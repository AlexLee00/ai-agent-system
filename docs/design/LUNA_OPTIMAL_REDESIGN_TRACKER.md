...
- **P1-6** ✅ **next-bar 백테스트 shadow**(2026-06-11, 코드 구현·검증): `LUNA_BT_NEXT_BAR_EXECUTION_ENABLED=false` 기본 OFF·`run_backtest()` 단일 지점에서 마스크 1봉 시프트·로컬 vectorbt `price=open` 지원 확인(`next_open`)·비교 스모크 추가·레지스트리 소스 30종. live DB seed 적용은 마스터 별도 실행 대기.
