...
- **[2026-06-25 갱신] DSR→PSR 게이트 전환**: 실매매 462건(crypto, +$2,988) AUC 실증 결과 **DSR=0.474(변별력 없음)·PSR=0.659**. ⚠️**[2026-06-26 주의]** 이 462건 손익(+$2,988)은 C7-9 손익회계 버그로 **오염됨**(crypto 16건 +$1,816 가짜수익). 따라서 **AUC 0.659 수치는 신뢰 불가 — 데이터 보정 후 재산출 필요**. 단 DSR 폐기 근거(0.474)는 데이터 오염이 아닌 공식 구조 문제(sr0 폭발+raw bar 증폭)라 견고하며, DSR→PSR 전환 결정 자체는 유효(PSR은 sim 백테스트서도 변별력 확인). 상세: `docs/session/LUNA_PNL_ACCOUNTING_BUG_2026-06-26.md`. DSR이 crypto 5분봉서 구조적 통과불가(sr0 폭발+T=34560 raw bar 186배 증폭→전종목 0). 3수정안 시뮬레이션 모두 0.9 통과 실패 → DSR 폐기, PSR 게이트로 대체(`LUNA_DSR_GATE_ENABLED=false`·`LUNA_PSR_GATE_ENABLED=true`·`LUNA_PSR_MIN=0.5`). `candidate-backtest-gate.ts`+refresh 양쪽 PSR 게이트 병렬 구현. 진입 게이트가 DB psr 값을 실시간 재평가(entry-trigger-engine L152). 상세: TRACKER C7-4.
- **[2026-06-25 신설] 게이트 결정 로깅**: `investment.gate_decision_log`(진입 시점 게이트 판정+지표 스냅샷+actually_fired 기록, 비차단). 로깅 지점은 실제 PSR 게이트가 적용되는 **핫패스**(`execution-guards.ts` runBuySafetyGuards — 모든 BUY 주문 통과)이며, 공용 모듈 `gate-decision-logger.ts`로 분리. 미래에 v_trades_real_usd와 조인하여 PSR 문턱 최적성 실증. **로깅은 `await`로 호출**(단명 프로세스 process.exit 전 INSERT 완료 보장 — 2026-06-25 유실 수정). 상세: TRACKER C7-5·C7-6·C7-7.
...
## 8. 현황 정밀 분석 — 요약 (2026-06-19~21) [상세 아카이브 분리]

> 상세 32개 하위섹션(8-1~8-32)은 `docs/design/archive/LUNA_REDESIGN_SECTION8_ANALYSIS_2026-06.md`로 분리. 핵심 결론만 아래 요약. 추가 맥락은 §9(시스템 프로세스+개선 종합) 참조.

### 8-요약-1. 구현 상태
- **완료·가동**: P0 6/6, P1 전부(C1 게이트·C2 레짐·C3 전략군·C4 사전게이트·C15 레지스트리·C17 파라미터스토어), 회의실, ALPHA, P2 C7, TOSS A~D, C8 실가동.
- **데이터 대기**: C5 융합, C8 30거래, ALPHA R5/R6, ET-B/D, promotion.

### 8-요약-2. 핵심 진단 (6/19~21)
- **데이터 병목**: "데이터 부족"은 시간 문제가 아니라 구조적 병목. 생성 데이터 vs 실거래 데이터가 두 별개 체계로 정렬 0이었음.
- **암호화폐 손실 근본원인**: TP/SL 청산이 무조건 학습에서 제외되어(67건) 피드백 루프 단절. fill-resolver 비활성화(plist drift 재배포 회귀)로 체결 검증 부재.
- **fill-resolver 복구 종결**(8-22~8-28): 정확성 100% 입증 → 가드 구현(64b8a4f8e·22786ac4a) → Phase B 백필 apply 완료. micro entry=dust position 규명, 학습 18건 정정.
- **병목 3계층**(8-31): 데이터 누적·프로세스·구조. sizing 단일화 완료(8-32).

### 8-요약-3. 핵심 교훈
- 손익 데이터 정합성이 피드백 루프의 전제. 체결 검증(fill-resolver) 없으면 가짜 손익이 학습을 오염.
- plist drift(재배포 회귀)가 반복 위험 → branch-guard로 방어.
- 이 교훈들이 6/26 C7-9~C7-13(손익버그·서킷마비·재설계)으로 이어짐 → §9 참조.

---

## §9. 시스템 전체 프로세스 + 2026-06 분석·개선 종합 [메티 2026-06-26]

> 목적: §3 컴포넌트(C1~C18)와 §8 현황분석(8-1~8-32)이 개별·시간순으로 흩어져 있어, 시스템 end-to-end 흐름과 최근 개선을 한눈에 보는 상위 요약을 제공. 신규 합류자·차기 세션의 진입점.

### 9-1. 루나 시스템 end-to-end 프로세스 (거래 생애주기)

```
[데이터] universe-refresh → 시세/캔들/펀더멘털(OpenDART)/FX
   ↓
[① 레짐 판정] C2 레짐 엔진 (HMM + Markov 전이행렬 B-10) → bull/bear/sideways
   ↓
[② 전략 신호 생성] C3 전략군 룰셋 (turtle/testah) → luna_strategy_signals
   │  · ai.luna.market-gate-30min (30분) → runtime-luna-market-gate.ts
   │  · ★서킷 브레이커(luna-loss-circuit.ts)가 여기서 잠금 판정 → 잠긴 심볼 신호 차단
   ↓
[③ 시장 배치 게이트] C1 (V-B 모델) → 시장별 진입 허용/차단
   ↓
[④ 사전 게이트] C4 (luna-entry-preflight-gate.ts) → preflight_log
   │  · G-rr(RR≥2.0), G-E(샘플), G-liquidity, G-sideways 등 다중 게이트
   │  · pass/pass_with_skips/block 판정
   ↓
[⑤ 스코어 융합] C5 fuseSignals (다중 분석가 confluence) → 진입 결정
   ↓
[⑥ 진입 실행] 체결 7단계 (C16 포지션 런타임): 신호→승인→주문→체결→fill-resolver 검증→journal open
   │  · crypto: binance / 국내·해외: KIS(hanul.ts)
   │  · 토스: 데이터·미러 채널(canTrade:false)
   ↓
[⑦ 청산]
   │  · crypto: getExitDecisions(LLM) → signal_reverse(최고 청산 88% 승률)/TP/SL
   │  · 국내·해외: 현재 시간초과 손실떨이(11% 승률)만 — signal_reverse 이식 예정(Phase 2)
   ↓
[⑧ 피드백 루프] C8 → trade_journal → 학습/스코어러
   │  · ★서킷 브레이커가 이 trade_journal을 읽어 누적 R 계산 → ②로 환류
   │  · pgvector RAG → 시그마 → 전략 진화(다윈)
```

**핵심 환류 구조**: ⑧ trade_journal → ② 서킷 브레이커 → 신호 차단. 이 환류가 C7-9/C7-10 문제의 핵심 — 손익 데이터가 왜곡되거나 서킷이 과민하면 ②에서 신호 생성이 막혀 전체 파이프라인이 마비됨.

### 9-2. PROTECTED launchd (무중단 핵심)
`ai.{ska,luna,investment,claude,elixir,hub}.*` 11개 — crypto LIVE 무중단·스카 실매출 무중단. 직접 중지 금지. market-gate-30min·marketdata-mcp·registry-evaluator-daily 등 포함.

### 9-3. 주요 데이터 테이블
- `luna_strategy_signals`: 전략 신호 (② 산출)
- `luna_entry_preflight_log`: 사전 게이트 결과 (④)
- `trade_journal`: 거래 원장 (⑥⑦⑧, 손익 SSOT)
- `luna_circuit_locks`: 서킷 잠금 (②)
- `luna_toss_paper_mirror_log`: 토스 미러 shadow
- `gate_decision_log`: 게이트 결정 로깅

### 9-4. 2026-06-26 분석·개선 종합 (C7-9 ~ C7-13)

> 마스터의 "암호화폐 실계좌는 마이너스인데 데이터는 수익" 직관 → 토스 shadow 점검에서 출발해 시스템 전체를 관통하는 근본 문제를 규명하고 개선까지 연결한 작업 사이클. TRACKER C7-9~C7-13 상세.

**발견 체인 (토스 직관 → 근본 원인):**
```
손익 회계 버그(C7-9, Layer1~3)
  → trade_journal 손익 왜곡 (binance 부분체결 delta가 entry_value≈0으로 기록 → 가짜 pnl)
  → market-gate 서킷 브레이커가 trade_journal로 누적 R 계산
  → 서킷 과민(min_sample 부재)으로 17개 심볼 LOCKED (C7-10)
  → 전략 신호 생성 차단 → luna_strategy_signals 고갈(국내 1·overseas 0)
  → 국내 preflight 전부 G-rr block → 국내 거래 3주 중단 + 토스 미러 0건
```

**핵심 분석 (3대 진단):**
- **C7-9 손익 버그**: 3 레이어 (Layer1 binance 부분체결 delta, Layer2 outlier, Layer3 KIS KRW/USD 혼재). Layer1 재발방지 SHIPPED(172d522f7), 백필 대기.
- **C7-10 서킷 마비**: 손익 버그가 데이터를 넘어 시스템 제어흐름(서킷·게이트·학습) 오염. 국내 트레이딩 마비의 인과 규명.
- **C7-12 진단 수정**: 서킷 잠금 17개 ⋈ 오염 11종 교집합 = **단 1개(MEGA)**. 즉 **오염은 잠금의 주원인 아님(94%가 오염 무관)**, 진짜 원인은 **서킷 과민 자체**. → 데이터 보정만으로 마비 안 풀림, 서킷 재설계가 주 레버.

**개선 (프로세스+데이터+영상 3분석 종합 — C7-13):**
- **코드 결함 규명**(프로세스): `luna-loss-circuit.ts` buildLowProfitLocks가 sample 1개여도 cumR<0이면 잠금(최소표본 부재). 같은 파일 stoploss_guard는 4건 요구하나 low_profit만 기준 없음.
- **영상 원칙**(19편 분석): V1-C(충분한 표본)·V4-C(노이즈 vs 의도적 차단)·V5-F(평활화)가 "서킷 과민이 트레이딩 원칙 위반"임을 입증.
- **데이터 시뮬레이션**: 현재 17개 잠금에 재설계안 대입 → **min_sample=3이면 16개 해제(노이즈성), MEGA 1개만 유지(정당)**. min_sample=5는 과함.
- **해법 확정**: buildLowProfitLocks에 `if (rValues.length < 3) continue` 1줄 + 파라미터 c4.low_profit_min_sample=3. 코덱스 구현 중.

**영상 분석 자산 (19편 → 단계적 로드맵):**
- 1차 13편(B-01~B-20) + 2차 6편(V1~V6, 39항목) → 6개 클러스터(리스크/서킷·진입/청산·검증·레짐·지표·인프라).
- 단계적 로드맵 Phase 0~5. 상세: `docs/design/LUNA_VIDEO_REINFORCEMENT_SYNTHESIS.md`.

### 9-5. 개선 로드맵 현황 (Phase별)

| Phase | 주제 | 상태 | 핵심 |
|---|---|---|---|
| **1** | 서킷 재설계 (min_sample=3) | **코덱스 구현 중** | 마비 해소 주 레버, 16/17 잠금 해제 |
| **2** | KIS signal_reverse 이식 | 명세 완료 | 매도 개선 (시간떨이→능동청산) |
| **0** | Layer1 데이터 보정 | 백필 대기 | expectancy/PSR 정확성 (병행) |
| **3** | 진입 RR 개선 (retest) | 설계 예정 | G-rr 통과율 보조 |
| **4** | 검증/레짐/지표 | 설계 예정 | PSR recency·B-18·POC |
| **5** | 운영 인프라 | 설계 예정 | 통합알림·단일변수 규율 |

**양대 축**: 서킷 재설계(국내 마비 해소) + signal_reverse 이식(매도 개선). 데이터 보정은 정확성 보완으로 병행.

**검증 기준 (Phase 1 완료 시)**: market-gate 재실행 → circuitLocks 17→1 + luna_strategy_signals 증가(신호 생성 재개) + 국내 preflight pass 발생 + 토스 미러 후보 생성.
