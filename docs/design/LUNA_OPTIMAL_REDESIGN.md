...
- **[v1.1] 손실 빈도 서킷 3종**(freqtrade protections 검증 패턴, E-4): ①**StoplossGuard형** — lookback L분 내 손절·강제청산 N회 → T분 진입 잠금(**전역/심볼/사이드 3레벨**, 초기값 L=1440·N=4·T=당일) ②**심볼 쿨다운** — 청산 직후 동일 심볼 재진입 금지(초기 2캔들) ③**저수익 심볼 잠금** — 기간 내 누적 손익 음수 심볼 잠금. 기존 회로차단기(B-13·WS-I)와 통합, 파라미터=스토어(tier=approve). V-M 3계층 서킷(거래당 1%·일일 −5%·빈도)의 일반화.
