...
The report also includes `details.promotionEvidence.observed`,
`details.promotionEvidence.requirements`, and
`details.promotionEvidence.remainingForProductionCertified`. Observed readiness
is derived read-only from Stage B request-log evidence, current Hub startup
uptime, and L5 readiness reports. Observed readiness is informational only; the
...
the top-level summary for fast status rendering and the nested evidence block
for full audit detail.

When latency is above the production target, the Stage D report carries the
Stage B provider latency and slow route hotspot lists in
`details.promotionEvidence.observed.slowRoutes` and the latency requirement
evidence. Use those read-only hotspots to decide whether to tune routing,
isolate long-running workloads, or run approved load evidence before attesting
`HUB_STAGE_D_LATENCY_LT_500MS=true`.

Stage D also requires fresh Stage B/C dependency reports. Reports older than
`HUB_STAGE_D_DEPENDENCY_REPORT_MAX_AGE_HOURS` (default: 24h) are treated as
stale evidence and block `codeComplete`.

The self-healing canary uses the non-protected `ai.hub.llm-tier-probe`
LaunchAgent. If the label is not loaded, the apply command returns
`canary_label_not_loaded` with the exact bootstrap command. The tier-probe runner
ignores `__SET_IN_LOCAL_LAUNCHAGENT__` placeholder tokens and falls back to
`launchctl getenv HUB_AUTH_TOKEN` and then the Hub secrets store, so the
repository plist can stay secret-free.
...
npm --prefix bots/hub run -s hub:stage-d-report -- --json --write
```

For the normal dashboard refresh path, use the single refresh command below. It
updates Stage B/C dependency evidence first, then writes the Stage D promotion
report with the refreshed dependency status.

```bash
npm --prefix bots/hub run -s hub:stage-reports-refresh
```
