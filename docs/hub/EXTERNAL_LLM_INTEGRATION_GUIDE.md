...
외부 프로젝트는 Hub를 단일 LLM gateway로 사용한다. 외부 프로젝트는 provider 토큰을 직접 보유하지 않고, Hub `/hub/llm/call`, `/hub/llm/jobs`, `/hub/llm/vision`, `/hub/llm/embeddings`만 호출한다. 모델 선택, fallback, 비용 로그, BillingGuard, provider circuit은 Hub가 담당한다.

## 0. 현재 운영 기준

2026-07-19 기준 운영 Hub 연결 상태는 다음 정책을 따른다. (provider admission, total deadline, 구조화 backpressure, Gemini retirement 패치 반영)
...
- Gemini 정책: provider 영구 폐기. `HUB_LLM_GEMINI_DISABLED=true`는 선언/관측용(`declaration_only`)이며 런타임 스위치가 아니다.
- Gemini 재활성화: 환경변수나 운영 명령으로 불가. 공통 폐기 정책과 회귀 테스트를 함께 바꾸는 검토된 코드 변경만 허용한다(`code_change_only`).
- Gemini 직접 호출/토큰 refresh/Code Assist 점검: 항상 네트워크 호출 전에 `gemini_provider_disabled`로 종료하거나 retired skip으로 기록한다.
...
- Gemini route는 selector/caller/direct OAuth 경로에서 항상 제거되며, 외부 프로젝트는 Gemini provider를 기대값으로 고정하지 않는다. `HUB_LLM_GEMINI_DISABLED=false`나 allow 계열 환경변수도 폐기 상태를 해제하지 못한다.
...
Gemini 폐기 상태에서 정상 응답의 `provider` 또는 `selected_route`에 `gemini-*`가 나오면 가이드/운영 불일치로 본다.
...
- `gemini-oauth`, `gemini-cli-oauth`, `gemini-codeassist-oauth`는 실행 전 항상 제거된다.
- Gemini만 남은 selector는 `gemini_provider_disabled`로 실패한다.
- Gemini token refresh/monitor/Code Assist 점검은 retired 상태로 skip되어 불필요한 인증 경고를 만들지 않는다.
- `HUB_LLM_GEMINI_DISABLED`는 배포 상태를 드러내는 선언값일 뿐이다(`declaration_only`). false/ALLOW 환경변수로 재활성화할 수 없고, 재도입은 별도 설계·코드 리뷰·회귀 테스트를 포함한 코드 변경으로만 가능하다(`code_change_only`).
- OAuth 운영 문서 `docs/hub/OAUTH_REAUTH_GUIDE.md`도 Gemini를 재인증 절차가 아닌 폐기 상태로만 설명한다.
...
| `ok=false`, `gemini_provider_disabled` | 폐기된 Gemini-only 경로 요청 | selectorKey 또는 Hub registry를 OpenAI/Groq/Claude/Local 포함 경로로 수정 |
...
- Gemini 폐기 운영이면 `gateway-contract.providerPolicy.geminiRetired=true`, `geminiReenablePolicy=code_change_only`와 agent-level drill 결과의 Gemini 잔여 `0건`을 확인한다.
