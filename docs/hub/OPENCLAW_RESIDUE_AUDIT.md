...
- generated_at: 2026-05-02T14:37:40.974Z
- ok: true
- runtime_blocker: 0
...
- docs/history/WORK_HISTORY.md:409 (markdown documentation/report reference) — `- `REPO_ROOT=... PROJECT_ROOT=... node bots/investment/scripts/parallel-ops-report.ts --publish` → legacy-gateway/Telegram fetch 실패 및 `127.0.0.1:18789` 연결 실패`
- docs/history/WORK_HISTORY.md:464 (markdown documentation/report reference) — `- `npm run parallel-report -- --publish` → alert 경로는 호출됐지만 `legacy-gateway`/Telegram fetch 및 `127.0.0.1:18789` 연결 실패 경고 발생`
...
- bots/hub/scripts/generate-hub-alarm-inventory.ts:139 (intentional smoke/report guard) — `legacy_gateway_compat: counts.legacy_gateway_compat || 0,`
- bots/hub/scripts/generate-hub-alarm-inventory.ts:184 (intentional smoke/report guard) — `if ((payload.categories.legacy_gateway_compat || 0) > 0) {`
- bots/hub/scripts/generate-hub-alarm-inventory.ts:186 (intentional smoke/report guard) — `.filter((row) => row.category === 'legacy_gateway_compat')`
- bots/hub/scripts/generate-hub-alarm-inventory.ts:206 (intentional smoke/report guard) — `'This inventory tracks the Hub alarm migration surface. `hub_alarm_native` entries are the desired path; `retired_gateway_guard` entries are regression guards; `legacy_gateway_compat` entries are remaining migration targ`
- bots/hub/scripts/generate-hub-alarm-inventory.ts:213 (intentional smoke/report guard) — ``- legacy_gateway_compat: ${payload.categories.legacy_gateway_compat || 0}`,`
...
- bots/hub/scripts/hub-alarm-import-transition-smoke.ts:125 (intentional smoke/report guard) — `legacy_gateway_runtime_defaults: 0,`
...
- bots/hub/scripts/run-tests.ts:202 (intentional regression guard) — `'openclaw-runtime-retirement-smoke.ts',`
