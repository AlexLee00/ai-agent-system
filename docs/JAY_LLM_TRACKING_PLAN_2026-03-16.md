...
- 오케스트레이터 LLM 호출 로그:
  ...
  둘 다 기록
- 제이 전용 사용량 리포트:
  - `scripts/jay-llm-usage-report.js`
- 전체 봇 통합 사용량 리포트:
...
- 제이 전체 트래픽을 OpenAI로 옮기지 않고, 비용이 큰 자유대화는 Gemini에 남긴다.
...
```bash
node /Users/alexlee/projects/ai-agent-system/scripts/jay-llm-usage-report.js --days=14
