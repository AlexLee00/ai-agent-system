...
### 완료된 추가 분석 (2026-03-29 세션 계속)

#### bots/reservation 전체 완료
- `lib/` 나머지 17개 파일: telegram.js(Forum 라우팅+BUG-006 파일명 누출 차단), utils.js, validation.js(transformPhoneNumber/Date/Time/Room), reservation-key.js(phone|date|start|end|room v2 해시키), study-room-pricing.js(A1/A2 3500원·B 6000원/30분), health.js(3중 가동·3중 중지), error-tracker.js(연속 오류 카운터), crypto.js(AES-256-GCM+SHA256 kiosk해시), files.js, browser.js(Puppeteer 옵션), pickko-stats.js(월별/일별 스터디룸 분리), mode.js(DEV/OPS 분리), status.js, vip.js, mainbot-client.js
- `src/ska.js` — 5초 bot_commands 폴링, analyze_unknown 핸들러, 6시간 정체성 점검, self-lock

#### bots/blog 전체 완료
- `lib/maestro.js` — n8n 웹훅 → directRunner 폴백, sectionVariation 동적 결정(인사말 스타일/FAQ/리스트/브릿지/카페위치/보너스인사이트), 7일 이력으로 패턴 회피
- `lib/blo.js` — 강의·일반 포스팅 오케스트레이션, AI 리라이팅 가이드, 커리큘럼 일일 체크, dailyCurriculumCheck 연동
- `lib/curriculum-planner.js` — HN+GitHub 트렌드 → LLM 후보 3개 → 텔레그램 제안 → 승인 → generateCurriculum → planned→active 자동 전환
- `lib/gems-writer.js` (1027줄) — IT전략 컨설턴트 젬스, gpt-4o 기본, 6000자+, 섹션별 최소글자, 최근 14일 제목 중복 회피, AI 탐지 회피, Continue 이어쓰기, repair 2회 보정
- `lib/pos-writer.js` (632줄) — 시니어 백엔드 아키텍트 포스, 8000자+, 4청크 분할생성(chunked-llm 기반), GEO 최적화
- `lib/richer.js` — HN/Node.js GitHub/날씨 수집, RAG 실전에피소드 검색, 관련 포스팅 검색
- `lib/quality-checker.js` — AI 탐지 리스크 점수(0~100), 반복어구/종결어미/문장길이 균일도/개인경험/날씨 맥락 체크
- `lib/publ.js` — HTML 변환(섹션헤더/코드블록/해시태그), 내부링크 자리표시자 치환, DB+RAG 저장, Google Drive 복사

#### bots/worker 핵심 완료
- `src/worker-lead.js` — 텔레그램 폴링(2초), Phase2봇 위임(emily/noah/oliver), NLP 채팅 폴백, 승인 인라인 버튼 처리
- `src/task-runner.js` — PostgreSQL FOR UPDATE SKIP LOCKED 선점, 6봇 실행기(emily/noah/sophie/oliver/ryan/chloe), pg_notify 실시간 이벤트, ai-feedback 커밋
- `lib/auth.js` — bcrypt(salt 12) + JWT(HS256/24h) + KISA 비밀번호 정책(8자+3종)
- `lib/ai-policy.js` — 3개 정책 축(ui_mode/llm_mode/confirmation_mode), master/admin/member 계층, 회사별 기본값
- `lib/approval.js` — 3단계 승인(auto/admin/master), agent_tasks 상태 동기화, 텔레그램 인라인 버튼, ai-feedback 통합
- `lib/chat-agent.js` (876줄) — 채팅 세션/메시지 CRUD, 규칙 기반 인텐트 파싱, LLM 폴백, 일정 생성/조회/수정/취소, 에이전트 태스크 큐잉, NLP 자동 학습 파이프라인

#### bots/orchestrator 핵심 완료
- `src/mainbot.js` — mainbot_queue 폴링(2초), 4096자 분할, 아침 브리핑(08:00), 무음/확인 만료 정리, bot_commands 타임아웃, 6시간 팀장 정체성 점검
- `lib/intent-parser.js` (698줄) — 3단계 파싱(slash/keyword/learned/llm), 50개+ 슬래시 명령, 80개+ 키워드 패턴, Groq/OpenAI/Gemini LLM 폴백, 동적 Few-shot 예시 주입
- `src/router.js` (2812줄) — 전체 인텐트→핸들러 매핑, promoteToIntent/rollback, 팀 키워드 → 팀장 위임 → Gemini 자유대화 폴백, 건강 리포트 실행, speed test/ska forecast/luna health 직접 실행

### 미완료 파일 (다음 세션에서 이어야 함)

우선순위 높음:
- `bots/orchestrator/src/router.js` 나머지 (~1800줄 미열람): 실제 핸들러 구현부 (sendBotCommand, formatLunaResult, handleUpbitTransfer 등)
- `bots/reservation/auto/monitors/pickko-kiosk-monitor.js`
- `bots/reservation/auto/scheduled/` 전체

우선순위 중간:
- `bots/orchestrator/lib/` 나머지: night-handler.js, mute-manager.js, team-bus.js, token-tracker.js, response-cache.js 등
- `bots/worker/lib/` 나머지: ai-client.js, document-ai.js, employee-ai.js, menu-policy.js, sales-ai.js 등
- `bots/video/` 전체
- `bots/ska/src/` Python 파일 전체 (forecast.py, etl.py, rebecca.py 등)
- `packages/core/lib/` 나머지 30+개 파일 (ai-feedback-*.js, chunked-llm.js, document-parser.js, llm-cache.js 등)
...
- `runtime-config-suggestions`도 같은 구조를 따르도록 보강했다. 이제 `capitalGuardBias` 스냅샷과 `capital_management.by_exchange.binance.trade_modes.validation.max_daily_trades` 관찰 제안이 함께 출력되어, crypto 병목이 threshold보다 validation daily budget 구조 쪽이라는 점을 설정 제안 레이어에서도 직접 읽을 수 있다.
- [VALIDATION_LANE_POLICY.md](/Users/alexlee/projects/ai-agent-system/bots/investment/docs/VALIDATION_LANE_POLICY.md)에 crypto validation daily budget 기준을 추가했다. 현재 운영값은 `validation max_daily_trades=10`, `soft cap=8`, `reserve=2`이며, 최근 `capital_guard`의 `90.8%`가 validation에 몰려 있어도 즉시 상향하지 않고 `observe`가 기본이라는 기준선을 문서에 고정했다.
- 같은 기준선을 [health-report.js](/Users/alexlee/projects/ai-agent-system/bots/investment/scripts/health-report.js)에도 정책 판단 레이어로 올렸다. 새 `cryptoValidationBudgetPolicyHealth` 섹션이 현재 `정책 분리 검토`를 출력하며, 근거는 `soft cap 차단 0건`, `validation capital guard 비중 90.8%`, `LIVE gate blocked`, `closed review 1건 / weak 50건`이다. 즉 지금은 총량 상향보다 validation 전용 budget 구조 분리 검토가 우선이라는 운영 해석이 health reason에도 직접 반영된다.
- 같은 정책 checkpoint를 [runtime-config-suggestions.js](/Users/alexlee/projects/ai-agent-system/bots/investment/scripts/runtime-config-suggestions.js)에도 정렬했다. 이제 suggestion 리포트가 `validationBudgetPolicy` 스냅샷과 `crypto validation budget 정책 판단` 섹션을 함께 출력하며, `capital_management.by_exchange.binance.trade_modes.validation.max_daily_trades`도 단순 `observe`가 아니라 현재 판단 근거(`정책 분리 검토 / soft cap 차단 0건 / validation capital guard 90.8% / LIVE gate blocked / closed review 1건 / weak 50건`)를 직접 담는다.
- suggestion log 원장도 함께 확장했다. [bots/investment/shared/db.js](/Users/alexlee/projects/ai-agent-system/bots/investment/shared/db.js)의 `runtime_config_suggestion_log`에 `policy_snapshot JSONB`를 추가하고, `runtime-config-suggestions --write`가 `validationBudgetPolicy`, `capitalGuardBias`, `validationBudgetSnapshots`를 함께 저장하도록 연결했다. 즉 이제 validation budget 정책 판단은 화면 출력뿐 아니라 회고 가능한 운영 이력으로도 남는다.
- 같은 suggestion 리포트에 `validationBudgetPolicyTrend`도 추가했다. 직전 `runtime_config_suggestion_log.policy_snapshot`과 현재 계산 결과를 비교해 `직전 대비 판단 유지/변경`을 바로 보여주며, 현재는 `정책 분리 검토 → 정책 분리 검토`, `validation capital guard 비중 90.8% → 90.8% (+0%p)`로 안정 상태임을 출력한다.
- 2026-03-28 아침 덱스터 유지보수 리포트는 실제 서비스 장애가 아니라 `코드 무결성/체크섬 baseline`과 `의미 있는 uncommitted 14개`가 섞인 상태였다. 이번 세션에서 `bots/claude/lib/checks/code.js`가 raw `git status`에서 temp/build generated path를 제외하도록 보강했고, `node bots/claude/src/dexter.js --update-checksums`로 checksum baseline도 갱신했다. 이후 남아 있던 의미 있는 변경 묶음(archer reports/patch request/video doc/upload/input asset 등)은 사용자 지시에 따라 함께 커밋/푸시해 덱스터 false positive를 줄이는 방향으로 정리한다.
- 2026-03-28: 국내장 장전 스크리닝의 주말/공휴일 가드를 복구했다. [bots/investment/scripts/pre-market-screen.js](/Users/alexlee/projects/ai-agent-system/bots/investment/scripts/pre-market-screen.js)가 그동안 `getKisMarketStatus()` / `getKisOverseasMarketStatus()`를 전혀 사용하지 않아 토요일 아침에도 `장전 스크리닝 완료 (국내주식)` 알림을 보냈는데, 이제 `db.initSchema()` 직후 시장 상태를 조회해 `holiday.isHoliday` 또는 `isWeekend`면 저장/알림 없이 즉시 스킵한다. 즉 장전 실행은 허용하되, `장외 시간`은 허용하고 `주말/공휴일/NYSE 휴장`만 막는 경계로 정렬됐다.
- 2026-03-28: 암호화폐 `collect_overload_detected` 경고 해석을 분리했다. [bots/investment/shared/pipeline-market-runner.js](/Users/alexlee/projects/ai-agent-system/bots/investment/shared/pipeline-market-runner.js)에 `overloadProfile` 계산을 추가해 `screeningSymbolCount`, `heldAddedCount`, `perSymbolNodeCount`를 기준으로 과부하 원인을 `screening / held / mixed`로 분류한다. 현재 `tasks=61, screening=8, held=7` 같은 케이스는 `mixed`로 보고, 알림 문구도 “보유 포지션 carry 관찰”과 “동적 스크리닝”이 함께 겹친 상태인지 직접 설명한다. 즉 universe를 더 줄여야 하는지, stale/held 부담을 먼저 봐야 하는지 운영 판단 경계를 복구한 패치다.
- 2026-03-28: 루나팀 전체 재설계 로드맵 문서 [bots/investment/docs/LUNA_REDESIGN_PHASE_1_TO_5.md](/Users/alexlee/projects/ai-agent-system/bots/investment/docs/LUNA_REDESIGN_PHASE_1_TO_5.md)를 추가했다. 현재 병목, 에이전트별 역할 재정의, `n8n`/`RAG` 적용 원칙, 맥스튜디오 도입 후 백테스트·예측·검증 엔진 배치 방향, 데이터 모델 초안, Phase 1~5 구현 순서를 한 문서에 고정했다.
