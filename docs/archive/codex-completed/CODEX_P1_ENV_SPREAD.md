 ...
 * DEV: http://localhost:7788 (Tailscale 또는 SSH 터널 경유)
 * 환경변수 HUB_BASE_URL로 오버라이드 가능
 */
const HUB_BASE_URL = process.env.HUB_BASE_URL ||
  (IS_OPS ? 'http://localhost:7788' : 'http://localhost:7788');

/**
 * Hub를 통해 DB/n8n 리소스에 접근해야 하는지 여부
 * OPS: false (직접 접근 — 불필요한 HTTP 오버헤드 방지)
 * DEV: true (Hub 경유 — 로컬 DB/n8n 없으므로)
 */
const USE_HUB = IS_DEV && !!HUB_BASE_URL;

/**
 * Hub를 통해 시크릿(API 키)에 접근하는지 여부
 * OPS: true (Hub가 config.yaml의 유일한 읽기 주체)
 * DEV: true (Hub 경유)
 * → 일관성: OPS/DEV 모두 동일한 경로로 시크릿 접근
 * → Hub 실패 시 OPS는 로컬 config.yaml 폴백
 */
const USE_HUB_SECRETS = !!HUB_BASE_URL;

/**
 * Hub 인증 토큰 (Bearer Token)
 * Hub 서버(OPS)와 클라이언트(DEV) 양쪽에서 동일 값 사용
 */
const HUB_AUTH_TOKEN = process.env.HUB_AUTH_TOKEN || '';

/**
 * Hub 서버 바인드 포트 (OPS에서만 사용)
 */
...
- env.js: N8N_ENABLED / LAUNCHD_AVAILABLE / OPENCLAW_PORT /
  ...
  DEV(맥북 에어):   n8n·launchd·openclaw 비활성, Hub 경유 접근

- env.js: HUB_BASE_URL / USE_HUB / HUB_AUTH_TOKEN / HUB_PORT 추가
  OPS: Hub 서버 호스트 (포트 7788)
  DEV: Hub 클라이언트 (Tailscale/SSH 터널 경유)
