...
9. [OpenClaw Gateway](#9-openclaw-gateway)
10. [멀티에이전트 시스템용 모델 선택 가이드](#10-멀티에이전트-시스템용-모델-선택-가이드)
...
| 모델 ID | 컨텍스트 | 최대 출력 | 상태 |
|---------|---------|---------|------|
| `gemini-2.5-pro` | 1,048,576 | ~65,536 | GA |
| `gemini-2.5-flash` | 1,048,576 | ~65,535 | GA ← **권장 교체 대상** |
| `gemini-2.5-flash-lite` | 1,048,576 | ~65,535 | GA |
| `gemini-2.0-flash` | 1,048,576 | 8,192 | **⚠️ 2026-03-03 은퇴** |
...
### 2.6 gemini-2.0-flash → gemini-2.5-flash 마이그레이션
...
### 3.1 모델 목록 (Free Tier 기준, 2026-03-04)

| 모델 ID | 컨텍스트 | RPM | TPM | RPD | TPD | 비고 |
|---------|---------|-----|-----|-----|-----|------|
| `meta-llama/llama-4-scout-17b-16e-instruct` | 131K | 30 | **30,000** | 1,000 | 500K | **루나팀 현재 사용** (Preview) |
| `llama-3.1-8b-instant` | 128K | 30 | 6,000 | 14,400 | — | 최고속 (TTFT 203ms) |
| `llama-3.3-70b-versatile` | 128K | 30 | 12,000 | 1,000 | — | 고품질 분석 |
| `meta-llama/llama-4-maverick-17b-128e-instruct` | 128K | 30 | 6,000 | 500 | 500K | 400B total MoE |
| `moonshotai/kimi-k2-instruct-0905` | **262K** | **60** | 10,000 | 1,000 | 300K | 최대 컨텍스트, Scout 백업 후보 |
| `qwen/qwen3-32b` | 32K | **60** | 6,000 | 500 | 500K | RPM 높음 |
| `groq/compound-mini` | 128K | 15 | — | 250 | — | 웹검색+코드 내장 에이전트 |
...
### 8.4 sendMessageDraft (Bot API 9.5 신메서드)
