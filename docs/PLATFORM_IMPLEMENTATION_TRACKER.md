...
- 운영 중 변경 가능한 값의 외부화도 큰 축으로 올라왔다.
  ...
  - 설정 조회 스크립트
  - 운영 가이드 문서
- 알림 UX도 운영 안정화 축으로 한 단계 올라왔다.
  - 긴 구분선 2줄 문제
  - notice 헤더 중복 문제
  - 장전/장마감 투자 알림 장문 문제
  를 우선 해결했다.
- 자동화 리포트도 “출력”보다 “판단” 중심으로 보강 중이다.
  - 제이 Gateway는 스냅샷 저장 실패 상황에서도 review를 남기도록 강인성을 높였다.
  - 제이 Gateway는 `~/.openclaw/workspace` 저장 실패 시 repo 내부 fallback 파일로 기록을 이어간다.
  - 제이 LLM 일일 리뷰는 DB 접근 실패 시 `partial` 상태와 `session_usage_fallback` 모델 현황을 함께 보여준다.
  - 일일 운영 분석은 `healthError`를 함께 노출해 `hold`의 근거를 더 명확히 보여주고, `현재 활성 이슈 / 누적 반복 이슈 / 입력 실패`를 분리한다.
  - 스카/투자 리포트는 즉시 조치 문구와 비용 효율 경고를 붙여 운영 해석성을 높였다.
