...
- 워커
  ...
  - 문서 목록/상세에서 OCR 품질과 짧은 추출 텍스트를 재사용 품질 신호로 함께 보여주기
  - `/documents`에서 품질 상태와 전환율 기준으로 문서를 우선 정렬/필터링하기
  - 문서 상세에서 무수정 확정률과 평균 수정 필드 수를 재사용 효율 지표로 함께 보여주기
  - 문서 품질, 전환율, 무수정 확정률, 수정량을 묶은 종합 효율 점수로 목록/상세 우선순위를 더 선명하게 만들기
  - 효율 점수를 바탕으로 개선 우선 문서 / 템플릿 후보 / OCR 재검토 후보를 자동 리뷰로 추리기
  - 워커 모니터링의 provider 변경 이력과 호출 통계를 운영 지표로 다듬기
  - provider 변경 사유(note)와 호출 품질 해석을 더 촘촘히 연결
  - 최근 변경이 실제 성공률/응답시간 개선으로 이어졌는지 비교 가능한 추세 보기를 더 길게 확장
- 투자
  ...
  - 적용 직후에는 health와 최근 실행률을 함께 보는 검증 리포트로 효과를 재확인하기
  - `legacy_*` 실패 이력을 구조화 코드 기준으로 더 줄이기
- 스카
  - shadow 모델 비교 데이터 누적 후 `ensemble experiment` 승격 여부 판단
  - 예약 모니터와 예측 리뷰 간 피드백 루프 강화
  - 일일/주간 예측 리뷰에서 `데이터 수집 / 비교 관찰 / 앙상블 후보 / 기존 유지` 단계가 바로 보이도록 shadow 판단 레이어 유지
- 공통
