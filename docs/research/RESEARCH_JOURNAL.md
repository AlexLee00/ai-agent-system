...
### DEC-027 | 모바일 알림은 공용 렌더러에서 먼저 축약한다

문제:
- 텔레그램 알림이 모바일에서 너무 길고, 긴 구분선이 화면 폭을 넘겨 2줄로 꺾였다.
- producer가 이미 `payload.summary/details`를 주고 있는데도 원문 `message`가 다시 detail로 붙어 중복 노출이 생겼다.

이번 판단:
- 모바일 최적화는 각 팀이 개별적으로 문자열을 줄이는 것보다 `reporting-hub` 공용 notice/report 렌더러에서 먼저 처리하는 것이 맞다.
- 다만 루나처럼 raw 문자열을 직접 만드는 경로는 공용 렌더러만으로는 부족하므로, 해당 producer에서 구분선과 장문 근거를 별도로 축약한다.

지금 당장 필요한 구조:
- `payload.details`가 있으면 detail 우선 렌더링
- detail/section line 수 제한
- 긴 구분선과 연속 공백은 `telegram-sender` 발송 직전 정규화

나중에 확장할 구조:
- 모바일용 `compact` / 데스크톱용 `full` 2단계 포맷 정책
- 팀별 producer가 payload만 넘기면 채널별 renderer가 최종 길이를 결정하는 구조

의미:
- 비즈니스 관점: 모바일에서 핵심 경고를 놓치지 않게 한다.
- 운영 관점: 알림 피로를 줄이고, 첫 화면에서 요지를 바로 읽게 한다.
- SaaS 관점: 이후 고객별 채널/디바이스별 포맷 정책으로 확장하기 쉽다.

_작성: 2026-03-18_

---

### DEC-028 | 투자 설정 변경은 적용보다 관찰이 우선이다

상태:
- suggestion log 기반 `review -> apply -> validate` 흐름을 닫았고,
- 실제 운영 `config.yaml`에 `runtime_config.luna.fastPathThresholds.minCryptoConfidence = 0.44`를 반영했다.

이번 판단:
- 첫 실험 적용 직후에는 추가 threshold 조정보다 `3~7일 observe`를 우선한다.
- 이유는 원인 분리다. 한 번에 여러 키를 바꾸면 실행률 변화가 어느 설정 때문인지 해석하기 어려워진다.

지금 당장 필요한 구조:
- 단건 적용
- `applied_at` 기록
- `validate-runtime-config-apply.js`로 즉시 검증
- 이후 3~7일 관찰

나중에 확장할 구조:
- 반영 전후 지표 비교 자동화
- 반복 보류/승인 패턴 분석
- 멀티테넌트 SaaS에서 워크스페이스별 설정 실험 감사 흐름

_작성: 2026-03-18_

## 2026-07-13
- 리서치→구현 완주: Avengers-Pro(클러스터 shadow 가동·서명 축적)·FundaPod(KG 610노드+검색 통합)·SEP/FinMem(루나 Phase A 로드맵 4-N). V2 스프린트 교훈="진단은 코드에서".
