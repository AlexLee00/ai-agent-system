defmodule Jay.Core.Config do
  def db_name, do: Application.get_env(:team_jay, Jay.Core.Repo)[:database]
  def db_user, do: Application.get_env(:team_jay, Jay.Core.Repo)[:username]
  def db_pass, do: Application.get_env(:team_jay, Jay.Core.Repo)[:password]
  def db_host, do: Application.get_env(:team_jay, Jay.Core.Repo)[:hostname]
  ...
  def n8n_base_url, do: Application.get_env(:team_jay, :n8n_base_url) || launchctl_env("N8N_BASE_URL") || "http://127.0.0.1:5678"
  def local_llm_base_url, do: Application.get_env(:team_jay, :local_llm_base_url) || launchctl_env("LOCAL_LLM_BASE_URL") || launchctl_env("MLX_URL") || "http://127.0.0.1:11434"
