defmodule Jay.Core.Agents.PortAgent do
  ...
  defp open_port(%{runner: :tsx, script: script}) do
    tsx = System.find_executable("tsx") ||
          Path.join(Jay.Core.Config.repo_root(), "node_modules/.bin/tsx")
  ...
  defp port_env do
    [
      {~c"MODE", ~c"ops"},
      {~c"PROJECT_ROOT", String.to_charlist(Jay.Core.Config.repo_root())}
    ]
  end
