defmodule Jay.Core.Diagnostics do
  @moduledoc """
  ...
  @shadow_agents [
    ...
    {:blog_collect_competition, :blog},
    {:blog_weekly_evolution, :blog},
  ...
  def handle_info(:check, state) do
    ...
    if length(alerts) > 0 do
      ...
      _ = Jay.Core.HubClient.post_alarm("🔍 Elixir 진단 경고!\n#{msg}", "claude", "diagnostics")
    end
  ...
  defp check_message_queues do
    processes = [
      ...
      Jay.Core.Agents.PortAgent.via(:dexter)
    ]
  ...
  defp build_shadow_report(state) do
    ...
    agent_statuses =
      Enum.map(@shadow_agents, fn {name, team} ->
        case GenServer.whereis(Jay.Core.Agents.PortAgent.via(name)) do
          nil ->
            %{name: name, team: team, status: :missing, runs: 0, consecutive_failures: 0}

          _pid ->
            status = Jay.Core.Agents.PortAgent.get_status(name)
    ...
    %{
      ...
      summary: summary,
      recent_failures:
        Jay.Core.EventLake.get_by_type("port_agent_failed", 20)
  ...
  defp build_launchd_shadow_agents(agents) do
    Enum.map(agents, fn agent ->
      ...
      case GenServer.whereis(Jay.Core.Agents.LaunchdShadowAgent.via(name)) do
        nil ->
          Map.merge(
            %{
            name: name,
        ...
        _pid ->
          status = Jay.Core.Agents.LaunchdShadowAgent.get_status(name)
  ...
  # 메모리 총량은 GC/바이너리 힙 변동으로 수 MB씩 흔들린다. 압박 상태가 유지되는
  # 동안에는 동일 signature로 묶고, ok/warn/error 전환만 EventLake에 남긴다.
  defp memory_signature(%{severity: :ok}), do: "ok"
  defp memory_signature(%{severity: :warn}), do: "warn"
  defp memory_signature(%{severity: :error}), do: "error"
  defp memory_signature(%{severity: severity}), do: Atom.to_string(severity)
  ...
  defp maybe_alarm_shadow_report(report, "warn") do
    ...
    _ = Jay.Core.HubClient.post_alarm(String.trim(message), "claude", "diagnostics")
  end
