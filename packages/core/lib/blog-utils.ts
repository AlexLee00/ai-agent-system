...
function weatherToContext(
  weather: WeatherLike = {},
  options: WeatherContextOptions = {},
): string {
...
function estimateCost(usage: UsageLike): number {
  if (!usage) return 0;
  return ((usage.prompt_tokens || 0) * 2.5 + (usage.completion_tokens || 0) * 10) / 1_000_000;
}

function loadPersonaGuide(filename: string): string {
