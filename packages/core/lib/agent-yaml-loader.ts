...
export const REQUIRED_AGENT_YAML_FIELDS = [
  'name',
  'tier',
  'runtime',
  'owner',
...
export function validateAgentDefinition(agent = {}) {
  const missing = REQUIRED_AGENT_YAML_FIELDS.filter((field) => agent[field] == null);
  const errors = [];
...
export function listAgentDefinitions({ teamDir } = {}) {
  if (!teamDir) throw new Error('teamDir is required for core agent YAML loading');
