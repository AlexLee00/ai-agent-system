...
function findRepoRoot(startDir) {
  ...
  while (true) {
    ...
    const sourcePath = path.join(current, 'packages/core/lib/env.ts');
...
function resolveSourcePath() {
  ...
  const candidates = [
    ...
    ...(repoRoot ? [path.join(repoRoot, 'packages/core/lib/env.ts')] : []),
    path.resolve(__dirname, '../../../../packages/core/lib/env.ts'),
    path.resolve(__dirname, '../../../../../packages/core/lib/env.ts'),
    path.resolve(__dirname, '../../../../../../packages/core/lib/env.ts'),
  ];
  ...
  throw new Error(`Unable to locate env.ts runtime source (checked: ${candidates.join(', ')})`);
}
