...
function inferProvider(baseUrl, profile, opts = {}) {
...
async function ensureDrawThingsReachable(baseUrl) {
  await fetchJson(`${baseUrl}/sdapi/v1/options`);
}

async function generateWithDrawThings(prompt, baseUrl, opts = {}, profile = null) {
  const aspectRatio = opts.aspectRatio || '1:1';
  const size = ASPECT_RATIO_MAP[aspectRatio] || ASPECT_RATIO_MAP['1:1'];
...
async function generateWithComfyUI(prompt, opts = {}) {
  ...
  const baseUrl = opts.baseUrl || process.env.BLOG_IMAGE_BASE_URL || profile.base_url || DEFAULT_BASE_URL;
  ...
  for (let attempt = 1; attempt <= retries; attempt += 1) {
    ...
    try {
      if (provider === 'drawthings') {
        const drawThingsBaseUrl = opts.baseUrl || process.env.BLOG_IMAGE_BASE_URL || profile.base_url || DEFAULT_DRAWTHINGS_BASE_URL;
  ...
  throw new Error(`${providerLabel} 로컬 이미지 생성 실패: ${lastError?.message || 'unknown error'}`);
}
