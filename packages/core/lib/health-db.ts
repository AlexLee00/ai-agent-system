const { getPromotionSummary } = require('./intent-store') as {
  getPromotionSummary: (
    pgPool: { get?: (...args: unknown[]) => Promise<unknown>; query?: (...args: unknown[]) => Promise<unknown>; run?: (...args: unknown[]) => Promise<unknown> },
    options?: { schema?: string; filters?: Record<string, unknown> }
  ) => Promise<{ pending_count?: number | string | null; applied_count?: number | string | null } | null>;
};

type PgPoolLike = {
  get: (dbName: string, sql: string, params?: unknown[]) => Promise<Record<string, unknown> | null>;
};

type PromotionPendingOptions = {
  schema?: string;
...
type PendingCommandOptions = {
  ...
  toBot?: string;
  pendingWarnThreshold?: number;
  ageWarnThresholdMinutes?: number;
...
async function getPromotionPendingHealth(
  pgPool: unknown,
  {
    schema,
  ...
  }: PromotionPendingOptions = {},
): Promise<HealthResult> {
  try {
    const summary = await getPromotionSummary(pgPool as Record<string, unknown>, { schema, filters: {} });
    ...
    if (/does not exist/i.test(String((error as Error).message || ''))) {
      return {
        pendingCount: 0,
        appliedCount: 0,
        hasWarn: false,
    ...
    return {
      ...
      appliedCount: 0,
      hasWarn: true,
      detail: `  인텐트 후보 조회 실패: ${(error as Error).message}`,
      reasons: [missingReason],
    };
  }
}

async function getPendingCommandHealth(
  pgPool: PgPoolLike,
  {
    dbName,
    table = 'bot_commands',
  ...
  try {
    const row = await pgPool.get(dbName || 'claude', `
      SELECT
    ...
    return {
      ...
      oldestMinutes: 0,
      hasWarn: true,
      detail: `  pending 명령 조회 실패: ${(error as Error).message}`,
