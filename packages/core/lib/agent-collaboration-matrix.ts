...
export function buildCollaborationMatrix(agents = []) {
...
export function getCollaborationFlow(decisionType = 'discovery_entry', { agents = [] } = {}) {
  const type = COLLABORATION_FLOWS[decisionType] ? decisionType : 'discovery_entry';
  const knownAgents = new Set(agents.map((agent) => agent.name));
  const steps = COLLABORATION_FLOWS[type].map((step, index) => {
    ...
    return {
      ...
      missingAgents: [step.from, ...targets].filter((agent) => knownAgents.size > 0 && !knownAgents.has(agent)),
    };
  });
...
export async function executeCollaboration(flow, context = {}, opts = {}) {
  ...
  if (typeof opts.sendMessageFn !== 'function') throw new Error('sendMessageFn required when publish is enabled');
  const published = [];
  for (const item of publishPlan) {
    const id = await opts.sendMessageFn(item.fromAgent, item.toAgent, item.payload, {
      incidentKey: item.incidentKey,
      messageType: item.messageType,
    });
    published.push({ ...item, id });
  }
  return { ok: published.every((item) => Number(item.id) > 0), status: 'collaboration_published', dryRun, publishEnabled, incidentKey, steps: flow?.steps || [], publishPlan, published };
}
