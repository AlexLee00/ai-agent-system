...
type FeedbackSession = {
  ...
  aiInputPayload?: Record<string, unknown>;
  aiOutputType: string;
...
type FeedbackEvent = {
  feedbackSessionId: number | string;
  eventType: string;
...
async function ensureAiFeedbackTables(pgPool: PgPool, { schema }: { schema: string }): Promise<void> {
...
async function createFeedbackSession(pgPool: PgPool, { schema, session }: { schema: string; session: FeedbackSession }) {
...
async function updateFeedbackSession(pgPool: PgPool, { schema, sessionId, patch }: { schema: string; sessionId: number | string; patch: FeedbackSessionPatch }) {
  return pgPool.get(schema, `
...
async function addFeedbackEvent(pgPool: PgPool, { schema, event }: { schema: string; event: FeedbackEvent }) {
  return pgPool.get(schema, `
...
async function listFeedbackEvents(pgPool: PgPool, { schema, feedbackSessionId }: { schema: string; feedbackSessionId: number | string }) {
...
async function clearFeedbackEditEvents(pgPool: PgPool, { schema, feedbackSessionId }: { schema: string; feedbackSessionId: number | string }) {
...
async function getFeedbackSessionById(pgPool: PgPool, { schema, id }: { schema: string; id: number | string }) {
  return pgPool.get(schema, `
    SELECT *
...
async function getFeedbackSessionBySource(pgPool: PgPool, { schema, sourceRefType, sourceRefId }: { schema: string; sourceRefType: string; sourceRefId: number | string }) {
  return pgPool.get(schema, `
    SELECT *
    FROM ${schema}.ai_feedback_sessions
...
async function getFeedbackSessionSummary(pgPool: PgPool, { schema, sinceDays = 30 }: { schema: string; sinceDays?: number }) {
...
async function getFeedbackFieldStats(pgPool: PgPool, { schema, sinceDays = 30, limit = 20 }: { schema: string; sinceDays?: number; limit?: number }) {
  return pgPool.query(schema, `
    SELECT
      field_key,
      event_type,
...
async function getFeedbackSessions(pgPool: PgPool, { schema, sinceDays = 30, limit = 50 }: { schema: string; sinceDays?: number; limit?: number }) {
  return pgPool.query(schema, `
    SELECT
      id,
      company_id,
...
async function getFeedbackDailyStats(pgPool: PgPool, { schema, sinceDays = 30 }: { schema: string; sinceDays?: number }) {
  return pgPool.query(schema, `
    SELECT
