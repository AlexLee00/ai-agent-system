...
type TeamMeta = {
  schema: string;
  title: string;
  thresholdTeam: string;
  learningProfile: string;
};

type IntentHealthTarget = {
  key: string;
  schema: string;
  title: string;
  learningProfile: string;
...
type PromotionCandidate = {
  ...
  latest_event_metadata?: Record<string, unknown> | null;
};
...
const AUTO_PROMOTE_THRESHOLDS: Record<string, Threshold> = {
...
function getTeamIntentMeta(team = ''): TeamMeta | null {
  return TEAM_INTENT_META[String(team || '').trim().toLowerCase()] || null;
}

function buildUnsupportedTeamIntentMessage(): string {
  return `⚠️ 지원하지 않는 팀입니다. (${SUPPORTED_TEAM_INTENTS.join(', ')})`;
}

function buildTeamIntentReportFrame(
  team = '',
  teamMeta: TeamMeta | null,
  { promotions = '', unrecSummary = '' }: { promotions?: string; unrecSummary?: string } = {},
): string {
...
function buildTeamRollbackOptions(
  team = '',
  teamMeta: TeamMeta | null,
  getLearningPath: (name?: string) => string,
): { schema: string; title: string; learningPath: string } | null {
  void team;
...
function buildIntentEngineHealthLines(targets: IntentHealthTarget[] = [], summaries: Record<string, TeamSummaryEntry> = {}): string[] {
...
function buildIntentEngineHealthReportFrame(
  targets: IntentHealthTarget[] = [],
  summaries: Record<string, TeamSummaryEntry> = {},
): { title: string; sections: Array<{ title: string; lines: string[] }>; footer: string[] } {
  return {
    title: '🩺 인텐트 엔진 상태',
    sections: [
      {
...
function buildAutoLearnPattern(text = ''): string | null {
  const normalized = normalizeIntentText(text);
  if (!normalized) return null;
  return normalized.split(' ').map(escapeRegex).join('\\s+');
}

function summarizeIntentFamily(intent = ''): string {
...
function isSafeAutoPromoteIntent(intent = ''): boolean {
  const value = String(intent || '').trim();
...
function getAutoPromoteThreshold(intent = '', options: { team?: string; profile?: string } = {}): Threshold {
  const value = String(intent || '').trim();
...
function evaluateAutoPromoteDecision({
  ...
  team?: string;
  profile?: string;
} = {}): { allowed: boolean; reason: string; threshold: Threshold } {
...
function loadLearnedPatternsFromFile(filePath: string): LearnedPattern[] {
  try {
    if (!filePath || !fs.existsSync(filePath)) return [];
    const raw = JSON.parse(fs.readFileSync(filePath, 'utf8')) as LearnedPatternRecord[];
    return raw
...
function createDynamicExampleLoader<TInput, TOutput>({
  ttlMs = 5 * 60 * 1000,
  fetchRows,
  formatRow = (row: TInput) => row as unknown as TOutput,
}: {
  ttlMs?: number;
  fetchRows?: (() => Promise<TInput[]>) | undefined;
  formatRow?: (row: TInput) => TOutput;
} = {}) {
  let cache: TOutput[] = [];
  let lastLoadedAt = 0;

  return async function loadDynamicExamples(): Promise<TOutput[]> {
    const now = Date.now();
...
function createPromotedIntentExampleLoader({
  ...
  ttlMs?: number;
  fetchRows?: (() => Promise<SpeedExampleRow[]>) | undefined;
  ...
  return createDynamicExampleLoader({
    ttlMs,
    fetchRows,
    formatRow: (row: SpeedExampleRow) => formatPromotedIntentExample(row, { maxTextLength, confidence }),
  });
}

function formatPromotedIntentExample(
  row: SpeedExampleRow = {},
  { maxTextLength = 60, confidence = 0.9 }: { maxTextLength?: number; confidence?: number } = {},
): string {
...
function buildDynamicExampleSection(examples: string[] = []): string {
  if (!Array.isArray(examples) || examples.length === 0) return '';
  return examples.filter(Boolean).join('\n');
}

function injectDynamicExamples(
  promptTemplate = '',
  examples: string[] = [],
  { placeholder = '{DYNAMIC_EXAMPLES}' }: { placeholder?: string } = {},
): string {
  return String(promptTemplate || '').replace(placeholder, buildDynamicExampleSection(examples));
}

function formatIntentConfidence(value: number | string | null | undefined, digits = 0): string {
  const ratio = Number(value || 0);
  return `${(ratio * 100).toFixed(digits)}%`;
}

function getPromotionCandidateStatus(candidate: PromotionCandidate = {}): string {
  if (candidate.latest_event_type) return candidate.latest_event_type;
  if (candidate.auto_applied) return 'auto_applied';
  if (candidate.suggested_intent) return 'candidate';
  return 'unlinked';
}

function getPromotionEventReason(metadata: Record<string, unknown> = {}): string {
  if (!metadata || typeof metadata !== 'object') return '';
  return metadata.reason ? String(metadata.reason) : '';
}

function buildPromotionFilterBits(filters: PromotionFilters = {}): string[] {
  const bits: string[] = [];
...
function parseUnrecognizedQuery(raw = ''): { summaryOnly: boolean } {
...
function parsePromotionQuery(raw = ''): PromotionFilters {
  const query = String(raw || '').trim().toLowerCase();
  const filters: PromotionFilters = { applied: null, intent: null, eventsOnly: false, eventType: null, actor: null, summaryOnly: false, thresholdsOnly: false };
...
function buildPromotionFamilySummary(rows: PromotionFamilyRow[] = []): Array<{ family: string; total: number; applied: number; pending: number; occurrences: number }> {
  const familyMap = new Map<string, { family: string; total: number; applied: number; pending: number; occurrences: number }>();
...
function buildPromotionEventLines(events: PromotionEvent[] = []): string[] {
...
function buildUnrecognizedSummary(
  rows: UnrecognizedRow[] = [],
  resolveCandidate: (row: UnrecognizedRow) => PromotionCandidate | null = () => null,
): { llmIntentCounts: Array<[string, number]>; candidateStatusCounts: Array<[string, number]> } {
...
function buildUnrecognizedEntryLine(row: UnrecognizedRow = {}): string {
  const promoted = row.promoted_to ? ` ✅→${row.promoted_to}` : '';
  const sample = String(row.text || '').slice(0, 50);
  return `  [${row.cnt}회] "${sample}"${promoted}`;
}

function buildUnrecognizedCandidateLine(candidate: PromotionCandidate = {}): string {
...
function buildUnrecognizedCandidateStatusLine(candidate: PromotionCandidate = {}): string {
...
function buildPromotionCandidateLine(candidate: PromotionCandidate = {}): string[] {
  if (!candidate || !candidate.suggested_intent) return [];
  const badge = candidate.auto_applied ? '✅자동반영' : '🕓후보';
  const conf = formatIntentConfidence(candidate.confidence);
...
function buildPromotionCandidateStatusLine(candidate: PromotionCandidate = {}): string {
  if (!candidate || !candidate.latest_event_type) return '';
  const metadata = candidate.latest_event_metadata && typeof candidate.latest_event_metadata === 'object'
    ? candidate.latest_event_metadata
    : {};
...
function buildPromotionThresholdLines(thresholds: Record<string, Threshold> = AUTO_PROMOTE_THRESHOLDS): string[] {
  return Object.entries(thresholds).map(([key, value]) => `  ${key}: ${value.minCount}회 / ${formatIntentConfidence(value.minConfidence)}`);
}

function buildPromotionThresholdSection(thresholds: Record<string, Threshold> = AUTO_PROMOTE_THRESHOLDS): string[] {
  return [
    '자동 반영 기준:',
...
function buildTeamPromotionThresholdLines(team = ''): string[] {
  const profile = AUTO_PROMOTE_TEAM_PROFILES[String(team || '').trim()] || {};
  const rows = Object.entries(profile);
  if (rows.length === 0) return [];
  return rows.map(([key, value]) => `  ${key}: ${value.minCount}회 / ${formatIntentConfidence(value.minConfidence)}`);
}

function buildPromotionPolicyNoteLines(windowDays = AUTO_PROMOTE_DEFAULTS.windowDays): string[] {
  return [
...
function buildPromotionSummaryLine(summary: PromotionSummary = {}, fallbackTotal = 0): string {
  return `요약: 전체 ${summary?.total_count ?? fallbackTotal}건 | 자동반영 ${summary?.applied_count ?? 0}건 | 후보 ${summary?.pending_count ?? 0}건`;
}

function buildPromotionFamilySection(
  familyRows: Array<{ family: string; total: number; applied: number; pending: number; occurrences: number }> = [],
...
function buildPromotionEventSection(events: PromotionEvent[] = [], options: { showEmpty?: boolean } = {}): string[] {
  if (events.length > 0) return ['', '최근 변경:', ...buildPromotionEventLines(events)];
  if (options.showEmpty) return ['', '최근 변경: 없음'];
  return [];
}

function buildUnrecognizedSummaryReport(
  title = '미인식 명령',
  summary: { llmIntentCounts: Array<[string, number]>; candidateStatusCounts: Array<[string, number]> } = { llmIntentCounts: [], candidateStatusCounts: [] },
): string {
  const lines = [`❓ ${title}`];
  lines.push('');
...
function buildUnrecognizedDetailFooter(): string[] {
  return [
    '',
...
function buildPromotionCompactCandidateLine(candidate: PromotionCandidate = {}): string {
  if (!candidate || !candidate.suggested_intent) return '';
  const badge = candidate.auto_applied ? '✅자동반영' : '📝후보';
...
function buildPromotionCandidateWhere(filters: PromotionFilters = {}, { tableAlias = '' }: { tableAlias?: string } = {}): QueryParts {
...
function buildPromotionEventWhere(filters: PromotionFilters = {}): QueryParts {
  const clauses: string[] = [];
  const params: unknown[] = [];
