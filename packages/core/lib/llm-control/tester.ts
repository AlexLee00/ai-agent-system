...
async function refreshGeminiToken() {
  ...
  const profileKey = Object.keys(profiles.profiles)
    .find((key) => profiles.profiles[key].provider === 'gemini-cli-oauth' || profiles.profiles[key].provider === 'google-gemini-cli');
...
async function benchmarkModel(modelId, ctx, options = {}) {
  ...
  for (let index = 0; index < runs; index += 1) {
    try {
      let result;
      if (provider === 'gemini-cli-oauth' || provider === 'google-gemini-cli') result = await testGemini(modelId, ctx.geminiToken, prompt);
