...
function _kstNow() { return kst.datetimeStr(); }
function _kstDate() { return kst.today(); }
...
async function _checkEmergencyLimits(cost, team, meta = {}) {
  ...
  try {
    ...
    const hrRow = await pgPool.get('reservation', `SELECT COALESCE(SUM(cost_usd),0)::float AS t FROM llm_usage_log WHERE created_at::timestamptz > NOW() - INTERVAL '1 hour'`);
    ...
    const dyRow = await pgPool.get('reservation', `SELECT COALESCE(SUM(cost_usd),0)::float AS t FROM llm_usage_log WHERE created_at::date = CURRENT_DATE`);
    ...
    if (!BATCH_TEAMS.has(team)) {
      ...
      if (!hasScopedSymbol) {
        const r10 = await pgPool.get('reservation', `SELECT COALESCE(SUM(cost_usd),0)::float AS t FROM llm_usage_log WHERE team = $1 AND created_at::timestamptz > NOW() - INTERVAL '10 minutes'`, [team]);
        const p10 = await pgPool.get('reservation', `SELECT COALESCE(SUM(cost_usd),0)::float AS t FROM llm_usage_log WHERE team = $1 AND created_at::timestamptz BETWEEN NOW() - INTERVAL '20 minutes' AND NOW() - INTERVAL '10 minutes'`, [team]);
      ...
      if (hasScopedSymbol) {
        const sr10 = await pgPool.get('reservation', `SELECT COALESCE(SUM(cost_usd),0)::float AS t FROM llm_usage_log WHERE team = $1 AND market = $2 AND symbol = $3 AND created_at::timestamptz > NOW() - INTERVAL '10 minutes'`, [team, meta.market, meta.symbol]);
        const sp10 = await pgPool.get('reservation', `SELECT COALESCE(SUM(cost_usd),0)::float AS t FROM llm_usage_log WHERE team = $1 AND market = $2 AND symbol = $3 AND created_at::timestamptz BETWEEN NOW() - INTERVAL '20 minutes' AND NOW() - INTERVAL '10 minutes'`, [team, meta.market, meta.symbol]);
        ...
        if (prevSymbol > 0.01 && recentSymbol / prevSymbol >= EMERGENCY_LIMITS.spikeRatio) {
          _triggerEmergency(`[${team}:${meta.market}:${meta.symbol}] 10분 급등 ${(recentSymbol / prevSymbol).toFixed(1)}배 ($${recentSymbol.toFixed(4)})`, recentSymbol, scopedGuard);
        }
      }
    }
  } catch (e) {
...
async function logLLMCall({
  team, bot, model,
  market = null, symbol = null, guardScope = null, requestType = 'unknown',
  inputTokens = 0, outputTokens = 0, costUsd, cacheHit = false, latencyMs = null, success = true, errorMsg = null,
}) {
...
async function getDailyCost(team, dateKst) {
  ...
  if (team) {
    const row = await pgPool.get('reservation', `
      SELECT SUM(cost_usd)::float AS total, SUM(cache_hit) AS cache_hits, COUNT(*) AS calls
      FROM llm_usage_log
  ...
  return pgPool.query('reservation', `
    SELECT team, SUM(cost_usd)::float AS total, SUM(cache_hit) AS cache_hits, COUNT(*) AS calls
    FROM llm_usage_log
...
async function analyzeCostTrend(days = 14) {
  ...
  const weekOverWeek = lastWeekCost > 0 ? ((thisWeekCost - lastWeekCost) / lastWeekCost * 100) : null;
  const avgDaily = rows.length > 0 ? rows.reduce((s, r) => s + r.daily_cost, 0) / rows.length : 0;
...
async function buildWeeklyFeedbackReport() {
  ...
  const freeCalls = models.filter(m => m.model.includes('llama') || m.model.includes('gemini')).reduce((s, m) => s + parseInt(m.calls), 0);
  const lines = [
    ...
    trend.weekOverWeek != null ? `  전주 대비: ${trend.weekOverWeek > 0 ? '+' : ''}${trend.weekOverWeek.toFixed(1)}%` : `  전주 대비: 데이터 부족`,
    ``,
    `🤖 모델별 사용 TOP5`,
    ...models.slice(0, 5).map(m => `  ${m.team}/${m.bot} [${m.model.slice(0, 20)}] $${parseFloat(m.total_cost).toFixed(4)} ${m.calls}회`),
    ``,
  ];
