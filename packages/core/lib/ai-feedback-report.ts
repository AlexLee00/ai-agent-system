type FeedbackSummary = {
  ...
  acceptedWithoutEditSessions: number;
...
function formatPercent(numerator: unknown, denominator: unknown): string {
...
function buildFeedbackSummaryLines({ schema, sinceDays, summary, fieldStats, sessions }: FeedbackReportInput): string {
  const lines = [
...
function buildFeedbackDailyLines({ schema, sinceDays, dailyStats }: FeedbackReportInput): string {
  const lines = [
...
function buildFeedbackCsvRows(mode: string, report: FeedbackReportInput): unknown[][] {
  if (mode === 'daily') {
    return [
...
function buildFeedbackCsv(mode: string, report: FeedbackReportInput): string {
